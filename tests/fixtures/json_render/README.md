...
for the catalogue-parity test (`tests/catalog_parity.rs`, **TUIDASH-010**), which
fails if the in-crate `Catalog::base()` mirror drifts from it or if the component
registry stops mapping any name it lists. Refresh it when the web catalogue
changes. Fully automated diffing against a TS-exported JSON Schema is gated on
DASHAI-002.
