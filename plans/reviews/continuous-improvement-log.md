...
- **Outcome:** Bounded ETXTBSY retry at the `eval_json` spawn site, extracted as the unit-testable free function `spawn_with_etxtbsy_retry(spawn, max_retries)` (`ETXTBSY_MAX_RETRIES = 5` retries, i.e. 6 total spawn calls worst-case, `1<<retries` ms back-off). The transient condition is detected via `ErrorKind::ExecutableFileBusy` (stable on the repo's Rust 1.95.0), avoiding a hard-coded errno 26 and a `libc` dependency. The `Command` is reconstructed per attempt for fresh stdio pipes. TDD: three unit tests proven red first (compile error, helper absent) — recovers-after-transient-ETXTBSY, gives-up-after-max-retries (last error preserved, exactly 1 initial attempt + N retries), and does-not-retry-non-ETXTBSY (ENOENT propagates on the first call). Validated with the 20x `cargo test -p eddacraft-anvil-policy` stress loop: zero ETXTBSY across all runs.
