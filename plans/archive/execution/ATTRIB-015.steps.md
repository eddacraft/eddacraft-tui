...
| Manifest location | `tools/dev/package.json` (NOT root) | Spec line 332-334 explicit. Keeps the dev-tooling intent declared separately from the workspace root manifest (which is the pnpm-workspace manifest, not a "what we depend on at build time" surface). |
| Licences to add to `licences.toml` for ATTRIB-015 | `Python-2.0` (argparse@2.0.1), `BlueOak-1.0.0` (npm-owned minimatch@10.2.5), `0BSD` (Microsoft tslib@2.8.1), `CC-BY-3.0` (spdx-exceptions, transitively pulled in by license-checker itself) | All four packages flagged by the spike-plus-implementation are mainstream, widely-used, OSI-approved permissive (BlueOak / 0BSD), Python Software Foundation (Python-2.0), or Creative Commons attribution (CC-BY-3.0 — data-files licence used by SPDX exception lists). `CC-BY-3.0` was added on top of the original spike's three after `license-checker` itself entered the attribution graph as a devDep — `spdx-exceptions` (data files used by `spdx-expression-parse`) ships under it. Adding all four to `about = true; deny = true` keeps the canonical allow-list authoritative across all consumers (Rust + Node + future Go/Python). Compound expression `(BSD-2-Clause OR MIT OR Apache-2.0)` on `run-con@1.3.2` passes `license-checker --onlyAllow` without intervention (all three components allowed); compound `(MIT AND CC-BY-3.0)` on `spdx-license-ids` passes once both `MIT` and `CC-BY-3.0` are in the allow-list. |
| Workflow trigger | Existing `acknowledgements-kit.yml` already fires on `licences.toml` changes; existing `rust.yml::acknowledgements-diff` already fires on Cargo deps + workspace edits | No CI wiring change needed beyond the new `npm ci` + PATH-prepend steps. The new `tools/dev/package.json` does not need pnpm-workspace inclusion (see next row). |
| pnpm-workspace inclusion | `tools/dev` **excluded** from `pnpm-workspace.yaml` via `!tools/dev` | pnpm workspace hoisting otherwise resolves a `glob` major incompatible with `license-checker@25.0.1`'s `read-package-json` transitive (`TypeError: glob is not a function`). An isolated `npm ci` inside `tools/dev` gives a clean transitive tree. The trade-off: contributors must `(cd tools/dev && npm ci)` once before running `generate-acknowledgements.sh` locally; CI does the same step automatically. |
| `tools/dev/package-lock.json` | Checked in (with a `.gitignore` exception) | Reproducible Node attribution output. Without the lock file, `npm install` resolves fresh transitive each run; a transitive version shift would silently break the acknowledgements freshness gate. |
