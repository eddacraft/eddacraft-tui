...
- **Status:** Complete — `github.com/eddacraft/anvil` publishes release
  binaries; release workflow writes to it via `ANVIL_RELEASES_TOKEN`.
...
- **Status:** Complete — `install.sh` lives at repo root; cargo-dist also
  generates `eddacraft-anvil-installer.sh` on each release.
...
- **Status:** Complete — `eddacraft-anvil-installer.ps1` is generated by
  cargo-dist and published to GitHub Releases (referenced in install.sh
  banner). `irm …/eddacraft-anvil-installer.ps1 | iex` is the Windows path.
...
- **Status:** Complete — Pages serves from `eddacraft/anvil` with the
  `install.eddacraft.ai` CNAME (see DIST-006).
...
- **Status:** Complete — `install-cname-eddacraft-ai` record defined in
  `infra/src/dns/eddacraft-ai.ts` pointing at `eddacraft.github.io`.
...
- **Status:** Complete — `.github/workflows/release.yml` drives the
  cargo-dist multi-target build and cross-repo publish to `eddacraft/anvil`.
...
- **Status:** Complete — release workflow patches and publishes
  `Formula/anvil.rb` to `eddacraft/homebrew-tap` on every tagged release.
...
- **Status:** Deferred — optional polish for developer-audience Windows
  users; WinGet (DIST-010) covers the primary install gap. Revisit if
  there is measurable scoop-install demand.
