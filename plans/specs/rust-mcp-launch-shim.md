...
| --- | :---: | --- |
| `workspaceRoot` | no | Defaults to server cwd. Must resolve to an existing directory when provided. |
...
1. If the daemon pre-write validation RPC is available and compatible, call it.
2. Otherwise use the embedded Rust validation fallback.
