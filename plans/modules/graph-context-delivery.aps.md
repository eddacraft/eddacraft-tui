...
- **Status:** Merged 2026-06-26 via #2942 — deterministic `token_reduction`
  ...
  real `ImpactOutcome::Ready(ImpactReport)` (asserted to deserialise back), a
  golden test pins the published numbers, and the README discloses the
  estimator's source-vs-identity bias. Recorded means: 89.2% reduction vs
  whole-repo, 87.3% vs neighbourhood. Promoted Draft → Ready → In Progress →
  Merged (sole dependency GCTX-023 Done; module entry gates satisfied).
...
- **Status:** Merged 2026-06-26 via #2952 — `docs/guides/ai-context-delivery.md`
  ...
  launch-validation (`anvil_validate_write`) vs graph-context distinction.
  Authored against the live surface and Council/Copilot fact-checked (commands,
  tool names, flag semantics, and outcome states verified from source). Promoted
  Draft → In Progress → Merged (deps GCTX-023 Done, GCTX-030 Merged).
...
| 3 — Resources, Benchmarks, Docs | 3 | GCTX-030 Merged #2772; GCTX-031 Merged #2942; GCTX-032 Merged #2952 |
| **Total** | **14** | **14/14** |
