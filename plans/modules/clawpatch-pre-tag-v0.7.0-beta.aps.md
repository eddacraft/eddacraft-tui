...
- **Severity / Triage / Category:** medium / contract-mismatch / api-contract
- **Confidence:** medium
- **Status:** Deferred (release-council verdict 2026-05-20, kernel-maintainer; tracked in #1737 — 2026-06-03 CIB-039 disposition: the sole externally-tracked carry-over; the test-support extraction lands with the first `anvil-rmcp`/RTAI-006 integration, so it does not block archiving this tracker)
...
- **Confidence:** medium
- **Status:** Ship (2026-06-03 CIB-039 disposition — declined — the JS/TS workspace is on the deliberate retirement path; the @types/node-vs-engines.node drift is left to bitrot per that posture rather than fixed forward)
...
- **Confidence:** medium
- **Status:** Ship (2026-06-03 CIB-039 disposition — declined — Vitest `__tests__` discovery / `passWithNoTests` on the retiring JS/TS workspace; left to bitrot)
...
- **Severity / Triage / Category:** medium / test-gap / test-gap
- **Confidence:** high
- **Status:** Ship (2026-06-03 CIB-039 disposition — obsolete — the dual-run parity test compares against the legacy TS engine, which is on the deliberate retirement path; a TS parity engine will not be wired. The scaffold is harmless and left as-is)
...
- **Severity / Triage / Category:** low / risk / build-release
- **Confidence:** high
- **Status:** Ship (2026-06-03 CIB-039 disposition — declined — inert root `tsconfig` includes on the retiring JS/TS workspace; left to bitrot)
...
- **Severity / Triage / Category:** low / test-gap / test-gap
- **Confidence:** medium
- **Status:** Ship (2026-06-03 CIB-039 disposition — accepted — production target is Linux; positive OPA assertions stay cfg-gated off Windows with the path-glob follow-up tracked in-code, and the negative-result cases already run on Windows)
...
### CLAWP-065: scan_buffer cannot enforce session ownership because the contract omits session_id

- **Clawpatch finding:** `fnd_sig-feat-cli-command-43c5f1e5c2-_2cf64bc98e`
- **Feature:** `feat_cli-command_43c5f1e5c2` — Rust command eddacraft-anvil-intercept
...
- **Verification (2026-06-01):** Confirmed still open against `origin/main`.
  ...
  authenticated `session_id`, so the mid-edit scan path cannot bind a request
  to a registered session or reject one issued under another session's
  identity. The existing `env_agent_tag` spoof cross-check (MLP2-025b) provides
  *attribution* defence (`Cross::Spoofed` on out-of-lineage forgeries) but is
  not the connection-bound session-ownership check this finding asks for.
  The eleven other actionable §A findings from the 2026-05-31 v0.7.3-beta
  sweep had already shipped untracked (see the provenance note above); this is
  the sole remaining one.
- **Recommendation:** Add a `session_id` (or equivalent authenticated session
  binding) to the `scan_buffer` request contract, validate it against the
  connection or registered lineage before scanning, and return a structured
  JSON-RPC error on mismatch.
- **Evidence:** `crates/anvil-intercept/src/midedit.rs:47` (`ScanBufferRequest`
  — no `session_id` field), `crates/anvil-intercept/src/main.rs:94` (daemon
  entrypoint, original audit anchor), `crates/anvil-intercept/tests/midedit_contract.rs:51`
  (contract fixtures; cross-*session* rejection at `:1120` covers subscriptions,
  not `scan_buffer`).
- **Source:** Clawpatch pre-tag sweep 2026-05-31 (full finding body in
  `plans/audits/2026-05-31-clawpatch-v0.7.3-beta.json`; triage in
  `plans/reviews/2026-05-31-clawpatch-triage.md` §A).
