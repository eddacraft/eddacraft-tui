...
- **Status:** Draft
- **Intent:** CIB-022 made the `plans/index.aps.md` per-module counts derived +
  CI-enforced, but kept them as a hand-editable cell. So every PR that completes
  a work item still *edits its module's index row* (the `N/M` token + prose), and
  two concurrent PRs touching the same module's row collide there — the index row
  is a shared mutable line, the same single-shared-file contention `merge=union`
  fixed for the CI log (CIB-021) but couldn't fix for a counter cell. Observed
  live: CIB-017/-018/-019/-024 in flight together kept making each other's PRs
  conflict on the CIB row, forcing serialized rebases. This is the deferred
  "option B" from CIB-022 (the operator chose option A — keep prose — at the
  time), now re-surfaced by real multi-PR contention rather than the original
  same-cell-merge concern.
- **Expected Outcome:** PRs stop editing `plans/index.aps.md`'s module rows by
  hand. Candidate shapes (pick at design time):
  1. **Fully generate the module-status table** from the `.aps.md` module files
     (status + `done/total`), like `docs/indexes/` are generated by
     `docs-index.mjs` — the per-row prose moves into each module (where the
     per-item `Status:` lines already hold the PR refs), so a PR only edits its
     own module file and never the shared index.
  2. **Split the index** so each module's row lives in a per-module fragment that
     a generator concatenates (the CI-log-fragments idea, applied to the index).
  Either way, `aps:index --check` stays the freshness gate; the win is that two
  PRs completing items in different modules never touch the same file.
- **Validation:** two branches that each complete an item in a *different* module
  rebase/merge with **zero** conflict on `plans/index.aps.md`; the generated
  rows match the module sources; `aps:index:check` is green.
- **Identified From:** CIB-019/-024 session 2026-05-26 — repeated CIB-row merge
  conflicts across concurrent in-flight CIB PRs; the count cell is the residual
  shared-file contention CIB-022 left in place.
- **Files:** `scripts/aps/index-counts.mjs` (→ a fuller index generator) or a new
  `scripts/aps/` generator + `plans/index.aps.md` structure; CI wiring already
  exists via `aps:index:check`.
- **Coordinates with:** CIB-022 (extends it — count derivation → full row
  generation), CIB-021 (same shared-append-file problem, different file),
  CIB-023 (sibling index-honesty drift check).
- **Confidence:** medium — clear precedent (`docs-index.mjs`) and the generator
  already exists, but fully generating the index means relocating the curated
  per-row prose into the modules and rewriting every index row; a real
  restructure, and worth confirming the contention recurs enough to justify it
  before building.
