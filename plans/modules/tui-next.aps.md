...
- Changes to crates.io publication policy (D-TUIR-005 / D-TUIR-006
  binding).
- Splitting `eddacraft-tui` into multiple published crates. If TUIN-001
  / TUIN-003 land a separate `eddacraft-tui-cli` or `-lifecycle` crate,
  that crate is born under TUIN; further splits get their own module.
...
**D-TUIN-001:** Argument-parser policy in `eddacraft-tui` core

- **Proposed resolution:** `eddacraft-tui` core has zero
  argument-parser dependency. Any helper that needs argument parsing
  ships either (a) behind an opt-in feature flag defaulted OFF, or
  (b) in a separate crate (`eddacraft-tui-cli`). Default-on argument
  parsing is rejected because it breaks the "library, not application"
  contract that lets downstream consumers (including Anvil) pick
  their own parser without inheriting transitive `clap` weight. The
  TUIR Out-of-Scope clause forbidding `clap` in core is upheld and
  carried forward.
- **Status:** Proposed (ratify via ADR-050 in TUIN-001).
...
- **Proposed resolution:** TTY / alt-screen / terminfo / colour-depth
  ...
  enums, not raw capability bits (rationale captured in ADR-050).
- **Status:** Proposed.
...
- **Proposed resolution:** Lifecycle helpers (alt-screen enter/exit,
  ...
  defaulted OFF. Apps that already own their lifecycle (Anvil) ignore
  the feature; apps that want managed lifecycle (downstream
  consumers, demos) opt in. Default-on is rejected because it would
  silently take over terminal state for any consumer that pulls
  `eddacraft-tui` for widget rendering only.
...
parser in core" and "mode probes are core API" — both are easy to
re-litigate without it.

**Outcome:** `plans/decisions/050-eddacraft-tui-cli-policy.md` with
status Proposed, covering parser-free core, opt-in feature gates,
mode-detection helper shape (typed enums), rejected alternatives
(`clap` default-on, `lexopt` as core dep, no helpers at all), and
the carry-forward of the TUIR clap clause.

**Validation:** `pnpm adr:check`; ADR appears in `DECISION-LOG.md`;
ADR number does not collide (next free at drafting was `050`; if
taken, re-allocate and update this work item).
...
consumers from TUIR-001 baseline) expects from terminal lifecycle
ownership and CLI helpers — do they own lifecycle, want it managed,
or wrap it. Same question for argument parsing.

**Outcome:**
`plans/specs/2026-XX-XX-eddacraft-tui-post-migration-survey.md` with
one section per consumer documenting current behaviour, desired
behaviour, and any pain points. Drives feature-flag default
sanity-check for D-TUIN-003 and confirms D-TUIN-001's "parser-free
core" matches consumer reality.

**Validation:** survey doc lists each known consumer from the TUIR-001
baseline; gaps explicitly flagged for follow-up. ADR-050 cites the
survey.
