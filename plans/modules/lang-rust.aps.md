...
- **Validation:** `cargo test -p eddacraft-anvil-kernel --test languages` (or the languages module tests) passes with Rust variant added + grammar_version distinctness test extended; `cargo check -p eddacraft-anvil-kernel` succeeds after dep addition; smoke: `cargo test -p eddacraft-anvil-kernel -- --quiet` includes a new test that does `Language::from_path("foo.rs") == Some(Rust)` and parses a minimal real .rs file from the kernel crate itself without panic. Reproducible via `cargo test` in the monorepo root.
...
- **Validation:** Kernel unit tests (new `extract/rust.rs` tests + integration in parser tests) assert the listed shapes produce correct `SymbolNode` + `ImportEdge` entries in `FileSymbols`; `cargo test -p eddacraft-anvil-kernel --test parser` (or equivalent) passes; additionally `cargo test -p eddacraft-anvil-kernel -- --quiet` exercises a fixture from the kernel's own src/ that uses `crate::` and `super::`. Workspace resolution smoke: `cargo metadata --format-version 1 | grep -o '"name":"eddacraft-anvil-kernel"'` + parse of a workspace member .rs succeeds and resolves a relative import to the correct file path in the symbols.
