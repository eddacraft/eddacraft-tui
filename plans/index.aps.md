...
| [tui-reintegration](./modules/tui-reintegration.aps.md)                                                | TUIR       | 2/8        | Supersedes TUIMIRROR. Brings `eddacraft-tui` canonical source back into `crates/eddacraft-tui/`, mirrors out to `eddacraft/eddacraft-tui:main` (read-only), publishes to crates.io from the Anvil canonical source. Pins crate location, mirror role, sync direction + automation, crates.io publish source, versioning/changelog ownership, CI gate split, internal-vs-external consumption contract, and backport/mirror conflict policy — **In Progress** (ADR-047 Accepted 2026-05-22; TUIR-001 baseline `completed`; TUIR-002 import `completed` 2026-05-23 — `crates/eddacraft-tui/` landed at v0.2.2 with mirror banner, D-TUIR-019 records the in-crate clippy-pedantic delta from D-TUIR-007 × D-TUIR-016 collision; TUIR-003..TUIR-008 execution-tokens stay `open` pending per-task prerequisites) |
