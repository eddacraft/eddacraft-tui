...
| 4 | DOCRB-010 — independent documentation re-baseline verification | In Progress | [docs-rebaseline](./modules/docs-rebaseline.aps.md), [design](./specs/2026-08-16-docs-rebaseline.md) | DOCRB-009 is Merged via #4099; the four clean-room journeys completed and three residuals are separately authorised as #4114/#4115/#4116. | Rerun exact-head independent verification and full Council, then publish and rebase-merge on green; preserve stored 10/11 until post-merge reconciliation. |
...
| 10 | GTAO — gate-time catalogue on always-on surfaces | Schedule | [gate-time-always-on](./modules/gate-time-always-on.aps.md) | AST/MCP check are dark on the golden-path daemon; full gate is opt-in. Operator added a bounded Python AST slice 2026-08-22. Not this cut claim. | Wave 1 Merged via #4084. Next: promote GTAO-003/005 when ready. Do not auto-run full `anvil gate` on save; do not convert regex PY-008 to AST. |
## Release Plan
...
| [gate-time-always-on](./modules/gate-time-always-on.aps.md) | GTAO | Proposed | 3/10 (created 2026-08-22: AST dark on daemon/MCP golden path; full gate stays commit/CI. Same day: bounded Python AST companions for PY-004/008/009 regex blinds. **GTAO-001/-002/-008 Merged via #4084**; -003..-007 Draft; -009/-010 Draft until promoted. Do not convert regex PY-* to AST. Not this cut claim.) | ADR-061/064/067/071/031/038; PYLAN grammar; RLB; CIB-294; CIB-332 |
...
Enterprise posture (2026-08-23, POLFIT-009): **no organisational policy
capability ships, and none is scheduled.** Organisational policy today means
hand-copying a pack directory into each repository — no distribution, no
inheritance, no versioning, no lifecycle. ORGHIER, POLLC, POLFED, COMPLY, and
CEWS are dormant with zero work items carrying a status; TRUST is Blocked.
ORGHIER is the root, so promoting it is the enterprise-tier trigger for the
rest, and its gate is demand-side: a concrete multi-repo/org signal or a
planning-council decision. Each module now carries a Posture block saying so.
...
| [policy-fit-for-purpose](./modules/policy-fit-for-purpose.aps.md) | POLFIT | Ready | 3/9 — **Conductor** for policy adoption readiness, created 2026-08-22 from a capability audit. **Current (2026-08-23):** POLFIT-001 and POLFIT-002 **Merged** via #4103 and #4104 (ADR-129 intra-repo surface precedence; ADR-130 authoring on-ramp), so both design gates are closed and POLFIT-003..-006 and -008 are no longer blocked by them. POLFIT-009 **Merged** via #4108 (all six enterprise modules now carry an explicit dormant/blocked posture). POLFIT-007 **Blocked 2026-08-23**: CPACKS-007 delivered (known-gaps copy, which the audit found did not exist), but CPACKS-006 is proven undeliverable as specified — the eval harness diffs `findings` while the shipped pack emits `warning` strings, so its suites cannot fail; three unblock paths recorded, each needing an owner decision. **Promoted Ready 2026-08-23:** POLFIT-007 (starter-pack residue) and POLFIT-009 (enterprise posture) advanced Draft -> Ready; neither depends on the design gates, and CPACKS-007 was promoted with them so -007's delegated work is fully authorised. **2026-08-22:** POLFIT-001 and POLFIT-002 advanced Draft -> Ready on operator instruction and run in parallel (POLFIT-002's POLFIT-001 dependency moved to `Coordinates with` as non-blocking — every on-ramp candidate targets the same pack surface). Ready authorises producing the two decision records, not implementing what they decide; POLFIT-003..-009 were Draft behind them at that point (superseded — see Current above). Owns two design gates: POLFIT-001 surface inventory and precedence (picks up the "policy merge semantics" carve-out ADR-120 left unowned) and POLFIT-002 the authoring on-ramp (sequences ACTAX Phase A against the OPAE-013..017 chain — **making policy easy to create**). Delegates to OPAE-010/-021/-022, CPACKS-006/-007, and DOCDEF-007; POLFIT-008 owns the ADR-026 registry-override surface outright because no vertical module claims the loader. Successor coordinator to POLRESET. |
...
| [org-policy-hierarchy](./modules/org-policy-hierarchy.aps.md)                     | ORGHIER | Draft    | **Dormant — demand-gated (POLFIT-009, 2026-08-23); nothing scheduled.** Root of the enterprise chain. POLENG/regorus, POLVAL (Done), EXCEPT store for tier exemptions (ADR-100), `crates/anvil-policy-engine`, `crates/anvil-kernel-types`; demand-gated (multi-repo/org signal or council) — restated 2026-07-11                                                   |
| [policy-lifecycle](./modules/policy-lifecycle.aps.md)                             | POLLC   | Draft    | **Dormant — blocked behind ORGHIER (POLFIT-009, 2026-08-23); nothing scheduled.** POLENG/regorus, POLVAL (Done), ORGHIER (the live prerequisite), `crates/anvil-policy-engine`, `crates/anvil-kernel-types`; grace periods restated in ControlDecision terms + EXCEPT relationship pinned 2026-07-11                                                                  |
...
| [trust-center-automation](./modules/trust-center-automation.aps.md)               | TRUST   | Blocked  | **Blocked — confirmed accurate (POLFIT-009, 2026-08-23); nothing scheduled.** CEWS and COMPLY remain Draft; Blocked posture retained 2026-07-17. Future publishing/freshness work targets the Rust CLI over COMPLY/CEWS contracts, not `anvil-policy`; rescope to policy+eval-only output remains an explicit alternative. |
...
| [compliance-policy-packs](./modules/compliance-policy-packs.aps.md)               | CPACKS  | In Progress    | Re-scoped 2026-07-11: the starter pack **shipped** (`anvil-baseline` via POLRESET-007 #3167 + OPAE-004 install); CPACKS-001..005 satisfied-by; **2026-08-24:** CPACKS-006 is **Blocked** — wiring anvil-baseline into `ci/eval/suites.json` was attempted and proven undeliverable as specified (the eval harness diffs `findings`; the pack emits `warning` strings, so its suites cannot fail). Not selectable; needs an owner decision on one of three recorded unblock paths. CPACKS-007 known-gaps docs audit **delivered** via [POLFIT-007](./modules/policy-fit-for-purpose.aps.md). No selectable live item remains. Broad OWASP/SOC2/ISO/GDPR/AI packs stay behind CPACKS-008 (COMPLY evidence semantics + AGOV signals). |
...
| [feature-flag-catalogue](./modules/feature-flag-catalogue.aps.md)                                     | FLAGCAT    | 11/16      | FLAGS and FLAGM complete; operational catalogue bootstrap and migrations shipped. FLAGCAT-008 remains Draft for the GA licence-gate review. FLAGCAT-010 **Merged 2026-08-20 via PR #4054** after ratifying the definitive product-feature catalogue contract. FLAGCAT-016 **Merged 2026-08-22 via PR #4086**, adopting `docs.access` in the live documentation shell. FLAGCAT-011 **Merged 2026-08-23 via PR #4111**, delivering the strict product-catalogue v2 schema, canonical cross-host inventory, immutable v1 compatibility view, and migration ledger without changing runtime enforcement. FLAGCAT-012..015 remain Draft in dependency order: host completeness gates, bidirectional flag linkage, generated views, then completeness-gated mapping to an approved potential Individual/Teams/Enterprise vocabulary — **In Progress** |
...
| [docs-rebaseline](./modules/docs-rebaseline.aps.md)                                                  | DOCRB      | 10/11      | **In Progress — high-priority engineering-effectiveness programme; not a release claim or cut gate.** DOCRB-001/-002/-003/-004/-005/-006/-007/-008/-009/-011 Merged (#3975/#3976/#4027/#4031/#4055/#4040/#4051/#4068/#4099/#4009). DOCRB-010 is In Progress under the approved four-path clean-room verification plan; stored progress remains 10/11 until post-merge reconciliation. The live sidebar unhide and live-nav check no longer wait on diagram work. Coordinates with DOCFRESH, DOCSYNC, DOCDEF, and DSITE without absorbing or closing their work. |
