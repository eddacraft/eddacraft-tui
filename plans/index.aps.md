...
| [symbol-call-graph](./modules/symbol-call-graph.aps.md) | GCALL | In Progress | 1/7 (filed 2026-06-17 to unblock GCTX-014 `anvil_find_callers`. Producer-side call-graph substrate — call-site extraction into `FileSymbols` + lifting `EdgeType::Calls` into the resident `SymbolGraph` via `apply_delta` + a bounded caller read API — within the ADR-031 save-time budget, behind a caller-egress privacy review. Not a GCTX item: GCTX consumes this substrate, mirroring how it consumes GV2. **GCALL-001 Merged 2026-06-17 via #2705** as ADR-086 (Accepted, operator) — caller = nearest enclosing named symbol else a synthetic file node; lift-time `re_resolve_calls` (alias/namespace/barrel-follow, default-export = Unresolved) scoped to `dependents_of`; best-effort static (overload fan-out capped + `heuristic:true`, unresolved drop + `partial`); identity-only egress via the CE-5 `GctxProjector`. **GCALL-002** (TS/JS extraction) now Ready — the next pick; GCALL-003 (resident edges + read API) follows; GCTX-014 blocks on GCALL-003 + GCALL-007.) | GV2, anvil-kernel-types (`EdgeType::Calls`), ADR-031, ADR-064, lang-python |
