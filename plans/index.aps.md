...
| [lang-rust](./modules/lang-rust.aps.md)         | RSTLAN | In Progress | 1     | §8.1 — RSTLAN-001 (grammar wiring) + RSTLAN-002 (Rust extraction) In Progress 2026-06-04 on `feat/rstlan-rust-grammar-extraction`; NBI re-eval complete 2026-06-03; ADR-065 (Rust-native) Accepted; re-score snapshot recorded (Rust #2 for dogfood); LANGTS/kernel gates satisfied; module promoted Proposed → Ready with 8 executable work items (grammar, extraction, T2 catalogue, entry, boundary per ADR, drift, validate surface, dogfood). Owner @aneki. (0/8) |
