...
| [operating-model-migration](./modules/operating-model-migration.aps.md)                               | OPMODEL    | 4/12       | Cross-cutting migration to the target Plan / Build / Release operating model — **In Progress**. OPMODEL-001 completed the current-state vs target-state migration map; OPMODEL-002 updated branching/worktree guidance; OPMODEL-003 aligned APS lifecycle and release metadata rules; OPMODEL-004 defined the release-record schema and authority contract. Remaining work covers deterministic agent guidance, review/council entrypoints, CI readiness, drift checks, and rollback playbooks. Implementation remains owned by specialist modules such as RELORCH and DOCGOV. |
