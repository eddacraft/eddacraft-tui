...
| Daemon-working product slate | Current candidate | MLP 18/18 Complete (Done 2026-05-13/-14); MLP2 2/60 (In Progress — MLP2-001 + MLP2-002 shipped 2026-05-14 via PR #1522; integration follow-ups split out from MLP-018 catalogue + 4 Council-filed hardening tasks in Group L); INTL 0/9 (In Progress — Wave 3 implementation lane opened 2026-05-14 on `feat/intl-launcher-v1`); carry-forward gates 6/6 confirmed (Wave 0 closed 2026-05-13) | Next tag candidate `v0.7.0-beta`; MLP v1 surface area shipped. Integration debt tracked module-locally in MLP2 — each of the 60 sub-items (Groups A–K from the MLP-018 catalogue + Group L production hardening from Council session `council-e2fdfc0c`) is now a first-class APS task. |
...
| N2 — Intercept Launcher v1 (INTL) | In Progress | 0/9 | `anvil-run` wrapped-launch ingress. New crate: `crates/anvil-run/` opened 2026-05-14 on `feat/intl-launcher-v1` with INTL-001..-009 implementation. 49 unit + 3 shell-integration tests green. Module status moved **Ready → In Progress**; the nine task statuses moved **Draft/Ready → In Progress** ahead of the PR. Progress count stays 0/9 until the PR merges (cleanup agent advances Merged → Complete per `plans/aps-rules.md`). |
...
| [intercept-launcher](./modules/intercept-launcher.aps.md) | INTL | In Progress | 0/9 | INTD; coordinates `AgentTag` proto with MLP-014 in next-release window; Wave 3 implementation lane opened 2026-05-14 on `feat/intl-launcher-v1` (`crates/anvil-run/`, 49 + 3 tests green) |
