...
| [ci-cd-validation](./modules/ci-cd-validation.aps.md)                                                 | CICD       | 2/12       | Specialist CI/CD and validation operating model implementation — **In Progress**. Coordinates with OPMODEL rather than replacing it; CICD-001 added CI cost/run-reason reporting via `pnpm ci:cost`; CICD-002 added the shared path/risk classifier via `pnpm ci:classify`. Remaining work covers local-first validation commands, trigger targeting, workflow decomposition, candidate-readiness CI touchpoints, and migration-safe validation cutover. |
