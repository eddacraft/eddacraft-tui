...
| ---- | --- | ---- | ------ | ------- | ----------- |
| 1 | DSV-051 — headless save-time driver closeout | In Progress | [daemon-save-time-validation](./modules/daemon-save-time-validation.aps.md) | **v0.9.0-beta usefulness cut-line:** DSV-048 Merged via PR #3186 (CLI driver entry), DSV-047 Merged via PR #3191 (daemon supervisor), DSV-049 Merged 2026-07-05 on `main` (status wire + `#[serde(other)] Unknown`), and DSV-050 Merged 2026-07-05 via PR #3200 (activation copy). Remaining wave: DSV-051 (E2E/runbook + Windows leg). | Complete DSV-051 runbook and E2E regression matrix for the headless save-time driver. |
...
| [cli-command-truth](./modules/cli-command-truth.aps.md)                                                 | CLICT     | 0/5        | CLI command documentation vs runtime audit — **In Progress** (CLICT-001 Ready: architecture reconciliation; CLICT-002..005 Proposed: policy, drift, watch, gate; operating model + slice queue in `docs/reviews/cli-command-truth-review.md`) |
