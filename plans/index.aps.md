...
| N1b — Multi-Layer Protection v2 (MLP2) | Draft | 0/56 | Follow-up module collecting the 56 integration items from MLP-018's catalogue. Closes the gap between every v1 primitive and the full surfaces it targets. 11 groups (A–K) covering daemon enforcement integration, witness-chain extensions, L4 policy execution, multi-session + fence isolation, cross-platform attribution, TS driver-client mirrors, baseline + identity wiring, hook + config completion, GH Action publishing, protection-claim render conformance, and Kindling activation orchestrator. Every task carries an explicit `Source:` line linking back to its originating MLP task / footnote / PR. |
...
| [rust-mcp-full-port](./modules/rust-mcp-full-port.aps.md) | RMCPF | In Progress | 2/9 (RMCPF-001 inventory and RMCPF-002 architecture spec Complete; Phase 1 readiness checklist still needs client/transport retirement decisions after the inventory landed) | RMCP, DRVR, `archive/anvil-mcp-server` (archived per ADR-033 — frozen reference) |
...
| [multilayer-protection-v2](./modules/multilayer-protection-v2.aps.md) | MLP2 | Draft | 0/56 (created 2026-05-14 from MLP-018 split-out; 11 groups A–K covering 56 integration items — see module file for full list) | All MLP v1 primitives; INTD enforcement pipeline; DRVR driver framework; RMCP/RMCPF MCP shim; RTAI mid-edit telemetry; LAUNCH activation orchestrator; kindling-integration. ADRs 036–039 already Accepted under MLP. |
