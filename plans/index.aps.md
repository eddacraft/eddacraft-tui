...
| [fleet-telemetry](./modules/fleet-telemetry.aps.md) | FLEET | Draft | 0/0 | Consent-posture ADR (design gate — nothing ships before it is Accepted); LAUNCH-013 `InstallMethod` detection; CIB-197 (local envelope enrichment — the beacon reuses those fields); `apps/anvil-api` (plausible ingest host). Filed 2026-07-14: opt-in/controlled **phone-home** fleet visibility — binary version, install method, and the FLAGS-design feature-usage dimensions (session-start snapshot + one stat per feature used; no PII, enumerated low-risk allowlist). A deliberate posture change vs the USAGE local-only privacy contract; distinct from EXPORT (tracing-pipe sink). OQ1 consent posture · OQ2 identity (salted hash vs none) · OQ3 Draft→Ready trigger (EXPORT-style paying-customer gate vs beta distribution). |
