...
| [release-orchestration](./modules/release-orchestration.aps.md)                 | RELORCH | Proposed    | 0/11 (Phase 1 nucleus: -001 spec, -002 harness, -003 assess, -004 preflight, -010 closeout, -011 wire-up + decommission. Phase 2: -005..-009 prepare/promote/tag/monitor/verify. Successor to archived RELMGMT; supersedes parts of `2026-04-20-relmgmt-agent-driven-release-design.md` while inheriting its no-persistent-manifest tradeoff as a hard constraint. Tracks the work to honour PR #1368's `scripts/release/*.sh` target surface; design must precede docs — see Council session `plan-9a6b3a94` 2026-05-09.) |
...
| [operating-model-migration](./modules/operating-model-migration.aps.md)                               | OPMODEL    | 1/12       | Cross-cutting migration to the target Plan / Build / Release operating model — **In Progress**. OPMODEL-001 completed the current-state vs target-state migration map; remaining work covers APS lifecycle vocabulary, `main`-first branching, release records, deterministic agent guidance, review/council entrypoints, CI readiness, drift checks, and rollback playbooks. Implementation remains owned by specialist modules such as RELORCH and DOCGOV. |
