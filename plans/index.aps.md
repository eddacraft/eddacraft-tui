...
| [activation-tui](./modules/activation-tui.aps.md)                     | ACTTUI   | In Progress | 0/9 | TUI-first `anvil start` on `eddacraft-tui` widgets (Tree, ParallelProgress, Select, Confirm, OverlayStack, Toast, BigBanner, LogPanel). Created 2026-07-08 via planning-workflow + planning-council direction-validate. **Single release cohort with welcome** — start + welcome both ship together; phase B runs ACTTUI-003..005 ∥ ACTTUI-008 ∥ WOW-001..004; phase C gates TTY-default on both surfaces. Council `proceed` with `amend`: ACTTUI-000 completed ADR-103 + plain fixture spec; MCP unticked consent; `ANVIL_HOME` banner; orchestrator event contract; 007 before default flip. Complements ACTMO, WOW, CIB-162..179; PR #3231 pins WOW-005/006 downstream gates on ACTTUI foundation/consent without changing the phase-B release cohort. |
