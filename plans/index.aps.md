...
| [dev-environment-hardening](./modules/dev-environment-hardening.aps.md)         | DEVENV | In Progress | 5/8 (ADR-057, council `plan-6b3be127`. DEVENV-001 Merged 2026-05-29 via PR #2090 — `[profile.dev]`/`[profile.test]` `debug = "line-tables-only"`, ~27% smaller debug targets. DEVENV-002 Merged 2026-05-29 via PR #2094 — per-worktree `CARGO_TARGET_DIR` relocation onto /home via committed `.envrc` + `wt` hook (the committed `.cargo/config.toml` floor was dropped — cargo config can't expand `$HOME`; env-only + a loud guard, CI-inert so the nx cache is unaffected and the -002→-003 dependency dissolved). DEVENV-003 Blocked 2026-05-29 — the active nx-rust plugin is `@eddacraft/nxrust` from the registry (not anvil); reloc-aware build outputs must land in its in-flight CACHE work (eddacraft/nxrust PRs #15/#16) then anvil bumps + verifies; correctness is already mitigated upstream (`CARGO_TARGET_DIR` is a cache-key input), so the residual is benign cache-reuse only. anvil's dead `tools/nx-rust` vendored copy removed in the same change. DEVENV-004 Merged 2026-05-29 via PR #2101 — `scripts/cache/anvil-target-evict.sh` (fail-closed prefix guard, `.cargo-lock` flock + mtime skips, dry-run default) + systemd timer templates + runbook; uses cargo's own lock, no plugin dep. DEVENV-005 Merged 2026-05-29 via PR #2104 — standardise on Node 24 (engines `>=24`, setup-workspace + nightly → 24; box keeps 26 globally via `fnm` honouring `.nvmrc`=24), nx cache-key `node --version` runtime input (closes the ADV-6 cross-Node poisoning gap), and `.envrc` pinned-bin PATH to beat the stale global oxfmt (CIB-032). DEVENV-006 Merged 2026-05-29 via PR #2113 — `.config/wt.toml` worktree bootstrap: branch off fresh `origin/main` via new `scripts/dev/wt-new.sh` (`wt` has no pre-create hook), the `install` post-start removes `.modules.yaml` to force a one-pass workspace-symlink relink (the real fresh-worktree `typecheck` breaker was the missing per-consumer `workspace:*` symlink, NOT missing dist — `copy-ignored` carries dist), and the `rust` post-start stops swallowing build errors. Remaining Wave 1 DEVENV-007: change-scoped parity into the `wt` pre-commit + classifier; Wave 2 DEVENV-008 is a spike that exits with a go/no-go ADR on a reproducible base (mise/devcontainer/Nix) + nx-cache/sccache dedup. The `origin/dev`→`origin/main` base-ref precursor shipped via PR #2086.) |
...
| [daemon-save-time-validation](./modules/daemon-save-time-validation.aps.md)     | DSV    | In Progress | 15/18 (filed 2026-06-03 — promotes the daemon save-time validation effort from the lone [MLP2-067](./modules/multilayer-protection-v2.aps.md) interim item + an execution plan into a tracked module. **Sub-phase A (interim-cache `validate_paths`, 9 items) is 9/9 Merged — Done, awaiting release**: DSV-001/002 (#2254/#2252), DSV-003 (#2264), DSV-004 (#2273), DSV-005 (#2282, `validate_paths` wired into ipc dispatch), DSV-006 (#2283), DSV-007 (#2284), DSV-008 (#2275), DSV-009 (#2294, cross-path antipattern parity gate + shared diagnostic sort). Sub-phase A-W (Windows + cross-platform parity, 2 — DSV-010/011): macOS already works via `cfg(unix)`; Windows was the gap. **DSV-010 Merged 2026-06-05 via PR #2328** (hardened #2340) — the save-time verbs are served over the per-user named pipe behind the ADR-068/070 read-safety guard, peer-SID auth, and owner-only config trust. **DSV-011 Merged 2026-06-05 via PRs #2317 + #2327** (the Windows `watch`/`status` clients; Sub-phase A-W closed 2/2 on 2026-06-07 — full cross matrix green on dispatch run 27102943706 after PR #2365 fixed the unrelated bench-test flake that had the Windows leg red). Sub-phase A deferred follow-ups (5 items, DSV-040..044, filed 2026-06-04 from the DSV-005/006/007 deferred notes): DSV-040 (unregister-hook warm-state reclamation) / DSV-041 (operator antipattern-config) / DSV-042 (interactive-pool parse offload) / DSV-043 (`validation.roundtrip` transport bench) Merged 2026-06-04 via PR #2296, DSV-044 (assurance-transition emission through the fanout) Blocked on MLP2-071 Phase 2 (the `Fanout::route` subscriber surface + broadcaster, deferred at #1722). Sub-phase A′ (GV2 hot-read swap, 1) — boundary gate **closed by [ADR-063](./decisions/063-gv2-hot-path-boundary.md)** (Accepted 2026-06-01); now blocked only on implementing the GV2 hot-read slice (GV2-010/011/020/022, still Draft); Sub-phase B (warm-start persistence, 1) — GV2-021 persistence ADR now **Accepted** ([ADR-069](./decisions/069-graph-v2-persistence.md), 2026-06-04); no longer decision-blocked, now blocked only on implementing warm-start persistence. Action plan: `execution/2026-06-01-daemon-save-time-subphase-a.md` Tasks 0–17. Count hand-curated — the module header carries no `N/M` cell, so `index-counts.mjs` leaves it unmanaged.) |
...
| Module                                                                      | Scope | Status      | Progress           |
| --------------------------------------------------------------------------- | ----- | ----------- | ------------------ |
| [continuous-improvement-backlog](./modules/continuous-improvement-backlog.aps.md) | CIB   | In Progress | 29/47 (CIB-013 done 2026-05-24 — dev-workflow now requires compact continuous-improvement session notes; CIB-011 PR #1818 `acc4db6f`, CIB-012 PR #1813 `ce0bd32b`; CIB-014/-015/-016 added 2026-05-24 from the Drako borrow assessment — SARIF export, `anvil bom` triage, baseline-posture phrasing; CIB-017/-018/-019 added 2026-05-25 from the POLENG full council — eval tracing, facade `catch_unwind`, parity-gate OPA stderr; CIB-020 merged 2026-05-25 via PR #1961 — version-agnostic TUI shell watermark in snapshots, surfaced by PR #1959; CIB-021 merged 2026-05-26 via PR #1967 — `merge=union` for the append-only CI log; CIB-022 merged 2026-05-26 via PR #1969 — APS index counts now derived from work-item statuses + CI-enforced; CIB-008/009/010 reconciled 2026-05-26 — 2026-05-21 new-user-journey audit fixes (PRs #1817/#1814/#1816) had merged but were left Draft; CIB-014 promoted 2026-05-29 to the SARIFOUT module (`sarif-output.aps.md`) after the SARIF output design pass; CIB-026/-027/-028/-029 added 2026-05-27 from CI-log review — cwd test isolation, cross-repo review fallback, safe worktree cleanup sweep, exact-semver docs; CIB-031 added 2026-05-28 from SCAN-005 PR #2034 — scope dependency-audit so Rust-only lockfile changes skip the npm-facing Trivy + license-check gates; CIB-032 added 2026-05-28 — fresh worktrees fall back to a stale global oxfmt, producing false format failures; CIB-033 added 2026-05-28 — sweep for open GitHub issues already resolved on main; CIB-034 added 2026-05-28 from public-release-mirror evidence discussion — sanitised release evidence for public mirror users; CIB-035 added 2026-05-29 from the clawpatch periodic scan — `drift-check.mjs` must stay advisory (exit 0) on invalid/unreadable release records instead of crashing; CIB-002 reconciled 2026-05-29 — Merged via PR #1453 (`7c59b2ee`); the agent-surface inventory shipped 2026-05-12 but was left In Progress; CIB-036/-037/-038 added 2026-05-29 from the CIB-002 reconcile + #2084 stuck-merge investigation — (036 **Done**) corpus migrated to canonical `aps lint` — 89/102 active files originally failed (85 missing `## Work Items`); resolved via PR #2095 (mechanical bulk) + cleanup, with anvil-plan-spec #56 exempting terminal items from E005, and `aps:active-lint` now a CI gate (`clawpatch-pre-tag` carved out as a non-module findings tracker); (037 **Done** — filed in #2084, fixed in #2089) `aps lint` only honoured the last of multiple file args so `aps:active-lint` silently checked one module — fixed by per-file invocation in `active-lint.mjs`, which un-masked 036's true scope; (038) CI skip-filler duplicate check names block the `main` ruleset merge on docs-path PRs despite a green rollup; CIB-039 added 2026-05-29 (Blocked) — archive `clawpatch-pre-tag-v0.7.0-beta` and drop its `active-lint` carve-out once its findings close out (per CIB-036); CIB-040 added 2026-05-29 (Ready) — full CLIC-010 help-text layout pass, the deferred thorough follow-up to the light identifier-stripping pass + the cli-surface runbook; CIB-041 merged 2026-06-02 via PR #2221 from the install.eddacraft.ai investigation — allowlist CLI `v*` tags in `release.yml`'s `plan` job so `eddacraft-tui-v*` (and future prefixed crate tags) don't run the CLI dist pipeline and publish a binary-less release on public `eddacraft/anvil` that steals the "Latest" pointer and 404s the installer URL; CIB-042/-043/-044 added 2026-06-02 (Ready) as the #2221 Council-deferred release-reliability follow-ups — installer-URL synthetic health check, `--latest=false` on the tui anvil-001 release, and skip `release-sign-artefacts.yml` for non-CLI library releases; CIB-045 done 2026-05-31 — repo-local Codex config + `dev-workflow` skill added for APS/Worktrunk/Council-aligned Codex execution; CIB-046 added 2026-06-03 (Ready) — gate `anvil plan dashboard` behind a new `tui-dashboard.aps-dashboard` flag scoped to a new `staff-internal-developer` audience; the surface ships unflagged + unauthenticated + User-explicit purely as a leftover local auth-down dogfooding workaround, with an `ANVIL_ADMIN_KEY`/flag escape hatch so it still works while `anvil auth` is down); CIB-038 implemented (consolidated the four CI twin *-skip filler jobs into internal quick-skip steps inside the primary jobs so each required check name has exactly one conclusion per PR run; eliminates ruleset BLOCKED on green docs/plans PRs; updated contract test + README; In Progress in module) |
...
| [git-native-exceptions](./modules/git-native-exceptions.aps.md)                   | EXCEPT  | In Progress | ADR-073 (Accepted 2026-06-08, full council), crates/anvil-policy — move exceptions from gitignored `.anvil/exceptions.json` to tracked `anvil/exceptions/` so they travel with the repo + are PR-reviewable; sibling of `@anvil-ignore` (ADR-004). EXCEPT-001/002 Done; EXCEPT-007 write-path hardening gates the CLI write surface; schema/CLI/L3-L4/capsule integration Proposed |
...
| [documentation-governance](./archive/modules/documentation-governance.aps.md)                                 | DOCGOV     | 12/12      | APS-linked operational knowledge architecture and agent closeout governance — **Complete** (DOCGOV-001 establishes the docs-workflow and closeout rules; DOCGOV-002 adds the documentation taxonomy and metadata convention; DOCGOV-003 aligns APS public docs, local rules, and the package schema/parser contract; DOCGOV-004 repairs ADR integrity — renames the duplicate ADR-026 to ADR-021, indexes all 42 ADR files in DECISION-LOG, and adds `pnpm adr:check` / `pnpm test:adr-integrity` to lock the invariants; DOCGOV-005 ships `pnpm docs:check`; DOCGOV-006 replaces the `asbuilt-paths` stub with a real governed as-built/runbook source-reference validator, adds runbook/as-built freshness templates, and closes absorbed DOCSYNC-015/-017/-018/-019/-020; DOCGOV-007 ships `pnpm docs:index` / `pnpm docs:index:check`, generated indexes under `docs/indexes/`, and the real `index-freshness` surface; DOCGOV-008 is **Complete** — release-runbook relocated to `docs/runbooks/` via Option A, 19 dead-doc archive moves landed, pitch-deck content relocated out-of-repo to `~/Projects/anvil-gtm-wip/`, entrypoints relinked through `docs/indexes/`, baseline `metadata` finding-keys shrank 179 → 140, and fresh closeout validation passed 2026-05-24 with `pnpm docs:check && pnpm docs:index:check && pnpm format:check && pnpm lint:check && pnpm test:docs-check`; closeout evidence at `plans/execution/DOCGOV-008.audit.md`. DOCGOV-009 is **Merged** as of 2026-05-25 via PR [#1927](https://github.com/eddacraft/anvil-001/pull/1927) at `02fc92f5`: the owner/freshness rubric is approved and the first high-authority live-doc metadata backfill batch has landed. DOCGOV-011 is **Merged** — public docs (`docs/public/**`, the Docusaurus docs-site source) were excluded from the internal metadata surface by operator decision, and the remaining 24 internal docs (13 as-built, 11 runbooks) were backfilled with the DOCGOV-002 metadata table, emptying the `metadata` baseline bucket (93 → 0); as-built/runbook body source-path debt revealed by the backfill was absorbed into the `asbuilt-paths` baseline per ADR-003 for a later burn-down. DOCGOV-012 is **Merged** as of 2026-05-30 via PR [#2147](https://github.com/eddacraft/anvil-001/pull/2147) at `fde8c256a` — the three `scripts/docs` robustness/contract fixes (`--update-baseline` partial-failure overwrite, `--no-baseline` misrouting, `check-links` percent-decode crash) shipped with fixture coverage; status reconciled here. DOCGOV-010 is **Merged** — no bulk move was required because the live doc tree was already taxonomy-aligned through the incremental DOCGOV-006/-007/-008/-009/-011 migration (verified: 112/112 governed non-public docs carry metadata, every doc sits under a Type-aligned directory, only `docs/README.md` at the `docs/` root); the deliverable is a new **Canonical Folder Layout** section in `docs/guides/documentation-governance.md` codifying placement-by-Type with authority/owner/status discovery via the generated `docs/indexes/`. All twelve items are terminal, so the module is **Complete**) |
...
| [tui-reintegration](./modules/tui-reintegration.aps.md)                                                | TUIR       | 9/10       | Supersedes TUIMIRROR. Brings `eddacraft-tui` canonical source back into `crates/eddacraft-tui/`, mirrors out to `eddacraft/eddacraft-tui:main` (read-only), publishes to crates.io from the Anvil canonical source. Pins crate location, mirror role, sync direction + automation, crates.io publish source, versioning/changelog ownership, CI gate split, internal-vs-external consumption contract, and backport/mirror conflict policy — **In Progress** (ADR-047 Accepted 2026-05-22; TUIR-001 baseline `Done`; TUIR-002 import `Merged` 2026-05-23; TUIR-003 consumer switch `Merged` 2026-05-23; TUIR-006 CI gate split `Merged` 2026-05-23 — `docs/policies/eddacraft-tui-mirror.md` documents Anvil-authoritative gates; TUIR-007 mirror public surfaces `Merged` 2026-05-23 — CONTRIBUTING.md / SECURITY.md / MIRROR-README.md carry mirror-aware language, `crates/eddacraft-tui/.github/workflows/pr-redirect.yml` auto-closes drive-by source PRs on the mirror, policy doc's Backport/Conflict section ratifies D-TUIR-009; TUIR-004 mirror workflow `Merged` 2026-05-24 via PR #1894 — `.github/workflows/mirror-eddacraft-tui.yml` force-pushes `crates/eddacraft-tui/` subtree to `eddacraft/eddacraft-tui:main` with banner-swap + empty-tree guard + sanitised dispatch-reason in step summary, [#1897](https://github.com/eddacraft/anvil-001/issues/1897) tracks the deferred hardening follow-ups; TUIR-005 publish workflow + release runbook `Merged` 2026-05-25 — `.github/workflows/publish-eddacraft-tui.yml` triggers on `eddacraft-tui-v*` tags, runs the D-TUIR-007 publish-side gate matrix, `cargo publish`es using `CRATES_IO_EDDACRAFT_TUI_TOKEN`, propagates the tag (append-only) via the `eddacraft-mirror-bot` GitHub App, then `gh release create --prerelease` on anvil-001 (ratified by TUIR-009 / D-TUIR-021: crate sub-product releases must not pin as the anvil-001 `latest` and shadow Anvil product releases); both mirror auth paths (TUIR-004 + TUIR-005 tag push) now use the same org-owned App so the legacy `EDDACRAFT_TUI_MIRROR_PUSH_TOKEN` PAT can be retired; TUIR-008 execution-token stays `open` pending the operator-driven E2E cut of `eddacraft-tui-v0.2.3`; **TUIR-009 mirror-side Release backfill + anvil-001 `--prerelease` `Merged` 2026-06-07 via PR #2339** — surfaced while manually backfilling `eddacraft-tui-v0.2.4` on the mirror, ratifies D-TUIR-021 that the mirror Release is an operator backfill and the anvil-001 release is `prerelease`-tagged so it doesn't shadow Anvil product releases, runbook gains a "Mirror Release backfill" subsection under Rollback plus a `gh api …/releases/latest` + `target_commitish` + `prerelease` + CHANGELOG/README byte-diff verify step (offset derived from `wc -l < MIRROR-README.md` per Copilot review); the existing v0.2.3 (orphan — never published to crates.io) and v0.2.4 anvil-001 releases were re-flagged `prerelease` and their `target_commitish` re-pinned as part of this work item; post-merge file: [plans/reviews/post-merge/chore-tuir-009-merge-closeout.md](./reviews/post-merge/chore-tuir-009-merge-closeout.md); **TUIR-010 deploy `mirror-drift-check.yml` `Merged 2026-06-08 via PR #2392`** — implements D-TUIR-018: a daily + dispatch watchdog that reconstructs the would-be-pushed mirror tree (subtree split + banner swap, replicated inline) and tree-diffs it against `eddacraft/eddacraft-tui:main`, opening/refreshing a `mirror-drift`-labelled issue on any drift; verified locally against the live mirror (clean = byte-identical tree; simulated out-of-band push = flagged); unblocks TUIR-008's drift-check validation line and TUIN's 7-green-runs gate) |
| [tui-next](./modules/tui-next.aps.md)                                                                  | TUIN       | 3/12       | Post-migration design work deliberately deferred out of TUIR scope (CLI/fallback timing, CLI independence, clap policy, terminal lifecycle ownership). Resolves argument-parser policy in core, mode-detection helper ownership, lifecycle feature-flag default, demo/`[[bin]]` shape, extension-surface stability markers, and a post-migration API stability checkpoint — **Proposed** (gated on TUIR-008 closure and drift-check green for 7 runs). **TUIN-001 docs-only `Merged` 2026-05-23** — [ADR-050](./decisions/050-eddacraft-tui-runner-and-cli-policy.md) locks the opt-in `runner` feature flag. **TUIN-011 docs-only `Done` 2026-06-08** amends ADR-050 for two imminent downstream consumers that need CLI tools: the runner is now a small fallback CLI shell (`launch_cli(cli)`) with lifecycle + minimal `lexopt` parser, global flags (`--help`, `--version`, `--theme`, `--no-tui`, `--config`), first-level subcommand dispatch, config handoff, and consumer-owned command semantics; single-app `launch_default(app)` may remain as a narrow adapter. **TUIN-012 added (`Proposed`)** as the future implementation authority for the feature-gated runner shell once TUIN opens. D-TUIN-001 reframed (runner shell is centrepiece, parser is subset); D-TUIN-003 amended (`runner` transitively enables `lifecycle`); TUIN-002 survey scope extended to capture per-consumer command, config, and `launch_cli` / `launch_default` expectations. Implementation tasks (TUIN-003 onward) remain inert until TUIR-008 closes. **TUIN-009 opportunistic colour-harmonisation spike `Done` 2026-05-28** (operator-authorised under the gate, no PR — ports tuie's CIELAB 256-colour palette generation onto `crossterm` with a three-tier degrade; 11 tests green, live query returns 18/18 in tmux via DCS passthrough + `allow-passthrough on`; decision: **SHIP**, but the implementation itself stays gated on TUIR-008). **TUIN-010 added (`Proposed`)** — triage of the remaining tuie port candidates (chord ergonomics = port the idea onto crossterm; image stack + layout/list/async/GUI/editor = skip). |
...
| [acknowledgements-starter-releases](./modules/acknowledgements-starter-releases.aps.md) | ATTRIB | 0/1 | **Proposed** 2026-06-08 — adds a deliberate semver-tag + GitHub-Release surface on the `eddacraft/acknowledgements-starter` mirror (starting v1.0.0) so consumers get notified, read a changelog, and pin to an immutable version — layered on top of the unchanged rolling-`main` mirror (ATTRIB-011). Retains the ATTRIB lineage (ATTRIB-017) rather than re-opening archived attribution-pipeline-v3; modelled on the `eddacraft-tui` release flow. Spec at [`plans/specs/2026-06-08-acknowledgements-starter-releases.md`](./specs/2026-06-08-acknowledgements-starter-releases.md); steps at [`plans/execution/ATTRIB-017.steps.md`](./execution/ATTRIB-017.steps.md). |
