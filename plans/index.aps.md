...
`LANGTS` (complete 6/6), `RSTLAN` (In Progress 3/8), `SURFSQL`, `PACKPUL`, and `PACKLLM`, with the
...
| [lang-rust](./modules/lang-rust.aps.md)         | RSTLAN | In Progress | 1     | §8.1 — RSTLAN-001/-002 Merged via PR #2303, RSTLAN-004 (Rust entry-point detection) Merged via PR #2319 (3/8); RSTLAN-005 (Rust layer/boundary enforcement — `crate::`/`super::`/`self::` resolver + gate `.rs` enablement) In Progress. RSTLAN-003 (AST antipattern catalogue) design-resolved by ADR-069 (Accepted — new `anvil-checks-ast` crate, gate-time only); impl pending. NBI re-eval complete 2026-06-03; ADR-065 (Rust-native) Accepted; re-score snapshot recorded (Rust #2 for dogfood); LANGTS/kernel gates satisfied; module promoted Proposed → Ready with 8 executable work items (grammar, extraction, T2 catalogue, entry, boundary per ADR, drift, validate surface, dogfood). Owner @aneki. (3/8) |
