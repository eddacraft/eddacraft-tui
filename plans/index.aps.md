...
| [graph-base-persistence](./modules/graph-base-persistence.aps.md) | GBASE | In Progress | 0/11 (created 2026-07-11 from planning council `plan-89a47ac7`, synthesised as [ADR-105](./decisions/105-shared-base-graph-persistence.md) — the ADR-069 storage-layout successor. Replaces the per-`WorktreeKey` snapshot with **one write-once, content-addressed base per repo per merge-base commit + live per-worktree overlays**: base read from the merge-base commit's committed tree via git objects (zero new deps) in a CLI subprocess, never the resident daemon; `SnapshotPayload` DTO + magic `ANVILGB1` reuse; disjoint base/overlay ids + a COMBINED-STATE golden parity fixture (GBASE-007, top schedule risk); `O_EXCL` single-flight production + refcount GC over ACTMO-registered worktrees (reclaims the new shared-base orphan class; per-worktree orphan race already closed by CIB-096; amends ADR-069 §5/§10); directory-level ref-watch triggers; re-entrant `persistence_route`; ADR-090 failure envelopes; the successor-specific graduation gate flips `ANVIL_PERSIST_GRAPH` default-on last (GBASE-010, terminal). Entry gate = the no-behaviour-diff `snapshot_io::store` extraction PR before GBASE-002; the old per-worktree path stays permanently for uncovered topologies. All 11 items Proposed.) | GV2, anvil-graph-cache, anvil-intercept, ADR-105, ADR-069, ADR-085, ADR-090, ADR-094, ADR-061/063/064/067 |
...
| [first-run-wow](./modules/first-run-wow.aps.md)                         | WOW      | In Progress | 4/6 | Tutorial and welcome first-run uplift, created 2026-07-08 via planning-workflow from the operator's first-run-experience review. Makes tutorial command execution evident-by-affordance (command bar + run/read-only badges + step-aware help, WOW-001; deterministic typed-command reveal, WOW-002) and personalizes the first run from the discovery scan (per-domain picker counts WOW-003; completion findings delta WOW-004). WOW-005 (first-win reroute after discovery) and WOW-006 (autoplay demo on a sandboxed fixture) are Draft behind design gates (first-run fix-write consent posture; sandbox lifecycle). Complements — not duplicates — the CIB-162..179 welcome/start repair items; LAUNCH-014 honesty pins unchanged. |
| [activation-tui](./modules/activation-tui.aps.md)                     | ACTTUI   | In Progress | 2/13 | TUI-first `anvil start` on `eddacraft-tui` widgets (Tree, ParallelProgress, Select, Confirm, OverlayStack, Toast, BigBanner, LogPanel). Created 2026-07-08 via planning-workflow + planning-council direction-validate. **Single release cohort with welcome** — start + welcome both ship together; phase B runs ACTTUI-003..005 ∥ ACTTUI-008 ∥ WOW-001..004; phase C gates TTY-default on both surfaces. Post-ACTTUI first-run review [`2026-07-09-acttui-first-run-journeys.md`](./reviews/2026-07-09-acttui-first-run-journeys.md) blocks the default flip until ACTTUI-009 (end-to-end consent wiring), ACTTUI-010 (contract/PTY matrix), ACTTUI-011 (typed verdict/evidence model), and ACTTUI-012 (polish/default-flip cleanup) are resolved. Complements ACTMO, WOW, CIB-162..184; PR #3231 pins WOW-005/006 downstream gates on ACTTUI foundation/consent without changing the phase-B release cohort. |
