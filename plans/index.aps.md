...
| [lang-python](./modules/lang-python.aps.md)     | PYLAN  | In Progress | 2     | §8.1 — promoted Draft → In Progress 2026-06-17 on operator direction ("build lang-python first") to unblock GCALL-005. **PYLAN-001** (tree-sitter-python grammar wired: `Language::Python`, `.py`/`.pyi`) + **PYLAN-002** (Python symbol/import extractor — `def`/`class`/method + `import`/`from`-import incl. relative/star) In Progress in the foundational PR; prerequisites LANGTS + RSTLAN both Complete; PYLAN-003..009 anticipated |
