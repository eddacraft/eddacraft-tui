...
| ---- | --- | ---- | ------ | ------- | ----------- |
| 1 | GCTX-011 `anvil_find_dependents` — dependency traversal | Ready | [graph-context-delivery](./modules/graph-context-delivery.aps.md) + [graph-v2-foundation](./modules/graph-v2-foundation.aps.md) | The next GCTX Phase-1 tool on the now-shipped GCTX-010 spine. Promoted to Ready 2026-06-17 (planning-workflow): all deps closed — GCTX-010 (#2657), GV2-011 (Released), GV2-026 depth lever (#2594). Identity-only, file-keyed dependents via `dependents_of`/`reverse_impact`, depth-bounded, reusing the sealed-DTO + `GctxProjector` + `GctxDispatch` spine and the CE gates. Caller traversal (symbol call graph) split to GCTX-014 (Blocked — no call edges in the warm graph). | Pick up **GCTX-011** through dev-workflow (TDD): add the `anvil/gctx/find_dependents` RPC + projector path + identity-only DTO + the `anvil_find_dependents` MCP tool. |
...
| Module                                                                      | Scope | Status      | Progress           |
| --------------------------------------------------------------------------- | ----- | ----------- | ------------------ |
| [continuous-improvement-backlog](./modules/continuous-improvement-backlog.aps.md) | CIB   | In Progress | 45/77 (standing continuous-improvement intake; recent CIB-041..-046 release-reliability + flag-gating items Merged/Ready; CIB-054/-055 pre-tag v0.8.0 changelog-wording + release-plan-table reconcile Merged 2026-06-10 via PRs #2493/#2496; CIB-056 driver-client Windows pipe SID gate Merged via PR #2485 (#2484 increment 2 remains); CIB-057 capsule as-built doc + CIB-058 v0.8.0-window as-built delta refresh Merged 2026-06-10; CIB-059/-060 beta user-journey walkthrough follow-ups (quickstart ADR-080 ordering + auth-wall access pointer) In Progress 2026-06-11; CIB-061 remaining as-built delta refresh Merged via PR #2526; CIB-065 schema $id domain drift Merged 2026-06-11 via PR #2539; CIB-062..-064 golden-path polish gaps (auth-gate WARN JSON on stderr + install.sh docs URL + double-reported planted secret) Merged 2026-06-11 via PRs #2529/#2528/#2530; CIB-071 migrate user-facing diagnostics to miette Merged 2026-06-14 via PRs #2597/#2611; CIB-072 clear stuck `ready_restart_required` on Windows when daemon attestation is Unreachable (Proposed 2026-06-14, issue #2609); CIB-073..077 demo-readiness/release-flow gaps Draft 2026-06-14; per-item status in the module file) |
