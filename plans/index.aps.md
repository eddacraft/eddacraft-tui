...
| ---- | --- | ---- | ------ | ------- | ----------- |
| 1 | v0.9.3-beta cut — validate P0 claims on Windows | Ready | [RELEASE-PLAN](../RELEASE-PLAN.md), [CIB](./modules/continuous-improvement-backlog.aps.md) | Implementation for the honesty + Windows window is on main (including CIB-228/229/250/252/254/100/160). Cut criteria still require **validation** (or explicit waive), not more feature work. | Run clean-PATH install + `update --check` (228/229); register empty + save-time secrets (252/254); tutorial safety chain (250); confirm ACKNOWLEDGEMENTS multi-notice (205); then preflight → prepare → tag. |
| 2 | CIB-288 — install banner save-time promise + lead with welcome | Ready | [CIB](./modules/continuous-improvement-backlog.aps.md) | Same honesty class as CIB-260 (merged) but on the post-install banner every new user sees first. Optional same-cut polish; not a hard cut gate. | Rewrite `install.sh` Get-started block: drop unattached save-time claim; prefer ungated `anvil welcome` ahead of licence-gated `anvil start`. |
| 3 | CIB-281 — audit security scope on TUI + SARIF | Ready | [CIB](./modules/continuous-improvement-backlog.aps.md) | Completes CIB-234 disclosure on the default surfaces. P2 presentation; optional same-cut if capacity after cut validation. | Surface SECURITY_SCOPE (or equivalent) on TUI/SARIF audit paths. |
| 4 | CIB-220..230 + pack trust (250/252/254) + start honesty | Merged | [CIB](./modules/continuous-improvement-backlog.aps.md), [RELEASE-PLAN](../RELEASE-PLAN.md) | Primary/secondary window claims are on main; evidence remains the open work. | None before release verification. |
| 5 | CGBDG-001..006 — council-gate bridge discovery | Ready | [council-gate-bridge](./modules/council-gate-bridge.aps.md), [Graph Trust Surfaces](./specs/2026-07-28-graph-trust-surfaces.md) | Highest-value programme track after the 0.9.3 cut; not a 0.9.3 claim. | After 0.9.3 or in parallel if capacity; prefer thin witness-lines path. |
...
window is **`v0.9.3-beta`** (honesty CIB-220..227 / #3510 + Dave CIB-228..243 / #3514 + pack-02 CIB-251..267; auth wall excluded), declared in
