...
(`POLRESET`, Proposed conductor). The reset combines the two policy-value lenses:
...
| [policy-value-enforcement-reset](./modules/policy-value-enforcement-reset.aps.md) | POLRESET | Proposed | **Conductor reset** for real policy value + opt-in save-time/pre-write enforcement. Coordinates POLVAL, OPAE, CPOL, IORISK, EXCEPT, CPACKS, EVALCI, ATC/PATT, ACTAX, OPAG, and enterprise policy modules; first gate is a policy admission/enforcement ADR reconciling ADR 002, ADR 015, ADR 037, and ADR 040. |
| [opa-enhancements](./modules/opa-enhancements.aps.md)                             | OPAE    | Draft    | Reset 2026-07-02 to **Policy Authoring and Runtime UX**: regorus-backed user policy discovery/loading, local policy install UX, remediation-first guidance, save-time/pre-write input adapter, and `warn`/`fence`/`interrupt` routing contracts. Depends on POLRESET, POLVAL, CPOL, EXCEPT; no second production OPA runtime. |
...
| [skill-packaging-distribution](./modules/skill-packaging-distribution.aps.md)     | SKPKG   | Draft    | ADR-018 (product/IP architecture — closed vs OSS-surface call), SKOBS-002 (manifest alignment) — design-only module; SKPKG-001 scopes packaging `anvil-developer-functions` for cross-agent customer distribution                                 |
