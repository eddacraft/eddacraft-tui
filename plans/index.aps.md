...
| [tui-reintegration](./modules/tui-reintegration.aps.md)                                                | TUIR       | 4/8        | Supersedes TUIMIRROR. Brings `eddacraft-tui` canonical source back into `crates/eddacraft-tui/`, mirrors out to `eddacraft/eddacraft-tui:main` (read-only), publishes to crates.io from the Anvil canonical source. Pins crate location, mirror role, sync direction + automation, crates.io publish source, versioning/changelog ownership, CI gate split, internal-vs-external consumption contract, and backport/mirror conflict policy — **In Progress** (ADR-047 Accepted 2026-05-22; TUIR-001 baseline `Done`; TUIR-002 import `Merged` 2026-05-23 — D-TUIR-019 records the clippy-pedantic delta; TUIR-003 consumer switch `Merged` 2026-05-23 — root workspace dep now `eddacraft-tui = { path = "crates/eddacraft-tui" }`; TUIR-006 CI gate split `Merged` 2026-05-23 — `docs/policies/eddacraft-tui-mirror.md` documents Anvil-authoritative gates + mirror-side smoke contract, dedicated `cargo test -p eddacraft-tui --all-features` step added to rust.yml so optional features (image, big-text, test-utils) are tested; TUIR-004/005/007/008 execution-tokens stay `open` pending per-task prerequisites) |
