...
| [release-orchestration](./modules/release-orchestration.aps.md)                 | RELORCH | In Progress | 1/11 (RELORCH-001 completed 2026-05-10 with `plans/specs/2026-05-10-release-orchestration-design.md`; RELORCH-002 in progress with initial `scripts/release/_test/` harness and `.github/workflows/release-harness.yml`. RELORCH-002 still needs the broader fixture matrix before completion. Phase 1 nucleus remaining: -002 harness, -003 assess, -004 preflight, -010 closeout, -011 wire-up + decommission. Phase 2: -005..-009 prepare/promote/tag/monitor/verify. Successor to archived RELMGMT; supersedes parts of `2026-04-20-relmgmt-agent-driven-release-design.md` while inheriting its no-persistent-manifest tradeoff as a hard constraint. Tracks the work to honour PR #1368's `scripts/release/*.sh` target surface; design now precedes implementation — see Council session `plan-9a6b3a94` 2026-05-09.) |
...
| [operating-model-migration](./modules/operating-model-migration.aps.md)                               | OPMODEL    | 8/12       | Cross-cutting migration to the target Plan / Build / Release operating model — **In Progress**. OPMODEL-001 completed the current-state vs target-state migration map; OPMODEL-002 updated branching/worktree guidance; OPMODEL-003 aligned APS lifecycle and release metadata rules; OPMODEL-004 defined the release-record schema and authority contract; OPMODEL-005 defined release-readiness and candidate artefact workflow design; OPMODEL-006 aligned release runbook/skill migration boundaries; OPMODEL-007 added deterministic advisory agent guidance; OPMODEL-008 aligned review/council entrypoints and planning-council playbooks. Remaining work covers workflow session/event schema, drift checks, rollback playbooks, and main-first cutover. Implementation remains owned by specialist modules such as RELORCH and DOCGOV. |
| [ci-cd-validation](./modules/ci-cd-validation.aps.md)                                                 | CICD       | 3/12       | Specialist CI/CD and validation operating model implementation — **In Progress**. Coordinates with OPMODEL rather than replacing it; CICD-001 added CI cost/run-reason reporting via `pnpm ci:cost`; CICD-002 added the shared path/risk classifier via `pnpm ci:classify`; CICD-003 added local validation commands via `pnpm validate:*`. Remaining work covers trigger targeting, workflow decomposition, candidate-readiness CI touchpoints, and migration-safe validation cutover. |
