...
| [lang-rust](./modules/lang-rust.aps.md)         | RSTLAN | In Progress | 1     | §8.1 — RSTLAN-001 (grammar wiring) + RSTLAN-002 (Rust extraction) Merged 2026-06-04 via PR #2303 (2/8); RSTLAN-003 scoping found the regex scanner needs cfg(test)/adjacent-line/AST context — owner steer pending on split. NBI re-eval complete 2026-06-03; ADR-065 (Rust-native) Accepted; re-score snapshot recorded (Rust #2 for dogfood); LANGTS/kernel gates satisfied; module promoted Proposed → Ready with 8 executable work items (grammar, extraction, T2 catalogue, entry, boundary per ADR, drift, validate surface, dogfood). Owner @aneki. (2/8) |
