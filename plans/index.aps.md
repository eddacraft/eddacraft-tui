...
| 3 | TUIDASH-002 — Component registry trait + infrastructure | Do now | [`tui-dashboard-render`](./modules/tui-dashboard-render.aps.md#tuidash-002-component-registry-trait-and-infrastructure) | TUIDASH-001 parser merged (PR #2068); the registry is the next Phase-1 step that turns parsed specs into renderable widgets. | Define `TuiComponent` + `TuiRegistry` on the merged `json_render` types. |
...
| [tui-dashboard-render](./modules/tui-dashboard-render.aps.md)             | TUIDASH | In Progress | 1/12                                                                                                              | eddacraft-tui (engine, feature-gated) + anvil-tui (catalogue/surface) per ADR-054; spec contract `@eddacraft/render` (`packages/libs/render/`); extends TDASH `anvil dashboard`. DASHAI parallel, not blocking                                                                      |
