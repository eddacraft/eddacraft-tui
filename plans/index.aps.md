...
| ---- | --- | ---- | ------ | ------- | ----------- |
| 1 | GCTX-010 `anvil_search_symbols` — Phase 1 pilot (CE-5 spine) | Ready | [graph-context-delivery](./modules/graph-context-delivery.aps.md) + [graph-v2-foundation](./modules/graph-v2-foundation.aps.md) | The assistant-facing substrate is complete: ADR-083 + PV-9 landed, GV2-023 #2621 + GV2-020 #2622 Merged, **GV2 20/20 (GV2-031 #2627)**, **GCTX Phase 0 complete** (GCTX-001 #2628 + GCTX-002 #2619), and the graph-handle design gate is **resolved — ADR-084 Accepted via #2632** (daemon-RPC + daemon-side projection, option A). **GCTX-010 is now Ready** with C1–C5 folded as acceptance criteria. | Pick up **GCTX-010** through dev-workflow (TDD): build the spine — `anvil-gctx-types` (sealed DTOs + structural no-leak test), `anvil-gctx-egress` (`GctxProjector`), the `anvil/gctx/search_symbols` RPC on its own `GctxDispatch` — then the identity-only tool. Sequence against DLIFE (daemon-required). |
