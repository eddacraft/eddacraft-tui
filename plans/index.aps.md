...
> remaining honesty — claim not frozen). Highest-value **programme** track is
> Graph Trust Surfaces Wave 0 (**CGBDG** discovery; **CONF-001** schedule).
> See [`RELEASE-PLAN.md`](../RELEASE-PLAN.md) and NBI.
...
| ---- | --- | ---- | ------ | ------- | ----------- |
| 1 | CGBDG-001..006 — council-gate bridge discovery | Ready | [council-gate-bridge](./modules/council-gate-bridge.aps.md), [Graph Trust Surfaces](./specs/2026-07-28-graph-trust-surfaces.md) | Highest-value programme track after `v0.9.3-beta`; Wave 0 of Graph Trust Surfaces. Not a release cut claim. | Execute discovery; prefer thin witness-lines path; CGBDG-006 report + follow-on implement/spec or park. |
...
| 4 | CIB-281 — audit security scope on TUI + SARIF | Ready | [CIB](./modules/continuous-improvement-backlog.aps.md) | Completes CIB-234 disclosure; Ready leftover from the 0.9.3 honesty class. | Surface SECURITY_SCOPE (or equivalent) on TUI/SARIF audit paths; candidate for provisional `v0.9.4-beta`. |
| 5 | Provisional v0.9.4-beta — field follow-up | Schedule | [RELEASE-PLAN](../RELEASE-PLAN.md) | Keeps one untagged active window after 0.9.3 closeout; claim not frozen. | Intake post-0.9.3 field signal; lock primary claim before implement/cut. |
| 6 | CIB-305 — concurrent CI-log tracked writers | Draft | [CIB](./modules/continuous-improvement-backlog.aps.md) | P0 internal data-loss correctness on bookkeeping path (not user-facing product). | Promote/shape then fix concurrent writers in `scripts/ci-log`. |
...
window is provisional **`v0.9.4-beta`** (field follow-up / remaining honesty;
claim not frozen), declared in
...
| [mcp-live-heal](./modules/mcp-live-heal.aps.md) | MCPLH | **Ready** | 0/7 (filed 2026-08-09. Design: [`specs/2026-08-09-mcp-live-heal-without-harness-restart.md`](./specs/2026-08-09-mcp-live-heal-without-harness-restart.md). Agent-ready MCP after upgrades **without** harness session restart: PATH-stable install, self-heal re-exec of `mcp serve`, `anvil mcp refresh` cascade, daemon skew recycle, status inventory + split ready claims, opt-in orphan reap. MCPLH-001..006 **Ready**; MCPLH-007 supervisor Draft; large-repo graph warm out of module. Extends CIB-242 visibility; no silent kill of live parents' children.) | MCPX, RMCP/RMCPF, DLIFE, CIB-242 posture; exclusive module |
