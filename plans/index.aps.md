...
processes. The active planning window is now the **`v0.7.3-beta`** candidate
(scope assembling on `main`).
...
| N7 — Distribution & Self-Update (DISTRIB) | In Progress | 6/6 | Harden the update/distribution loop so hotfix iteration actually reaches users. Signature verification + resolution-chain robustness (**-001 Merged via PR #1562**), **`anvil version --check` advisory surface (-002 Merged via PR #1569)**, **Homebrew formula automation (-003 Merged via PR #1652)**, release cadence + EOL policy doc (**-004 Done 2026-05-16**, `docs/policies/release-cadence.md`), `anvil migrate` (-005). **DISTRIB-006 Merged 2026-05-31 via PR #2185** (`ANVIL_HOME` / `--anvil-home` install-root override; merged after v0.7.3-beta tag, rides `v0.7.4-beta` as freight — module stays In Progress at 6/6 until that tag ships -006 Released/Shipped → Complete). Promoted **Proposed → Ready** 2026-05-14. ADR-044 §9 makes -001 and -002 load-bearing for the MCP-backend swap discovery gap. Lands in Wave 3A. |
...
| [distribution-and-update](./modules/distribution-and-update.aps.md)     | DISTRIB  | In Progress | 6/6      | Harden `anvil update` + Homebrew + cadence policy so hotfix iteration reaches users. **DISTRIB-001 Merged via PR #1562** (minisign verification + ADR-045). **DISTRIB-002 Merged via PR #1569** (`anvil version --check` advisory surface + watch/status hint). **DISTRIB-003 Merged via PR #1652** (Homebrew formula auto-bump extracted into tested script + workflow + runbook + macOS smoke matrix). **DISTRIB-004 Done 2026-05-16** (`docs/policies/release-cadence.md`). **DISTRIB-005 Merged 2026-05-26 via PR #1984** (`anvil migrate schema`). **DISTRIB-006 Merged 2026-05-31 via PR #2185** — `ANVIL_HOME` / `--anvil-home` install-root override for side-by-side candidate installs (test a pre-release without releasing it), ADR-060 gate Accepted 2026-05-31. Merged after the v0.7.3-beta tag commit, so it rides `v0.7.4-beta` as freight — module **stays In Progress at 6/6** (all Merged) until that tag ships -006 Released/Shipped → Complete. ADR-044 §9 makes DISTRIB-001 / -002 load-bearing for the MCP-backend swap discovery gap. Wave 3A. |
...
| [tui-dashboard-render](./modules/tui-dashboard-render.aps.md)             | TUIDASH | In Progress | 13/13 (TUIDASH-003..-012 engine/components/charts/binding/surface+CLI/parity/responsive/previews Merged 2026-06-02 via PR #2229; TUIDASH-013 ship example gate-summary spec + gate-result persistence Merged 2026-06-02 via PR #2246 — GH #2237/#2242; awaiting release) | eddacraft-tui (engine, feature-gated) + anvil-tui (catalogue/surface) per ADR-054; spec contract `@eddacraft/render` (`packages/libs/render/`); extends TDASH `anvil dashboard`. DASHAI parallel, not blocking                                                                      |
...
| [git-native-governance](./modules/git-native-governance.aps.md)                   | GITGOV  | Proposed | ADR-072/-073/-074 (Proposed); crates/anvil-witness (`WitnessLine`/`verify_chain_dag`), anvil-baseline, anvil-rules (`rules_sha`), anvil-policy (exceptions), anvil-cli SARIF (ADR-058) — Review Capsules wedge: file-first portable governance evidence, offline-verifiable. Decision-gated on ADR-072/-074 council acceptance before capsule impl (GITGOV-003+) |
| [git-native-exceptions](./modules/git-native-exceptions.aps.md)                   | EXCEPT  | In Progress | ADR-073 (Proposed), crates/anvil-policy — move exceptions from gitignored `.anvil/exceptions.json` to tracked `anvil/exceptions/` so they travel with the repo + are PR-reviewable; sibling of `@anvil-ignore` (ADR-004). EXCEPT-001/002 (path migration + read-fallback) operator-authorised 2026-06-06; schema/CLI/L3-L4/capsule integration Proposed |
