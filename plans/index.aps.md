...
| [tui-reintegration](./modules/tui-reintegration.aps.md)                                                | TUIR       | 3/8        | Supersedes TUIMIRROR. Brings `eddacraft-tui` canonical source back into `crates/eddacraft-tui/`, mirrors out to `eddacraft/eddacraft-tui:main` (read-only), publishes to crates.io from the Anvil canonical source. Pins crate location, mirror role, sync direction + automation, crates.io publish source, versioning/changelog ownership, CI gate split, internal-vs-external consumption contract, and backport/mirror conflict policy — **In Progress** (ADR-047 Accepted 2026-05-22; TUIR-001 baseline `completed`; TUIR-002 import `completed` 2026-05-23 — D-TUIR-019 records the clippy-pedantic delta from D-TUIR-007 × D-TUIR-016 collision; TUIR-003 consumer switch `completed` 2026-05-23 — root workspace dep now `path = "crates/eddacraft-tui"`, `cargo hakari generate` dropped the workspace-hack `eddacraft-tui` entry entirely (path crate needs no version aggregation), registry copy no longer in Cargo.lock; TUIR-004..TUIR-008 execution-tokens stay `open` pending per-task prerequisites) |
