...
| 5 | CIB-316 — retire the stale cutover-readiness guard | Ready | [CIB](./modules/continuous-improvement-backlog.aps.md) | The unwired test still demands retired `dev` triggers and is red on `main`. | Remove the retired test and its package script, then prove only historical references remain. |
