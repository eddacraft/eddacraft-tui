...
| ---- | --- | ---- | ------ | ------- | ----------- |
| 1 | GCTX-010 `anvil_search_symbols` — Phase 1 pilot (CE-5 spine) | Merged | [graph-context-delivery](./modules/graph-context-delivery.aps.md) + [graph-v2-foundation](./modules/graph-v2-foundation.aps.md) | The CE-5 hard-gate pilot — sealed egress DTO + `GctxProjector` + structural no-leak spine and the identity-only tool. **Merged 2026-06-16 via #2657**: spine + tool #2637, CE-6 cursors #2645, CE-10/CE-11 telemetry+kill-switch #2648, then the C1 cold-start warm-up triggers on top of the DSV-045 executor (#2674). The C4 `gctx.egress` manifest flag is Phase-2. | Cleanup agent advances Merged → Released/Shipped on the v0.9.0-beta tag. Next GCTX pick — GCTX-011 (caller/dependent traversal) — needs promotion from Draft on this spine. |
