...
| Daemon-working product slate | Current candidate | MLP 13/17 (In Progress; Done 2026-05-13: MLP-001 / -002 / -003 / -004 / -005 / -006 / -007 / -008 / -011 / -012 / -013 / -015 / -017 — covers Wave 1 backbone, Wave 2 primitives, and the first two Wave 3 surfaces MLP-004 + MLP-015), INTL 0/9 (Ready, ready-to-start-Wave-3), carry-forward gates 6/6 confirmed (Wave 0 closed 2026-05-13) | Next tag candidate `v0.7.0-beta`; MLP-009 remains the hard release gate for any full-protection claim. Wave 3 in flight per `RELEASE-PLAN.md` — remaining items: MLP-014 (multi-session + fence isolation), MLP-016 (L1 → Kindling), MLP-010 (GH Action publishing), MLP-009 (HARD GATE). |
...
| ---- | ------ | -------- | ----- |
| N1 — Multi-Layer Protection v1 (MLP) | In Progress | 13/17 | Witness chain + hooks + L4 policy + baseline + multi-agent coord + rule distribution. New crates: `anvil-witness`, `anvil-config`, `anvil-rules`, `anvil-baseline`, `anvil-hook`, `anvil-l4`, `anvil-attribution`. **Hard gate: MLP-009.** Promoted from Proposed during Wave 0 readiness review (2026-05-13). Wave 1 complete (MLP-001 / -002 / -011 / -013 / -017 Done 2026-05-13). Wave 2 complete (MLP-003 / -005 / -006 / -007 / -008 / -012 Done; Wave 2 library primitives merged via #1489 / #1490 / #1491 / #1492; CLI integration + `anvil hook pre-commit/post-commit/post-merge/post-rewrite/bootstrap` + `anvil baseline [verify\|--refresh]` merged via earlier PR; `anvil-witness` extended with `parent_commits[]` / `prev_line_hashes[]` for DAG-aware merge writes). Wave 3 in flight: MLP-004 pre-push hook merged via PR #1499 (`crates/anvil-hook/src/pre_push.rs` + `crates/anvil-l4/src/decide.rs` + `anvil hook pre-push` CLI; 89 + 35 + 25 tests green); MLP-015 L5 audit-chain merged via PR #1500 (`anvil audit-chain` + nightly workflow template; 10 tests green). Remaining Wave 3 items: MLP-014 (multi-session + per-task fence isolation; new `anvil-attribution` crate), MLP-016 (L1 editor driver → Kindling integration; coordinates with RTAI-007 telemetry), MLP-010 (GitHub Action publishing; external `eddacraft/anvil-action` repo), MLP-009 (Protection-claim contract test suite — HARD RELEASE GATE; deps MLP-002..-008 now satisfied). |
...
| --------------------------------------------------------------------------------- | ------- | -------- | --------------------------------------------------------------------------------------------------------------------------------------------------- |
| [policy-engine](./modules/policy-engine.aps.md)                                   | POLENG  | Ready    | ADR-040 (Accepted 2026-05-13), crates/anvil-kernel, crates/anvil-policy — substrate for OPAE/ORGHIER/POLLC/COMPLY/POLFED/CPACKS                     |
...
| [rust-mcp-full-port](./modules/rust-mcp-full-port.aps.md) | RMCPF | Draft | 0/9 (RMCPF-001 Ready; module remains Draft until inventory and client/transport retirement decisions satisfy the module Ready Checklist) | RMCP, DRVR, `archive/anvil-mcp-server` (archived per ADR-033 — frozen reference) |
