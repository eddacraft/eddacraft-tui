...
| [lang-tail-wave-2](./modules/lang-tail-wave-2.aps.md) | LTW2 | Ready | 2 | WebAssembly text (`.wat`/`.wast`) + Zig (`.zig`/`.zon`) at T1, batched per the LANGTAIL pattern. Owner-directed addition under [ADR-093](./decisions/093-tail-wave-2-wasm-text-and-zig-reentry.md) (**Accepted** 2026-06-29); module **Ready** 2026-06-29. Scope is **text only — binary `.wasm` is excluded**. Ready authorises **LTW2-001 (grammar audit) + LTW2-005 (doc fix)**; the wiring items LTW2-002/-003/-004 stay Proposed until LTW2-001 confirms each grammar binds tree-sitter 0.26 (ABI compat unverified; drop-not-stall per C-005). Zig **re-enters from the §13 cut list**. |
