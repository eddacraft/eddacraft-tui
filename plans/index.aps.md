...
| Release nominations + as-built sweep follow-ups | In Progress | V060F 1/11 | V060F-001 (RCLI2-009 admin command parity) complete; V060F-002..V060F-011 filed 2026-05-07 from the v0.6.0-beta as-built sweep — CLI gaps (stop/unblock/Windows MCP daemonStatus), macOS interrupt branch, activation hand-offs, doc alignment, kernel spec divergences |
...
| [v060-release-candidates](./modules/v060-release-candidates.aps.md)             | V060F  | In Progress | 1/11 (V060F-001 nomination complete via RCLI2-009 admin command parity; V060F-002..V060F-011 filed 2026-05-07 from the v0.6.0-beta as-built sweep — `intercept stop`/`unblock` CLI front-ends, macOS `current_process_start_time` branch, Windows MCP `daemonStatus` cfg(unix) gate, LAUNCH-016 `extensions:` user-config opt-in, watch-liveness probing, `rust-kernel-spec.md` status header, `quality-model.md` watch→checks framing, checks pipeline APS ownership, kernel spec/code divergences) |
