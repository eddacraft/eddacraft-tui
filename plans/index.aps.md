...
| ---- | --- | ---- | ------ | ------- | ----------- |
| 1 | GCTX Phase 1 design gate — sealed projection + graph handle | Schedule | [graph-context-delivery](./modules/graph-context-delivery.aps.md) + [graph-v2-foundation](./modules/graph-v2-foundation.aps.md) | The assistant-facing substrate is complete: ADR-083 + PV-9 landed, GV2-023 #2621 + GV2-020 #2622 are Merged, **GV2 is 20/20 with GV2-031 Merged via #2627**, and **GCTX Phase 0 is complete** (GCTX-001 #2628 + GCTX-002 #2619). GCTX module is In Progress 2/13. Phase 1 is the next front, but it is design-gated rather than pick-up-ready. | Shape the Phase 1 plan/ADR for GCTX-010 `anvil_search_symbols`: CE-5 sealed egress DTO, `GctxProjector`, structural no-leak test, and how `anvil mcp serve` obtains a graph handle (fresh construction vs daemon-RPC). |
...
| [graph-context-delivery](./modules/graph-context-delivery.aps.md) | GCTX | In Progress | 2/13 (Phase 0 — Delivery Contract — complete: **GCTX-001 projection contract Merged 2026-06-15 via #2628** (spec folds PV-9 CE-1..CE-12 onto the GV2-023 contract) and **GCTX-002 MCP delivery target Merged 2026-06-15 via #2619** (discharged by ADR-083 Accepted). Phase 1 tool items (GCTX-010..013/021..023/030) stay Draft — they build the CE-5 sealed egress DTO + `GctxProjector` + no-leak test in Rust (net-new) and need a design for how `anvil mcp serve` obtains a graph handle) | GV2 |
...
| [daemon-lifecycle](./modules/daemon-lifecycle.aps.md)                           | DLIFE  | Proposed    | 0/6 (created 2026-06-14 from operator direction that `anvil start` and `anvil watch` should make daemon-backed protection the normal user path, with explicit opt-out. ADR-082 is Proposed and would supersede ADR-079 before implementation work proceeds. DLIFE-006 added 2026-06-15 from #2609 triage — a terminating `--verify` diagnostic for the daemon-unreachable case, independent of ADR-082.) |
