...
| 5 | v0.9.4-beta claim set | Released/Shipped | [release record](./releases/v0.9.4-beta.md) | Install honesty + field follow-up published 2026-08-10. | None — historical. |
...
| [beta-account-activity](./modules/beta-account-activity.aps.md) | BACT | In Progress | 0/6 | Operator-accepted 2026-08-11 two-pipe design: FLEET remains “is anyone?” (ADR-107); BACT is authenticated **customer-success** activity for named beta accounts. BACT-001 boundary docs/decision log · 002 `first_login_at`/`last_login_at`/`last_login_method` on session mint · 003 admin show/list login surface · 004 allowlisted per-user feature-touch store · 005 authenticated emit · 006 CS filters + runbook (never logged in / idle / missing core feature). Explicitly **out of scope:** re-identifying fleet `install_id`, free-form argv, Resend webhooks (Draft BACT-010). Coordinates with FLEET, EMAIL `beta:active-*` cohorts, admin CLI, auth mint paths. Priority High. |
...
| [documentation-freshness](./modules/documentation-freshness.aps.md)                                   | DOCFRESH   | 0/8        | Executes [ADR-119](./decisions/119-documentation-freshness-from-declared-upstream.md) — **Draft** until that ADR is Accepted. Makes the `Upstream` + `Last reviewed` metadata every governed doc already declares machine-checked against git history (new `docs-owed` surface); file-level upstreams gate, directory/glob upstreams advisory; moves the trigger off `markdownlint-required` so a code-only PR can no longer stale docs unnoticed. Public docs get non-rendered frontmatter governance (`owner`/`upstream`/`verified_against`) verified at the **release boundary**, not per-PR. Measured baseline: 86 owed, 103 of 228 docs checkable. Complements DOCSYNC (owns public content) and the Complete DOCGOV (seeded the convention). |
