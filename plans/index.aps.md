...
| Daemon-working product slate | Current candidate | MLP 1/17 (In Progress; MLP-001 Done 2026-05-13), INTL 0/9 (Ready, ready-to-start-Wave-3), carry-forward gates 6/6 confirmed (Wave 0 closed 2026-05-13) | Next tag candidate `v0.7.0-beta`; MLP-009 remains the hard release gate for any full-protection claim. Wave 1 in flight per `RELEASE-PLAN.md`. |
...
| ---- | ------ | -------- | ----- |
| N1 — Multi-Layer Protection v1 (MLP) | In Progress | 1/17 | Witness chain + hooks + L4 policy + baseline + multi-agent coord + rule distribution. New crates: `anvil-witness`, `anvil-hook`, `anvil-l4`, `anvil-config`, `anvil-baseline`, `anvil-attribution`. **Hard gate: MLP-009.** Promoted from Proposed during Wave 0 readiness review (2026-05-13). MLP-001 (project identity) reconciled Done 2026-05-13 against the v1-narrowed scope shipped in `activation/identity.rs` (22 tests green); composite-identity attach-check, `--new-identity` flag, and `anvil baseline` integration documented as deferred follow-ups (see MLP-001 footnotes). Wave 1 backbone in flight. |
...
| --------------------------------------------------------------------------------- | ------- | -------- | --------------------------------------------------------------------------------------------------------------------------------------------------- |
| [policy-engine](./modules/policy-engine.aps.md)                                   | POLENG  | Draft    | ADR-040 (Proposed), crates/anvil-kernel, crates/anvil-policy — substrate for OPAE/ORGHIER/POLLC/COMPLY/POLFED/CPACKS                                |
...
| [multilayer-protection](./modules/multilayer-protection.aps.md) | MLP | In Progress | 1/17 (MLP-001 Done 2026-05-13) | INTD, DRVR, RMCP/RMCPF, RTAI, anvil-checks, kindling-integration, anvil-cli activation/init/baseline; ADRs [036](./decisions/036-daemon-scope-discovery-and-boundaries.md) (rewritten), [037](./decisions/037-witness-chain-and-l4-policy.md), [038](./decisions/038-hook-surface-and-noise-discipline.md), [039](./decisions/039-baseline-policy-and-hard-pinned-classes.md) — all Accepted 2026-05-13. **MLP-009 hard release gate**; sits on top of INTD/DRVR. Sequenced as N1 in [next-release window](../RELEASE-PLAN.md#next-release-window-proposed--post-v060-beta-daemon-working-slate). Promoted from Proposed during Wave 0 (2026-05-13). Wave 1 in flight: MLP-001 reconciled Done against v1-narrowed scope (`activation/identity.rs`, 22 tests green; composite-attach + `--new-identity` + baseline-integration filed as deferred follow-ups in MLP-001 footnotes). Next: MLP-011 multi-format config. |
...
| [early-access-tests](./modules/early-access-tests.aps.md) | EATEST | Early access test infrastructure | In Progress |
