...
| [dev-environment-hardening](./modules/dev-environment-hardening.aps.md)         | DEVENV | In Progress | 1/8 (ADR-057, council `plan-6b3be127`. DEVENV-001 Merged 2026-05-29 via PR #2090 — `[profile.dev]`/`[profile.test]` `debug = "line-tables-only"`, ~27% smaller debug targets. Remaining Wave 1 DEVENV-002..007 harden the concurrent-agent box: layered Rust `target/` relocation off the full disk (committed `.cargo/config.toml` floor + per-worktree `CARGO_TARGET_DIR` override), `[profile.dev]` debuginfo trim, nx-rust executor relocation-awareness + `.anvil-building` sentinel, disk-pressure eviction timer, Node version alignment, and `.config/wt.toml` worktree-lifecycle + parity fixes; Wave 2 DEVENV-008 is a spike that exits with a go/no-go ADR on a reproducible base (mise/devcontainer/Nix) + nx-cache/sccache dedup. The `origin/dev`→`origin/main` base-ref precursor shipped via PR #2086.) |
