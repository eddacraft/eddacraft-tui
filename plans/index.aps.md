...
the `v0.6.3-beta` patch (2026-05-15) rolled WATCHUX 8/8 and ADOPT-005
`anvil uninstall` on top. The current planning window has moved to the
daemon-working product slate.

| Area | Status | Progress | Notes |
| ---- | ------ | -------- | ----- |
| Shipped baseline | Shipped | `v0.6.3-beta` tag (2026-05-15) | Wow-start activation, daemon-backed validation, the executable release operating model, and the beta watch UX / uninstall hotfix are behind us; current work should not reopen operational substrate scope. |
...
at 43/68 (MLP2-025 was split into MLP2-025 + MLP2-025b + MLP2-025c during the daemon-control wiring; the Stats footer now counts each sub-task separately). After the full MLP/MLP2 Council audit reopened MLP2-016 and MLP2-048
and added Group M (MLP2-061..-066), Group M closed 6/6 on 2026-05-16 via PRs
#1602 and #1604. Earlier wave prose that says MLP2-016 / MLP2-048 closed hard
...
| [adoption-friction](./modules/adoption-friction.aps.md)                 | ADOPT    | In Progress | 2/6 | First-week friction removal. **ADOPT-005 `anvil uninstall` shipped 2026-05-14 (PR #1521); ADOPT-001 hook coexistence Done 2026-05-15** (runbook at `docs/runbooks/anvil-hook-coexistence.md`); resource budget (-002 In Progress), AI auto-detect (-003 In Progress), shared ignore (-004), editor coexistence (-006) remain. Wave 3A. |
| [distribution-and-update](./modules/distribution-and-update.aps.md)     | DISTRIB  | In Progress | 2/5      | Harden `anvil update` + Homebrew + cadence policy so hotfix iteration reaches users. **DISTRIB-001 Merged via PR #1562** (minisign verification + ADR-045). **DISTRIB-002 Merged via PR #1569** (`anvil version --check` advisory surface + watch/status hint). Operator follow-up tracked in post-merge plans. ADR-044 §9 makes DISTRIB-001 / -002 load-bearing for the MCP-backend swap discovery gap. Wave 3A. |
...
| [watch-ux-advisory-rules](./archive/modules/watch-ux-advisory-rules.aps.md) | WATCHUX | Complete | 8/8 (WATCHUX-001..004 reconciled against merged PR #1497; WATCHUX-005..007 merged via PR #1524; WATCHUX-008 implemented on `feat/watchux-008-config-cache`) | anvil-cli audit/start/watch/status/config, anvil-kernel watch/watcher, anvil-tui watch surface, MLP config/baseline |
