...
| ---- | --- | ---- | ------ | ------- | ----------- |
| 1 | GCALL-003 — resident call edges + caller read API | Merged | [symbol-call-graph](./modules/symbol-call-graph.aps.md) | Lifts `FileSymbols.calls` into resident `EdgeType::Calls` edges via a symbol-granular `re_resolve_calls` (scoped to `dependents_of`) and a bounded, depth-clamped identity-only `callers_of` read API — the substrate GCTX-014 consumes. **Merged 2026-06-17 via #2708** (+ CALL-1 heuristic marker #2712). With GCALL-004/005/006/007 also Merged, the GCALL module is 7/7. | Cleanup agent advances Merged → Released/Shipped on the next release tag that includes the GCALL substrate. |
| 2 | GCALL-002 — TS/JS call-site extraction into `FileSymbols` | Merged | [symbol-call-graph](./modules/symbol-call-graph.aps.md) | The primary-language `calls` producer on the GCALL-001 (ADR-086) contract. **Merged 2026-06-17 via #2707**: the `CallSite` / `CalleeRef` / `LocalSymbolRef` types + the `serde(default) calls` channel on `FileSymbols`, and the TS/JS extractor call-site pass (caller via `for_file_symbols` ordinals / `module_scope` synthetic node; callee export-name + `via_import`, alias reverse-map + namespace member; unresolved drop) — fixtures for direct/method/imported/namespace/module-scope/unresolved, deterministic. | Cleanup agent advances Merged → Released/Shipped on the next release tag that includes #2707. |
| 3 | GCALL-001 — call-graph substrate design + ADR | Merged | [symbol-call-graph](./modules/symbol-call-graph.aps.md) | The gating design that unblocks the caller line. **Merged 2026-06-17 via #2705** as [ADR-086](./decisions/086-symbol-call-graph-substrate.md) (Accepted, operator 2026-06-17): caller `SymbolIdentity` → callee `SymbolIdentity` edges, the `#[serde(default)] calls` `FileSymbols` channel + lift-time `re_resolve_calls` (scoped to `dependents_of`), best-effort static model (overload fan-out capped + `heuristic:true`, unresolved drop + `partial`), a `collect_dependents`-shaped bounded identity-only `callers_of` read API, the ADR-031 budget posture (no double parse, per-file cap, cold-warm via ADR-085), and the CE-5/PV-9 identity-only egress posture. Adversarial-reviewed (8 gaps closed). | Cleanup agent advances Merged → Released/Shipped on the next release tag that includes #2705. |
| 4 | GCTX-013 `anvil_affected_tests` — change→test attribution + coverage gaps | Merged | [graph-context-delivery](./modules/graph-context-delivery.aps.md) + [graph-v2-foundation](./modules/graph-v2-foundation.aps.md) | The test-attribution tool on the GCTX-010/011/012 spine. **Merged 2026-06-17 via #2700**: the `anvil/gctx/affected_tests` RPC + `GctxProjector::collect_affected_tests` (reverse BFS for dependent tests + a budget-shared forward BFS for transitive coverage) / `project_affected_tests` + sealed identity-only `AffectedTestsReport` (tests with evidence edges + distance, plus coverage gaps; `heuristic: true`) + the graph-free `anvil_affected_tests` MCP tool, reusing the CE-5/CE-6/CE-7/C3/CE-10/CE-11 gates — no new substrate. | Cleanup agent advances Merged → Released/Shipped on the next release tag that includes #2700. |
| 5 | GCTX-012 `anvil_impact_of_change` — change blast-radius report | Merged | [graph-context-delivery](./modules/graph-context-delivery.aps.md) + [graph-v2-foundation](./modules/graph-v2-foundation.aps.md) | The change-impact report on the GCTX-010/011 spine. **Merged 2026-06-17 via #2693**: the `anvil/gctx/impact_of_change` RPC + `GctxProjector::collect_impact` (multi-source reverse-impact BFS) / `project_impact` + sealed identity-only `ImpactReport` (affected symbols + dependent files + heuristic known tests, both result sets node-budget capped) + the graph-free `anvil_impact_of_change` MCP tool, reusing the CE-5/CE-6/CE-7/C3/CE-10/CE-11 gates — no new substrate. | Cleanup agent advances Merged → Released/Shipped on the next release tag that includes #2693. |
| 6 | GCTX-011 `anvil_find_dependents` — dependency traversal | Merged | [graph-context-delivery](./modules/graph-context-delivery.aps.md) + [graph-v2-foundation](./modules/graph-v2-foundation.aps.md) | The first dependency-traversal tool on the GCTX-010 spine. **Merged 2026-06-16 via #2685**: the `anvil/gctx/find_dependents` RPC + `GctxProjector::collect_dependents`/`project_dependents` (depth-bounded reverse-impact walk, sorted-frontier + node-budget capped, keyset cursors domain-separated from search) + sealed file-keyed identity-only DTOs + the graph-free `anvil_find_dependents` MCP tool, reusing the CE-5/CE-6/CE-7/C3/CE-10/CE-11 gates. Caller traversal (symbol call graph) split to GCTX-014 (Merged 2026-06-17 via #2715). | Cleanup agent advances Merged → Released/Shipped on the next release tag that includes #2685. |
| 7 | GCTX-010 `anvil_search_symbols` — Phase 1 pilot (CE-5 spine) | Merged | [graph-context-delivery](./modules/graph-context-delivery.aps.md) + [graph-v2-foundation](./modules/graph-v2-foundation.aps.md) | The CE-5 hard-gate pilot — sealed egress DTO + `GctxProjector` + structural no-leak spine and the identity-only tool. **Merged 2026-06-16 via #2657**: spine + tool #2637, CE-6 cursors #2645, CE-10/CE-11 telemetry+kill-switch #2648, then the C1 cold-start warm-up triggers on top of the DSV-045 executor (#2674). The C4 `gctx.egress` manifest flag is Phase-2. | Cleanup agent advances Merged → Released/Shipped on the v0.9.0-beta tag. |
...
| [daemon-save-time-validation](./modules/daemon-save-time-validation.aps.md)     | DSV    | In Progress | 20/20 (all items Merged-or-Released; daemon save-time arc Released/Shipped via v0.8.0-beta, 2026-06-11; Sub-phase A 9/9 + A-W 2/2 Merged, deferred follow-ups Done, A′ 2/2 Done — GV2 backing swap reconciled from #2446 and DSV-021 #2473 flipped `ANVIL_WATCH_DAEMON` default-on with live-daemon guard + opt-out/force. **DSV-045 (full-scan executor) Merged 2026-06-16 via #2674 per [ADR-085](./decisions/085-daemon-full-scan-executor.md)** (planning council `plan-898d9222`) — drives `request_full_scan` `Pending→Running→Clean`/`Bounded` and populates the warm cache without a save, closing the ADR-084 C1 cold-start gap; new `AssuranceState::Bounded` wire variant (+`#[serde(other)]` fallback) + scan watchdog + dirty-race guard; unblocks the GCTX-010 warm-up triggers. **DSV-030 (warm-start persistence, sub-phase B) Merged 2026-06-17 via #2688 per [ADR-069](./decisions/069-graph-v2-persistence.md)** — daemon disk I/O + timing for the (pre-existing) snapshot core behind `ANVIL_PERSIST_GRAPH` (default-off, fail-closed); disk-authoritative reconcile prune. Module stays In Progress until DSV-045/-030 ride the v0.9 release (then Complete). PR detail in the module file) |
...
| Module                                                                      | Scope | Status      | Progress           |
| --------------------------------------------------------------------------- | ----- | ----------- | ------------------ |
| [continuous-improvement-backlog](./modules/continuous-improvement-backlog.aps.md) | CIB   | In Progress | 60/103 (standing continuous-improvement intake; recent CIB-041..-046 release-reliability + flag-gating items Merged/Ready; CIB-054/-055 pre-tag v0.8.0 changelog-wording + release-plan-table reconcile Merged 2026-06-10 via PRs #2493/#2496; CIB-056 driver-client Windows pipe SID gate Merged via PR #2485 (#2484 increment 2 remains); CIB-057 capsule as-built doc + CIB-058 v0.8.0-window as-built delta refresh Merged 2026-06-10; CIB-059 quickstart ADR-080 ordering In Progress 2026-06-11; CIB-060 auth-wall early-access pointer Merged 2026-06-22 via PR #2858; CIB-061 remaining as-built delta refresh Merged via PR #2526; CIB-065 schema $id domain drift Merged 2026-06-11 via PR #2539; CIB-062..-064 golden-path polish gaps (auth-gate WARN JSON on stderr + install.sh docs URL + double-reported planted secret) Merged 2026-06-11 via PRs #2529/#2528/#2530; CIB-071 migrate user-facing diagnostics to miette Merged 2026-06-14 via PRs #2597/#2611; CIB-072 clear stuck `ready_restart_required` on Windows when daemon attestation is Unreachable Merged 2026-06-22 via PR #2859 (issue #2609); CIB-073..077 demo-readiness/release-flow gaps Draft 2026-06-14; CIB-078 freeze the `anvil policy eval --json` output contract at v1 before EVAL binds Merged 2026-06-17 via PR #2717; CIB-079 Rust serde-hygiene + clone-in-hot-loop AST anti-pattern rules (RSTLAN-003b deferral) Proposed 2026-06-18; CIB-080..082 external-codebase FP dogfood residuals (secret-detection test-vector FPs, RS-005 doc-config residual, tree-sitter-rust parse-skip gap, CIB-083 context-stack interpolation masker) Proposed 2026-06-18; CIB-084 bounded baseline/drift readers Merged 2026-06-20 via PR #2815; CIB-085 clawpatch Rust contract batch Merged 2026-06-20 via PR #2823; CIB-086..090 OPSUP milestone-council follow-ups (FP egress bridge, FP local read path, drift-migrate backup-retention + partial-failure reporting, unknown-check-ID semantics reconcile, NDJSON sidecar O_NOFOLLOW hardening) Proposed 2026-06-21 (`plans/reviews/2026-06-21-opsup-council.md`); CIB-091..095 v0.9.0-beta release-council survivors (091 GCTX egress incl. CE-3 cut-blocker, 092 persistence wire-integrity, 093 GV2 substrate hot-path/trust + N1/N3/N7 trust gates, 094 USAGE controls + N2, 095 intercept follow-through + N2/N5/N8) Merged 2026-06-22 via PR #2852 (`plans/audits/2026-06-21-v090-council-survivors.md`; 092c/092d/092h deferrals filed as CIB-096 (sweep wiring) Merged 2026-06-22 via PR #2870, CIB-097 (write-anchor, Merged 2026-06-22 via PR #2865), CIB-098 (notification delivery) Merged 2026-06-22 via PR #2887, CIB-102 (delete/sweep dirfd anchoring) Merged 2026-06-22 via PR #2884); CIB-099..101 release-window hygiene follow-ups Ready 2026-06-23 (PR #2890 triage); CIB-103 GCTX cursor-fingerprint (HMAC) decision split from CIB-099 Proposed 2026-06-23; per-item status in the module file) |
...
| [kindling-daemon-sink](./modules/kindling-daemon-sink.aps.md) | KDS | Proposed | 0/5 | USAGE (the producers), `kindling-client` + `kindling-spool` (crates.io `>=0.1`, the upstream Rust-canonical Kindling daemon/client/spool), ADR-035 / D-035 (three-pipe rule — this is its write-side realisation), ADR-064 (daemon dep-boundary: the networking client stays in `anvil-cli`, never `anvil-intercept`). **DRAFT** authored from the Kindling side — makes the Kindling daemon (SQLite) authoritative for Anvil's observations and demotes the `usage.ndjson` workaround to a transient `kindling-spool` fallback. KDS-001 the `KindlingDaemonSink` over the spooled client · KDS-002 wire it primary + a `daemon\|ndjson\|off` sink-selection flag (privacy default preserved) · KDS-003 daemon-vs-NDJSON parity (PORT-011 acceptance) · KDS-004 re-source `anvil kindling usage` views from the authoritative store · KDS-005 retire the bespoke `DaemonUsageSink`. Blocked on the kindling crates landing on crates.io; needs council + a D-035 reconciliation decision before Ready. |
...
| [insecure-construction-catalogue](./modules/insecure-construction-catalogue.aps.md)                   | INSEC      | 0/8        | ADR-087 (accepted), ADR-071 (AST tier), `patterns/` registry + `anvil-checks` scanner — **Ready** (realises the syntactic-smell subset of the sec-context triage: first-wave `weak-cryptography` + `unsafe-rendering` families, SSTI into `dynamic-execution`, scope-guard note, FP-bar dogfood. INSEC-007 `injection-smell` (AST) + INSEC-008 insecure-RNG stay **Proposed — deferred opt-in** per ADR-087. Distinct from the SEC CI-pipeline module.) |
...
| [test-coverage-uplift](./modules/test-coverage-uplift.aps.md)               | TCOV  | In Progress | 25/25 (Phase 1+2 complete: 13/13; Phase 3: TCOV-014 Complete + TCOV-015..-018 Merged (4), TCOV-019..-021 Complete (covered by the Rust RMCPF MCP port #2809; archived TS target retired under ADR-033); Phase 4: TCOV-022..-025 all Merged (re-scoped 2026-06-20 to `crates/eddacraft-tui/` widgets/theme/keyboard — primitives crate kept the targets after the anvil-tui split). All 25 work items done; module ready for Complete pending release-tag verification) | TFIX                    |
