...
| [lang-tail-wave-2](./modules/lang-tail-wave-2.aps.md) | LTW2 | Ready | 2 | WebAssembly text (`.wat`/`.wast`) + Zig (`.zig`/`.zon`) at T1, batched per the LANGTAIL pattern. Owner-directed addition under [ADR-093](./decisions/093-tail-wave-2-wasm-text-and-zig-reentry.md) (**Accepted** 2026-06-29). Scope is **text only — binary `.wasm` is excluded**. **LTW2-001 grammar audit Done 2026-06-29**: both bind + parse tree-sitter 0.26 (spike-verified) — **wave membership is both**. Zig ships from the published `tree-sitter-zig` 1.1.2 crate; **WAT is vendored** (`wasm-lsp/tree-sitter-wasm` `wat/parser.c`, ABI 13, no scanner, dormant since 2022 — owner accepted the maintenance liability). **LTW2-002 (WAT) + LTW2-003 (Zig) Ready** for wiring; LTW2-004 acceptance + LTW2-005 doc fix follow. Zig **re-enters from the §13 cut list**. |
