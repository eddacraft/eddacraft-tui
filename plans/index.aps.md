...
| [activation-tui](./modules/activation-tui.aps.md)                     | ACTTUI   | In Progress | 7/14 | TUI-first `anvil start`. JOURNEY requires ACTTUI-009 (end-to-end consent), ACTTUI-010 (contract/PTY matrix), and ACTTUI-012 (default-path polish), plus WOW-005 — all Merged. ADR-103 Accepted 2026-07-25; ACTTUI-013 (Ready) now owns the Release 2 TTY-default flip, until which `anvil start` stays opt-in behind `--tui` / `ANVIL_ACTIVATION_TUI=1`. ACTTUI celebration and richer diagnostic depth remain coordinated enhancements unless required to close a correctness defect. |
