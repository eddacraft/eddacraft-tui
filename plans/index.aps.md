...
| ---- | --- | ---- | ------ | ------- | ----------- |
| 1 | DSV-050..051 — headless save-time driver (Sub-phase C tail) | Ready | [daemon-save-time-validation](./modules/daemon-save-time-validation.aps.md) | **v0.9.0-beta usefulness cut-line:** DSV-048 Merged via PR #3186 (CLI driver entry), DSV-047 Merged via PR #3191 (daemon supervisor), and DSV-049 Merged 2026-07-05 on `main` (status wire + `#[serde(other)] Unknown`). Remaining wave: DSV-050 (start copy; depends on 049's wire field) → DSV-051 (E2E/runbook + Windows leg). | Start DSV-050 (`anvil start` / activation copy) so an attached driver removes the stale foreground-`watch` next-step. |
