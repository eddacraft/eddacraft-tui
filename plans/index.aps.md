...
| [release-orchestration](./modules/release-orchestration.aps.md)                 | RELORCH | Blocked | 3/12 (**Practically unblocked 2026-05-11** — CICD-012 and OPMODEL-012 both landed; the Status column still reads `Blocked` because the module file's own header has not yet been flipped by the in-flight RELORCH agent. The Current window table above tracks the practical state. RELORCH-001 completed 2026-05-10 with `plans/specs/2026-05-10-release-orchestration-design.md`; RELORCH-003 completed 2026-05-10 with standalone `scripts/release/assess.sh` and command-specific harness tests; RELORCH-004 completed 2026-05-10 with standalone `scripts/release/preflight.sh` and command-specific harness tests. RELORCH-002 has the initial `scripts/release/_test/` harness and `.github/workflows/release-harness.yml`; the broader fixture matrix work is what's resuming. Phase 1 nucleus remaining: -002 harness, -010 closeout, -011 wire-up + decommission. Phase 2: -005..-009 prepare/promote/tag/monitor/verify. RELORCH-012 added 2026-05-11 to close the release-record schema gap (yank state + policyDecisions enum) surfaced by OPMODEL-011 rollback playbooks. Successor to archived RELMGMT; supersedes parts of `2026-04-20-relmgmt-agent-driven-release-design.md` while inheriting its no-persistent-manifest tradeoff as a hard constraint. Tracks the work to honour PR #1368's `scripts/release/*.sh` target surface; design now precedes implementation — see Council session `plan-9a6b3a94` 2026-05-09.) |
