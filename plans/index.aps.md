...
| [first-run-wow](./modules/first-run-wow.aps.md)                         | WOW      | In Progress | 0/6 | Tutorial and welcome first-run uplift, created 2026-07-08 via planning-workflow from the operator's first-run-experience review. Makes tutorial command execution evident-by-affordance (command bar + run/read-only badges + step-aware help, WOW-001; deterministic typed-command reveal, WOW-002) and personalizes the first run from the discovery scan (per-domain picker counts WOW-003; completion findings delta WOW-004). WOW-005 (first-win reroute after discovery) and WOW-006 (autoplay demo on a sandboxed fixture) are Draft behind design gates (first-run fix-write consent posture; sandbox lifecycle). Complements — not duplicates — the CIB-162..179 welcome/start repair items; LAUNCH-014 honesty pins unchanged. |
| [activation-tui](./modules/activation-tui.aps.md)                     | ACTTUI   | In Progress | 0/9 | TUI-first `anvil start` on `eddacraft-tui` widgets (Tree, ParallelProgress, Select, Confirm, OverlayStack, Toast, BigBanner, LogPanel). Created 2026-07-08 via planning-workflow + planning-council direction-validate. **Single release cohort with welcome** — start + welcome both ship together; phase B runs ACTTUI-003..005 ∥ ACTTUI-008 ∥ WOW-001..004; phase C gates TTY-default on both surfaces. Council `proceed` with `amend`: ACTTUI-000 merged (PR #3232, ADR-103 + plain fixture spec); ACTTUI-001 scaffold + opt-in `--tui` dispatch In Progress; ACTTUI-002 progress-event contract In Progress; ACTTUI-003 working progress UI In Progress; ACTTUI-004 consent chrome In Progress; ACTTUI-005 verdict tree In Progress; ACTTUI-006 tier-evidence LogPanel In Progress; ACTTUI-007 fixture hardening In Progress; ACTTUI-008 welcome widget convergence In Progress; MCP unticked consent; `ANVIL_HOME` banner; orchestrator event contract; 007 before default flip. Complements ACTMO, WOW, CIB-162..179; PR #3231 pins WOW-005/006 downstream gates on ACTTUI foundation/consent without changing the phase-B release cohort. |
...
| [cli-command-truth](./modules/cli-command-truth.aps.md)                                                 | CLICT     | 0/7        | CLI command documentation vs runtime audit — **In Progress** (runtime registry + slices 1–6 audited; CLICT-004..006 Done 2026-07-07: watch, gate vocabulary, intercept/workspace runbook reconciliation; CLICT-001..003 remain recorded In Progress pending bookkeeping; CLICT-007 Proposed: tier-2 runbook sweep; `docs/reviews/cli-command-truth-review.md`) |
