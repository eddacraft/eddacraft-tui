...
| [skill-packaging-distribution](./modules/skill-packaging-distribution.aps.md)     | SKPKG   | In Progress | ADR-018 (product/IP architecture), SKOBS-002 (manifest alignment) — SKPKG-001 design spec drafted 2026-07-02 (`plans/specs/2026-07-02-skill-packaging-distribution.md`), pending owner review; proposes `anvil skill install` sibling to `anvil mcp install`; SKPKG-002..005 filed as follow-on decisions (IP-boundary ratification, update cadence, target-harness reconciliation, SKOBS schema coordination) |
