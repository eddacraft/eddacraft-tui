...
| [tui-next](./modules/tui-next.aps.md)                                                                  | TUIN       | 5/13       | Post-migration design deferred out of TUIR (parser policy, lifecycle ownership, runner-shell shape). TUIN-001/-011 docs Merged, TUIN-009 spike Done; TUIN-012 Done (operator override) — feature-gated `lifecycle` + `runner` fallback CLI shell. TUIN-003 Done 2026-06-21 — `eddacraft-tui::mode` TTY/alt-screen/colour-depth typed-enum probes (zero new deps), Anvil `OutputMode` delegates (D-TUIN-002 Accepted). Per-item detail in the module file. |
