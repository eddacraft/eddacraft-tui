...
| Module                                                                      | Scope | Status      | Progress           |
| --------------------------------------------------------------------------- | ----- | ----------- | ------------------ |
| [continuous-improvement-backlog](./modules/continuous-improvement-backlog.aps.md) | CIB   | In Progress | 47/78 (standing continuous-improvement intake; recent CIB-041..-046 release-reliability + flag-gating items Merged/Ready; CIB-054/-055 pre-tag v0.8.0 changelog-wording + release-plan-table reconcile Merged 2026-06-10 via PRs #2493/#2496; CIB-056 driver-client Windows pipe SID gate Merged via PR #2485 (#2484 increment 2 remains); CIB-057 capsule as-built doc + CIB-058 v0.8.0-window as-built delta refresh Merged 2026-06-10; CIB-059/-060 beta user-journey walkthrough follow-ups (quickstart ADR-080 ordering + auth-wall access pointer) In Progress 2026-06-11; CIB-061 remaining as-built delta refresh Merged via PR #2526; CIB-065 schema $id domain drift Merged 2026-06-11 via PR #2539; CIB-062..-064 golden-path polish gaps (auth-gate WARN JSON on stderr + install.sh docs URL + double-reported planted secret) Merged 2026-06-11 via PRs #2529/#2528/#2530; CIB-071 migrate user-facing diagnostics to miette Merged 2026-06-14 via PRs #2597/#2611; CIB-072 clear stuck `ready_restart_required` on Windows when daemon attestation is Unreachable (Proposed 2026-06-14, issue #2609); CIB-073..077 demo-readiness/release-flow gaps Draft 2026-06-14; CIB-078 freeze the `anvil policy eval --json` output contract at v1 before EVAL binds Merged 2026-06-17 via PR #2717; per-item status in the module file) |
...
| [lang-python](./modules/lang-python.aps.md)     | PYLAN  | In Progress | 2     | §8.1 — promoted Draft → In Progress 2026-06-17 on operator direction ("build lang-python first") to unblock GCALL-005. **PYLAN-001/-002** (tree-sitter-python grammar + symbol/import extractor) Merged via #2716; **PYLAN-005** (entry-point detection) Merged via #2731. In progress: **PYLAN-006/-008** (import resolver + boundary/architecture-validate surface, #2732) and **PYLAN-003/-004/-007** (anti-pattern catalogue + `#`-suppression + `.py` drift default-on, #2734). Prerequisites LANGTS + RSTLAN both Complete; **PYLAN-009** (User B/C dogfood validation) anticipated, gated on the deferred Ready-Checklist items (owner, re-scoring, User C framework choice) |
