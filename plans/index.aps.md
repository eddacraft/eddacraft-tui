...
| [sarif-output](./modules/sarif-output.aps.md) | SARIFOUT | 6 | **Proposed** 2026-05-29 — additive `--format sarif` on `anvil check`/`gate`/`audit`, promoted from CIB-014 after the [2026-05-29 design pass](./specs/2026-05-29-sarif-output-design.md) resolved the flag-surface (`--format` value-enum; `--json` becomes an alias), module-home (this dedicated module), and shared-model (thin shared SARIF emitter + per-command adapters, no engine refactor) gates. Pinned to the GitHub Code Scanning subset of SARIF 2.1.0 (results/rules/locations/suppressions). Two candidate ADRs flagged. Not Ready until the three decisions are signed off. |
