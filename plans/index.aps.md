...
| [operating-model-migration](./modules/operating-model-migration.aps.md)                               | OPMODEL    | 0/12       | Cross-cutting migration to the target Plan / Build / Release operating model — **Proposed**. Coordinates current-state vs target-state boundaries, APS lifecycle vocabulary, `main`-first branching, release records, deterministic agent guidance, review/council entrypoints, CI readiness, drift checks, and rollback playbooks. Implementation remains owned by specialist modules such as RELORCH and DOCGOV.                                |
