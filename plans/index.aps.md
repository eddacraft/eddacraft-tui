...
| [lang-python](./modules/lang-python.aps.md)     | PYLAN  | In Progress | 2     | §8.1 — promoted Draft → In Progress 2026-06-17 on operator direction ("build lang-python first") to unblock GCALL-005. **PYLAN-001/-002** (tree-sitter-python grammar + symbol/import extractor) Merged via #2716; **PYLAN-005** (entry-point detection) Merged via #2731. **PYLAN-006/-008** (import resolver + boundary/architecture-validate surface) Merged via #2732; **PYLAN-003/-004/-007** (`python-reliability` anti-pattern catalogue + `#`-suppression + `.py` drift default-on) Merged via #2734. Eight of nine items Merged — Python functionally at T3. Prerequisites LANGTS + RSTLAN both Complete; **PYLAN-009** external-codebase validation **done** (httpx + rich, ~270 `.py`, 0 panics; 2.9%→**0.0% FP** after 3 precision fixes; **proposed N = 1%**; evidence `plans/reviews/2026-06-18-pylan-009-external-validation.md`). Awaiting operator: accept N, name owner, re-scoring gate; module advances to Complete only with that sign-off + release-tag evidence |
