...
| ---- | --- | ---- | ------ | ------- | ----------- |
| 1 | GCTX-010 `anvil_search_symbols` — Phase 1 pilot (CE-5 spine) | Ready | [graph-context-delivery](./modules/graph-context-delivery.aps.md) + [graph-v2-foundation](./modules/graph-v2-foundation.aps.md) | The assistant-facing substrate is complete: ADR-083 + PV-9 landed, GV2-023 #2621 + GV2-020 #2622 are Merged, **GV2 20/20 (GV2-031 #2627)**, **GCTX Phase 0 complete** (GCTX-001 #2628 + GCTX-002 #2619), and the graph-handle design gate is **resolved — ADR-084 Accepted via #2632** (daemon-RPC + daemon-side projection, option A). **GCTX-010 is now Ready** with C1–C5 folded as acceptance criteria. | Pick up **GCTX-010** through dev-workflow (TDD): build the spine — `anvil-gctx-types` (sealed DTOs + structural no-leak test), `anvil-gctx-egress` (`GctxProjector`), the `anvil/gctx/search_symbols` RPC on its own `GctxDispatch` — then the identity-only tool. Sequence against DLIFE (daemon-required). |
| 2 | GV2-031 — lift re-export edges for transitive privilege | Merged | [graph-v2-foundation](./modules/graph-v2-foundation.aps.md) + [`RELEASE-PLAN.md`](../RELEASE-PLAN.md) | The last internal GV2 substrate item. Merged 2026-06-15 via PR #2627 — re-exports lifted into `Reexports` edges; `annotate_trust`/`certify` follow them transitively to a privileged module (monotone `previously_reexported_privileged` baseline); daemon/watcher `all_reexports` accumulator; backing-parity test covers re-export chains. | Cleanup agent advances Merged → Released/Shipped on the v0.9.0-beta tag. |
...
| [graph-context-delivery](./modules/graph-context-delivery.aps.md) | GCTX | In Progress | 2/13 (Phase 0 — Delivery Contract — complete: **GCTX-001 projection contract Merged 2026-06-15 via #2628** (spec folds PV-9 CE-1..CE-12 onto the GV2-023 contract) and **GCTX-002 MCP delivery target Merged 2026-06-15 via #2619** (discharged by ADR-083 Accepted). Graph-handle access fixed by **ADR-084 Accepted via #2632** (daemon-RPC + daemon-side projection, two crates, option A same-process service). **GCTX-010 (`anvil_search_symbols`) is now Ready** — the CE-5 hard-gate pilot that builds the sealed egress DTO + `GctxProjector` + no-leak-test spine, with C1–C5 folded as acceptance criteria; GCTX-011/012/013/021..023/030 stay Draft pending that spine) | GV2 |
...
| [daemon-lifecycle](./modules/daemon-lifecycle.aps.md)                           | DLIFE  | In Progress | 3/6 (DLIFE-001 Done — ADR-082 Accepted 2026-06-15 with the tiered startup mode: `anvil start` auto-starts the daemon; `anvil watch` prompts in TTY and falls back headless. ADR-079 superseded. DLIFE-002 — idempotent `ensure_daemon` primitive (probe → same-user lock → re-probe → detached spawn → bound-wait, Unix-first) — Merged via #2644. DLIFE-006 — terminating `--verify` diagnostic for the daemon-unreachable case (#2609) — Merged via #2639. DLIFE-003/-004 Proposed; DLIFE-005 Blocked.) |
