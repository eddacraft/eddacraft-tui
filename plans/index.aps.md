...
| ---- | --- | ---- | ------ | ------- | ----------- |
| 1 | GCALL-001 — call-graph substrate design + ADR | Schedule | [symbol-call-graph](./modules/symbol-call-graph.aps.md) | The GCTX Phase-1 Ready queue is drained and GCTX-014 `anvil_find_callers` is Blocked on resident symbol call edges the warm graph does not yet carry. GCALL (filed 2026-06-17) is the producer-side substrate that unblocks it; GCALL-001 is the Critical, dependency-free design gate that fixes the call-edge model (overload/cycle/recursion semantics), the `FileSymbols` call-site contract, the ADR-031 save-time budget posture, and the PV-9 caller-egress posture before any extraction begins. | Shape **GCALL-001**: author + accept the call-graph ADR in `DECISION-LOG.md`, which lets GCALL-002 (extraction) and GCALL-003 (resident edges + read API) flip to Ready. |
