...
| [aps-canonical-alignment](./modules/aps-canonical-alignment.aps.md)                                   | APSCAN     | 3/11       | Migration from Anvil's original APS dialect to canonical `anvil-plan-spec` v0.3.0 while preserving Anvil-specific release metadata and lifecycle prose — **In Progress**. APSCAN-001 Done: `pnpm aps:active-lint --list-files` now enumerates active APS lint scope while excluding `plans/archive/**` and legacy `.steps.md` execution plans. APSCAN-002 Done: portable APS rules now live in `plans/aps-rules.md`, Anvil-specific workflow/release/documentation context lives in `plans/project-context.md`, and `AGENTS.md` links agents to both surfaces. APSCAN-011 Merged 2026-05-24 via PR [#1906](https://github.com/eddacraft/anvil-001/pull/1906) at `4c6e1e2a`: APS-only TUI work dashboard (`anvil plan dashboard`) with local snapshot parsing, read-only Ratatui surface, warnings, JSON/plain fallbacks, and a future GitHub/CI enrichment seam. Remaining work tracks parser/validator aliases, `## Work Items`, `.actions.md`, canonical statuses, issues/context package support, CLI adoption boundary, progress-counter reconciliation, and the first active-module migration wave. Numeric module filename prefixes remain allowed when dependency order benefits from them. |
