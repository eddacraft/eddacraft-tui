...
| [compliance-policy-packs](./modules/compliance-policy-packs.aps.md)               | CPACKS  | In Progress    | Re-scoped 2026-07-11: the starter pack **shipped** (`anvil-baseline` via POLRESET-007 #3167 + OPAE-004 install); CPACKS-001..005 satisfied-by; **2026-08-24:** CPACKS-006 **Merged** via #4107 (eval wrappers, unblock path 2) and CPACKS-007 **Merged** via #4113. The wrappers live **outside** the shipped pack, with lockstep tests in `starter_proof.rs` failing on drift. **CPACKS-009** (matcher/branch coverage — 6 of 10 `sensitive-paths` matchers are silently deletable) and **CPACKS-010** (re-run the falsification against the landed suites) filed 2026-08-24 from planning council `council-9021df43`. Broad OWASP/SOC2/ISO/GDPR/AI packs stay behind CPACKS-008 (COMPLY evidence semantics + AGOV signals). |
