...
| ---- | --- | ---- | ------ | ------- | ----------- |
| 1 | GCALL-001 — call-graph substrate design + ADR | In Progress | [symbol-call-graph](./modules/symbol-call-graph.aps.md) | The GCTX Phase-1 Ready queue is drained and GCTX-014 `anvil_find_callers` is Blocked on resident symbol call edges the warm graph does not yet carry. GCALL (filed 2026-06-17) is the producer-side substrate that unblocks it; GCALL-001 is the Critical, dependency-free design gate that fixes the call-edge model (overload/cycle/recursion semantics), the `FileSymbols` call-site contract, the ADR-031 save-time budget posture, and the PV-9 caller-egress posture before any extraction begins. **Design authored as [ADR-086](./decisions/086-symbol-call-graph-substrate.md) (Proposed), adversarial-reviewed.** | Review + **accept ADR-086** (council/owner), which flips GCALL-001 → Merged and lets GCALL-002 (extraction) + GCALL-003 (resident edges + read API) flip to Ready. |
| 2 | GCTX-013 `anvil_affected_tests` — change→test attribution + coverage gaps | Merged | [graph-context-delivery](./modules/graph-context-delivery.aps.md) + [graph-v2-foundation](./modules/graph-v2-foundation.aps.md) | The test-attribution tool on the GCTX-010/011/012 spine. **Merged 2026-06-17 via #2700**: the `anvil/gctx/affected_tests` RPC + `GctxProjector::collect_affected_tests` (reverse BFS for dependent tests + a budget-shared forward BFS for transitive coverage) / `project_affected_tests` + sealed identity-only `AffectedTestsReport` (tests with evidence edges + distance, plus coverage gaps; `heuristic: true`) + the graph-free `anvil_affected_tests` MCP tool, reusing the CE-5/CE-6/CE-7/C3/CE-10/CE-11 gates — no new substrate. With this the GCTX Phase-1 Ready queue is drained; the GCALL substrate (rank 1) unblocks the remaining caller line (GCTX-014). | Cleanup agent advances Merged → Released/Shipped on the next release tag that includes #2700. |
| 3 | GCTX-012 `anvil_impact_of_change` — change blast-radius report | Merged | [graph-context-delivery](./modules/graph-context-delivery.aps.md) + [graph-v2-foundation](./modules/graph-v2-foundation.aps.md) | The change-impact report on the GCTX-010/011 spine. **Merged 2026-06-17 via #2693**: the `anvil/gctx/impact_of_change` RPC + `GctxProjector::collect_impact` (multi-source reverse-impact BFS) / `project_impact` + sealed identity-only `ImpactReport` (affected symbols + dependent files + heuristic known tests, both result sets node-budget capped) + the graph-free `anvil_impact_of_change` MCP tool, reusing the CE-5/CE-6/CE-7/C3/CE-10/CE-11 gates — no new substrate. | Cleanup agent advances Merged → Released/Shipped on the next release tag that includes #2693. |
| 4 | GCTX-011 `anvil_find_dependents` — dependency traversal | Merged | [graph-context-delivery](./modules/graph-context-delivery.aps.md) + [graph-v2-foundation](./modules/graph-v2-foundation.aps.md) | The first dependency-traversal tool on the GCTX-010 spine. **Merged 2026-06-16 via #2685**: the `anvil/gctx/find_dependents` RPC + `GctxProjector::collect_dependents`/`project_dependents` (depth-bounded reverse-impact walk, sorted-frontier + node-budget capped, keyset cursors domain-separated from search) + sealed file-keyed identity-only DTOs + the graph-free `anvil_find_dependents` MCP tool, reusing the CE-5/CE-6/CE-7/C3/CE-10/CE-11 gates. Caller traversal (symbol call graph) split to GCTX-014 (Blocked — no call edges in the warm graph). | Cleanup agent advances Merged → Released/Shipped on the next release tag that includes #2685. |
| 5 | GCTX-010 `anvil_search_symbols` — Phase 1 pilot (CE-5 spine) | Merged | [graph-context-delivery](./modules/graph-context-delivery.aps.md) + [graph-v2-foundation](./modules/graph-v2-foundation.aps.md) | The CE-5 hard-gate pilot — sealed egress DTO + `GctxProjector` + structural no-leak spine and the identity-only tool. **Merged 2026-06-16 via #2657**: spine + tool #2637, CE-6 cursors #2645, CE-10/CE-11 telemetry+kill-switch #2648, then the C1 cold-start warm-up triggers on top of the DSV-045 executor (#2674). The C4 `gctx.egress` manifest flag is Phase-2. | Cleanup agent advances Merged → Released/Shipped on the v0.9.0-beta tag. |
| 6 | GV2-031 — lift re-export edges for transitive privilege | Merged | [graph-v2-foundation](./modules/graph-v2-foundation.aps.md) + [`RELEASE-PLAN.md`](../RELEASE-PLAN.md) | The last internal GV2 substrate item. Merged 2026-06-15 via PR #2627 — re-exports lifted into `Reexports` edges; `annotate_trust`/`certify` follow them transitively to a privileged module (monotone `previously_reexported_privileged` baseline); daemon/watcher `all_reexports` accumulator; backing-parity test covers re-export chains. | Cleanup agent advances Merged → Released/Shipped on the v0.9.0-beta tag. |
...
| [symbol-call-graph](./modules/symbol-call-graph.aps.md) | GCALL | In Progress | 0/7 (filed 2026-06-17 to unblock GCTX-014 `anvil_find_callers`. Producer-side call-graph substrate — call-site extraction into `FileSymbols` + lifting `EdgeType::Calls` into the resident `SymbolGraph` via `apply_delta` + a bounded caller read API — within the ADR-031 save-time budget, behind a caller-egress privacy review. Not a GCTX item: GCTX consumes this substrate, mirroring how it consumes GV2. **GCALL-001 design authored as ADR-086 (Proposed)** — caller = nearest enclosing named symbol else a synthetic file node; lift-time `re_resolve_calls` (alias/namespace/barrel-follow, default-export = Unresolved) scoped to `dependents_of`; best-effort static (overload fan-out capped + `heuristic:true`, unresolved drop + `partial`); identity-only egress via the CE-5 `GctxProjector`. Acceptance flips GCALL-001 Merged + GCALL-002/003 Ready; GCTX-014 blocks on GCALL-003 + GCALL-007.) | GV2, anvil-kernel-types (`EdgeType::Calls`), ADR-031, ADR-064, lang-python |
