...
| [ci-cd-validation](./modules/ci-cd-validation.aps.md)                                                 | CICD       | 7/12       | Specialist CI/CD and validation operating model implementation — **In Progress**. Coordinates with OPMODEL rather than replacing it; CICD-001 added CI cost/run-reason reporting via `pnpm ci:cost`; CICD-002 added the shared path/risk classifier via `pnpm ci:classify`; CICD-003 added local validation commands via `pnpm validate:*`; CICD-004 redesigned fast PR validation around classifier-selected checks without routine PR coverage or broad matrices; CICD-006 moved TypeScript and Rust coverage off `dev`-push integration runs onto the nightly assurance workflow; CICD-007 targeted Semgrep/Trivy/TruffleHog/license-check/cargo-deny/acknowledgements at classifier-selected signals plus a weekly scheduled assurance sweep; CICD-009 reconciled — release-readiness workflow shipped via PR #1398 with exact-SHA validation, required readiness checks, candidate metadata artefact, and no publishing credentials. Remaining work covers integration validation, matrix targeting, drift checks, and migration-safe validation cutover. |
