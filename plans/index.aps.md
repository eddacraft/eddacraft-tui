...
| [fleet-telemetry](./modules/fleet-telemetry.aps.md) | FLEET | Draft | 0/0 | [ADR-106](./decisions/106-fleet-telemetry-consent-posture.md) (design gate, Proposed — nothing ships before it is Accepted); LAUNCH-013 `InstallMethod` detection; CIB-197 (local envelope enrichment — the beacon reuses those fields); `apps/anvil-api` (plausible ingest host). Filed 2026-07-14: opt-in/controlled **phone-home** fleet visibility — binary version, install method, and the FLAGS-design feature-usage dimensions (session-start snapshot + one stat per feature used; no PII, enumerated low-risk allowlist). A deliberate posture change vs the USAGE local-only privacy contract; distinct from EXPORT (tracing-pipe sink). **OQ3 resolved 2026-07-15 (operator): needed as investor evidence — no paying-customer gate; Ready waits only on the design gate.** Design gate drafted: [ADR-106](./decisions/106-fleet-telemetry-consent-posture.md) (Proposed 2026-07-15 — disclosed opt-out beacon, anonymous rotatable install UUID, enumerated dimension allowlist, ≤1 beacon/24h, `anvil telemetry` payload viewer, `apps/anvil-api` ingest; OQ1/OQ2 answers proposed there) — awaiting the operator's Accepted, then work items are drafted and the module goes Ready. Priority raised Medium→High. |
...
| [skill-packaging-distribution](./modules/skill-packaging-distribution.aps.md)     | SKPKG   | In Progress | Owner-approved beta distribution: embed a pinned customer-readable skill snapshot in Anvil, default interactive installs to user-global with project choice, and ship a managed cross-agent installer through SKPKG-008. Coordinates with MCPX and SKOBS-002. |
...
| [mcp-client-expansion](./modules/mcp-client-expansion.aps.md) | MCPX | In Progress | 0/6 (MCPX-001 verified first-wave contracts on 2026-07-14. Retain Claude Code/Cursor; add Codex, OpenCode, Gemini CLI, Antigravity, OpenClaw, VS Code/Copilot, Copilot CLI, Grok, Warp, and project-scoped Zed. Devin remains manual until it exposes a supported local mutation contract.) | RMCPF, RCLI3-016/-016b, ACTMO-012, SKPKG |
