...
| ---- | --- | ---- | ------ | ------- | ----------- |
| 1 | GCALL-003 — resident call edges + caller read API | In Progress | [symbol-call-graph](./modules/symbol-call-graph.aps.md) | The critical-path next item now that GCALL-002 landed the `calls` producer: lift `FileSymbols.calls` into resident `EdgeType::Calls` edges via a distinct symbol-granular `re_resolve_calls` (scoped to `dependents_of`) and a bounded, depth-clamped identity-only `callers_of` read API — the substrate GCTX-014 consumes. **In Progress via #2708.** GCALL-004 (Rust extraction, unblocked by GCALL-001) is an available parallel pickup. | Land **GCALL-003** (#2708): resident edges + `callers_of` mirroring `collect_dependents` (seen-set, GV2-026 depth clamp, sorted frontiers, truncation), fixtures for direct/transitive/recursion/overload. |
...
| 6 | GCTX-011 `anvil_find_dependents` — dependency traversal | Merged | [graph-context-delivery](./modules/graph-context-delivery.aps.md) + [graph-v2-foundation](./modules/graph-v2-foundation.aps.md) | The first dependency-traversal tool on the GCTX-010 spine. **Merged 2026-06-16 via #2685**: the `anvil/gctx/find_dependents` RPC + `GctxProjector::collect_dependents`/`project_dependents` (depth-bounded reverse-impact walk, sorted-frontier + node-budget capped, keyset cursors domain-separated from search) + sealed file-keyed identity-only DTOs + the graph-free `anvil_find_dependents` MCP tool, reusing the CE-5/CE-6/CE-7/C3/CE-10/CE-11 gates. Caller traversal (symbol call graph) split to GCTX-014 (Blocked — no call edges in the warm graph). | Cleanup agent advances Merged → Released/Shipped on the next release tag that includes #2685. |
...
| ------ | ----- | ------ | -------- | ------------ |
| [graph-v2-foundation](./modules/graph-v2-foundation.aps.md) | GV2 | In Progress | 20/20 (all items Merged; module awaits the v0.9.0-beta release tag → Released/Shipped → Complete. A′ slice shipped in v0.8.0-beta; Phase 1 complete — GV2-013/014 contracts Merged 2026-06-13 via #2578/#2579; GV2-023 consumer query contract Merged 2026-06-15 via #2621 (four read classes, one mapped scenario each for INTD/DRVR/GCTX/WEAVE); **GV2-020 multi-graph registry + typed query traits Merged 2026-06-15 via #2622** (the impl behind the GV2-023 contract; control/provenance as trait stubs, ADR-064); GV2-026 reverse-impact depth lever Merged 2026-06-14 via #2594 and GV2-030 sealed-DTO no-leak guard Merged 2026-06-14 via #2595; **GV2-031 re-export edge lift for transitive privilege Merged 2026-06-15 via #2627** — the last internal GV2 item) | KERN, anvil-graph-cache, ADR-061/063/064/067/069, ADR-031, INTD, GCTX, EDDA |
| [graph-context-delivery](./modules/graph-context-delivery.aps.md) | GCTX | In Progress | 6/14 (Phase 0 — Delivery Contract — complete: **GCTX-001 projection contract Merged 2026-06-15 via #2628** (spec folds PV-9 CE-1..CE-12 onto the GV2-023 contract) and **GCTX-002 MCP delivery target Merged 2026-06-15 via #2619** (discharged by ADR-083 Accepted). Graph-handle access fixed by **ADR-084 Accepted via #2632** (daemon-RPC + daemon-side projection, two crates, option A same-process service). **GCTX-010 (`anvil_search_symbols`) Merged 2026-06-16 via #2657** — the CE-5 hard-gate pilot delivered the sealed egress DTO + `GctxProjector` + no-leak-test spine across #2637/#2645/#2648 plus the C1 cold-start warm-up triggers on top of DSV-045 (#2674). **GCTX-011 (`anvil_find_dependents`) Merged 2026-06-16 via #2685** — file-keyed/identity-only dependency traversal on the spine. **GCTX-012 (`anvil_impact_of_change`) Merged 2026-06-17 via #2693** — multi-source change-impact report (affected symbols + dependent files + heuristic known tests), no new substrate. **GCTX-013 (`anvil_affected_tests`) Merged 2026-06-17 via #2700** — test attribution (evidence edges) + coverage gaps over the same spine (reuses GCTX-012's `is_test_file` + the forward `dependencies_of` edges), no new substrate. With the Phase 1 Ready queue drained, GCTX-021..023/030 stay Draft and **GCTX-014 (`anvil_find_callers`) is Blocked** on the new symbol-call-graph (GCALL) substrate module — GCALL-003 (resident call edges + read API) + GCALL-007 (caller-egress privacy review); split from GCTX-011 — the warm graph has no call edges. The C4 `gctx.egress` manifest flag is Phase-2.) | GV2 |
| [symbol-call-graph](./modules/symbol-call-graph.aps.md) | GCALL | In Progress | 2/7 (filed 2026-06-17 to unblock GCTX-014 `anvil_find_callers`. Producer-side call-graph substrate — call-site extraction into `FileSymbols` + lifting `EdgeType::Calls` into the resident `SymbolGraph` via `apply_delta` + a bounded caller read API — within the ADR-031 save-time budget, behind a caller-egress privacy review. Not a GCTX item: GCTX consumes this substrate, mirroring how it consumes GV2. **GCALL-001 Merged 2026-06-17 via #2705** as ADR-086 (Accepted, operator) — caller = nearest enclosing named symbol else a synthetic file node; lift-time `re_resolve_calls` (alias/namespace/barrel-follow, default-export = Unresolved) scoped to `dependents_of`; best-effort static (overload fan-out capped + `heuristic:true`, unresolved drop + `partial`); identity-only egress via the CE-5 `GctxProjector`. **GCALL-002 Merged 2026-06-17 via #2707** (TS/JS extraction — the `CallSite`/`CalleeRef`/`LocalSymbolRef` types + `calls` channel + extractor call-site pass). **GCALL-003** (resident edges + read API) is the next pick — In Progress (#2708); GCALL-004/005 (Rust/Python extraction) unblock on GCALL-001; GCTX-014 blocks on GCALL-003 + GCALL-007.) | GV2, anvil-kernel-types (`EdgeType::Calls`), ADR-031, ADR-064, lang-python |
...
| [daemon-lifecycle](./modules/daemon-lifecycle.aps.md)                           | DLIFE  | In Progress | 4/6 (DLIFE-001 Done — ADR-082 Accepted 2026-06-15 with the tiered startup mode. ADR-079 superseded. DLIFE-002 — idempotent `ensure_daemon` primitive (probe → same-user lock → re-probe → detached spawn → bound-wait, Unix-first) — Merged via #2644. DLIFE-003 — `anvil start` daemon lifecycle (interactive auto-start; CI/hook/piped + `--no-daemon`/`ANVIL_NO_DAEMON` fall back; honest `daemon:` line) — Merged via #2678. DLIFE-006 — terminating `--verify` diagnostic for the daemon-unreachable case (#2609) — Merged via #2639. DLIFE-004 Proposed; DLIFE-005 Blocked on 003/004.) |
...
| Module                                                                      | Scope | Status      | Progress           |
| --------------------------------------------------------------------------- | ----- | ----------- | ------------------ |
| [continuous-improvement-backlog](./modules/continuous-improvement-backlog.aps.md) | CIB   | In Progress | 46/77 (standing continuous-improvement intake; recent CIB-041..-046 release-reliability + flag-gating items Merged/Ready; CIB-054/-055 pre-tag v0.8.0 changelog-wording + release-plan-table reconcile Merged 2026-06-10 via PRs #2493/#2496; CIB-056 driver-client Windows pipe SID gate Merged via PR #2485 (#2484 increment 2 remains); CIB-057 capsule as-built doc + CIB-058 v0.8.0-window as-built delta refresh Merged 2026-06-10; CIB-059/-060 beta user-journey walkthrough follow-ups (quickstart ADR-080 ordering + auth-wall access pointer) In Progress 2026-06-11; CIB-061 remaining as-built delta refresh Merged via PR #2526; CIB-065 schema $id domain drift Merged 2026-06-11 via PR #2539; CIB-062..-064 golden-path polish gaps (auth-gate WARN JSON on stderr + install.sh docs URL + double-reported planted secret) Merged 2026-06-11 via PRs #2529/#2528/#2530; CIB-071 migrate user-facing diagnostics to miette Merged 2026-06-14 via PRs #2597/#2611; CIB-072 clear stuck `ready_restart_required` on Windows when daemon attestation is Unreachable (Proposed 2026-06-14, issue #2609); CIB-073..077 demo-readiness/release-flow gaps Draft 2026-06-14; per-item status in the module file) |
...
| [rust-cli-tier3](./modules/rust-cli-tier3.aps.md)                         | RCLI3   | In Progress | 5/20 (6 Ready, RCLI3-005 In Progress)                                                                             | RCLI; RCLI3-001 merged 2026-05-17 (PR #1664, `anvil edda list` Rust port). RCLI3-002 completed 2026-05-26 (`anvil edda show <id>` over the existing YAML store). Readiness audit 2026-05-17 promoted RCLI3-005/-008/-012/-014/-015/-017/-018 to Ready; RCLI3-005 (`anvil ember list`) In Progress 2026-06-17 (`feat/rcli3-005-ember-list`). Earlier 2026-05-17: RCLI3-017b merged (PR #1657); RCLI3-016b reconciled (RMCP-007 79da411d) |
