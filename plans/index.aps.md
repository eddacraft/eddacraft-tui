...
| [daemon-save-time-validation](./modules/daemon-save-time-validation.aps.md)     | DSV    | In Progress | 2/11 (filed 2026-06-03 — promotes the daemon save-time validation effort from the lone [MLP2-067](./modules/multilayer-protection-v2.aps.md) interim item + an execution plan into a tracked module. Sub-phase A (interim-cache `validate_paths`, 9 items) is authorised — GO-WITH-CONDITIONS per the 2026-06-02 holistic re-review ([verdict](./reviews/2026-06-02-b-corrections-holistic-verdict.md)); Sub-phase A′ (GV2 hot-read swap, 1) Blocked on the GV2 hot-/non-hot-path boundary gate; Sub-phase B (warm-start persistence, 1) Blocked on GV2-021. Action plan: `execution/2026-06-01-daemon-save-time-subphase-a.md` Tasks 0–17. Count hand-curated — the module header carries no `N/M` cell, so `index-counts.mjs` leaves it unmanaged.) |
