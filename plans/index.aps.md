...
| [release-orchestration](./modules/release-orchestration.aps.md)                 | RELORCH | Blocked | 3/12 (Paused 2026-05-11 until CICD-012 and OPMODEL-012 land. RELORCH-001 completed 2026-05-10 with `plans/specs/2026-05-10-release-orchestration-design.md`; RELORCH-003 completed 2026-05-10 with standalone `scripts/release/assess.sh` and command-specific harness tests; RELORCH-004 completed 2026-05-10 with standalone `scripts/release/preflight.sh` and command-specific harness tests. RELORCH-002 has the initial `scripts/release/_test/` harness and `.github/workflows/release-harness.yml`, but the broader fixture matrix is blocked until main-first validation and cutover land. Phase 1 nucleus remaining: -002 harness, -010 closeout, -011 wire-up + decommission. Phase 2: -005..-009 prepare/promote/tag/monitor/verify. RELORCH-012 added 2026-05-11 to close the release-record schema gap (yank state + policyDecisions enum) surfaced by OPMODEL-011 rollback playbooks. Successor to archived RELMGMT; supersedes parts of `2026-04-20-relmgmt-agent-driven-release-design.md` while inheriting its no-persistent-manifest tradeoff as a hard constraint. Tracks the work to honour PR #1368's `scripts/release/*.sh` target surface; design now precedes implementation — see Council session `plan-9a6b3a94` 2026-05-09.) |
...
| Module                                                                      | Scope | Status      | Progress           |
| --------------------------------------------------------------------------- | ----- | ----------- | ------------------ |
| [continuous-improvement-backlog](./modules/continuous-improvement-backlog.aps.md) | CIB   | In Progress | 0/0                |
| [codebase-maintenance](./archive/modules/codebase-maintenance.aps.md)       | MAINT | Complete    | 11/11 (1 deferred) |
...
| Module                                              | Scope | Status | Progress | Dependencies                                                                                                                                                                                                                |
| --------------------------------------------------- | ----- | ------ | -------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [usage-analytics](./modules/usage-analytics.aps.md) | USAGE | Draft  | 0/3      | Kindling, TRACE-001 (consumes `TraceContext`); coordinates with TRACE-004 (incoming `traceparent` binding), FLAGCAT-007 (resolved-snapshot shape + stable join key — gates USAGE-002), TRACE-003 (shared `SENSITIVE_FIELDS` deny-list), OBS-001 (post-launch). Privacy contract + OQ2 anonymisation (hash + per-deployment salt) confirmed 2026-05-11. |
...
| [feature-flag-catalogue](./modules/feature-flag-catalogue.aps.md)                                     | FLAGCAT    | 0/7        | FLAGS (complete), FLAGM (complete); coordinates with USAGE-002 via FLAGCAT-007 (resolved-snapshot shape + stable join key + ADR-019 scope, added 2026-05-11) — **Draft**                                                                                                                                                                                                                                                                                       |
...
| [operating-model-migration](./modules/operating-model-migration.aps.md)                               | OPMODEL    | 11/12      | Cross-cutting migration to the target Plan / Build / Release operating model — **In Progress**. OPMODEL-001 completed the current-state vs target-state migration map; OPMODEL-002 updated branching/worktree guidance; OPMODEL-003 aligned APS lifecycle and release metadata rules; OPMODEL-004 defined the release-record schema and authority contract; OPMODEL-005 defined release-readiness and candidate artefact workflow design; OPMODEL-006 aligned release runbook/skill migration boundaries; OPMODEL-007 added deterministic advisory agent guidance; OPMODEL-008 aligned review/council entrypoints and planning-council playbooks; OPMODEL-009 defined workflow session/event schemas; OPMODEL-010 added warning-mode APS/repo/release drift checks; OPMODEL-011 added rollback and incident playbooks for bad `main`, bad candidate artefacts, bad published releases, and emergency hotfixes. Remaining work covers main-first cutover. Implementation remains owned by specialist modules such as RELORCH and DOCGOV. |
| [ci-cd-validation](./modules/ci-cd-validation.aps.md)                                                 | CICD       | 8/12       | Specialist CI/CD and validation operating model implementation — **In Progress**. Coordinates with OPMODEL rather than replacing it; CICD-001 added CI cost/run-reason reporting via `pnpm ci:cost`; CICD-002 added the shared path/risk classifier via `pnpm ci:classify`; CICD-003 added local validation commands via `pnpm validate:*`; CICD-004 redesigned fast PR validation around classifier-selected checks without routine PR coverage or broad matrices; CICD-006 moved TypeScript and Rust coverage off `dev`-push integration runs onto the nightly assurance workflow; CICD-007 targeted Semgrep/Trivy/TruffleHog/license-check/cargo-deny/acknowledgements at classifier-selected signals plus a weekly scheduled assurance sweep; CICD-009 reconciled — release-readiness workflow shipped via PR #1398 with exact-SHA validation, required readiness checks, candidate metadata artefact, and no publishing credentials; CICD-012 added cutover-readiness — release-class gates use a head allowlist that survives the `dev` → `main` rename, `pr-base-guard.yml` is labelled migration-only, the PR template names both modes, and `scripts/ci/cutover-readiness.test.sh` locks the dual-mode invariants. Remaining work covers integration validation redesign, matrix targeting, drift checks, and workflow consolidation. |
