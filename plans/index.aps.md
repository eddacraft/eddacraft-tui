...
| N2 — Intercept Launcher v1 (INTL) | Merged | 9/9 | `anvil-run` wrapped-launch ingress. Crate `crates/anvil-run/` shipped via PR #1528 (merged 2026-05-14 at `5d38e546`) with INTL-001..-009 covered by 49 unit + 3 shell-integration tests. Module + all nine tasks moved **In Progress → Merged**. Cleanup agent advances **Merged → Released/Shipped → Complete/Archived** when release-record evidence from the `v0.7.0-beta` runbook lands. Two QoL follow-ups deferred to #1529 (foreground TTY passing + blocked-launch shell quoting). |
...
| [intercept-launcher](./modules/intercept-launcher.aps.md) | INTL | Merged | 9/9 | INTD; coordinates `AgentTag` proto with MLP-014; shipped via PR #1528 (merged 2026-05-14 at `5d38e546`) with `crates/anvil-run/` + 49 unit + 3 shell-integration tests green |
