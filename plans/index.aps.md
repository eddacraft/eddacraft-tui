...
to rank 2; its browser foundation does not block terminal usefulness.
...
| [graph-base-persistence](./modules/graph-base-persistence.aps.md) | GBASE | Done | 11/11 (created 2026-07-11 from planning council `plan-89a47ac7`, synthesised as [ADR-105](./decisions/105-shared-base-graph-persistence.md) — the ADR-069 storage-layout successor. Replaces the per-`WorktreeKey` snapshot with **one write-once, content-addressed base per repo per merge-base commit + live per-worktree overlays**: base read from the merge-base commit's committed tree via git objects (zero new deps) in a CLI subprocess, never the resident daemon; `SnapshotPayload` DTO + magic `ANVILGB1` reuse; disjoint base/overlay ids + a COMBINED-STATE golden parity fixture (GBASE-007, top schedule risk); `O_EXCL` single-flight production + refcount GC over ACTMO-registered worktrees (reclaims the new shared-base orphan class; per-worktree orphan race already closed by CIB-096; amends ADR-069 §5/§10); directory-level ref-watch triggers; re-entrant `persistence_route`; ADR-090 failure envelopes; the successor-specific graduation gate flips `ANVIL_PERSIST_GRAPH` default-on last (GBASE-010, terminal). Entry gate = the no-behaviour-diff `snapshot_io::store` extraction PR before GBASE-002; the old per-worktree path stays permanently for uncovered topologies. All 11 items Merged; module Done 2026-07-12 — graduation gate passed (plans/audits/2026-07-12-gbase-graduation-gate.md) and `ANVIL_PERSIST_GRAPH` flipped default-on.) | GV2, anvil-graph-cache, anvil-intercept, ADR-105, ADR-069, ADR-085, ADR-090, ADR-094, ADR-061/063/064/067 |
...
| Module                                                                      | Scope | Status      | Progress           |
| --------------------------------------------------------------------------- | ----- | ----------- | ------------------ |
| [continuous-improvement-backlog](./modules/continuous-improvement-backlog.aps.md) | CIB   | In Progress | 153/190 (standing continuous-improvement intake; recent CIB-099..105 GCTX/Windows/insights follow-ups recorded; CIB-106..122 Deepsec triage clusters filed from run `20260629190245-caf2a4b60b2715fe` across Windows pipe rooting/auth, workflow secret hygiene, sandbox/tool execution, TS path containment, provenance redaction, runtime/APS races, Edda/Kindling atomicity, infra secret/resource gating, release installer pinning, and residual P2 disposition; CIB-123 language-profile registry reconciled/Merged 2026-06-30 via PR #3011; CIB-124..126 witness lock timeout, cross-process append linearisation, and chain-init marker follow-ups filed from MLP2-005 phase-1 council; CIB-127 (Ready) tracks wiring the LAUNCH-010 activation finding-baseline into `anvil check` / `audit`, filed from the 2026-07-01 welcome/start user-journey review; CIB-128..132 filed 2026-07-02 from the clawpatch triage (`plans/reviews/2026-07-02-clawpatch-triage.md`) — anvil-intercept tracing-before-clap, anvil-rayon-init pool-cap test, Rust test-hardening batch, dogfood classifier path handling, admin-key SSL detection; CIB-162..179 filed 2026-07-04 from the anvil welcome/start user-journey pass (`plans/audits/2026-07-04-anvil-start-welcome-user-journey.md`) — activation output honesty and next-step arbitration, JSON log leak, workflow-picker consent, daemon stop verb, Windows recipe/PATHEXT gaps, welcome TUI navigation; CIB-183/184 filed from the ACTTUI first-run review for quiet repeat `start` output and unticked live MCP picker defaults; CIB-190 adds the JOURNEY healthy repeat-start value receipt; per-item status in the module file) |
...
| ----------------------------------------------------------------------- | -------- | ------ | -------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [release-user-journeys](./modules/release-user-journeys.aps.md)         | JOURNEY | In Progress | 6/10 | **Conductor** for repository-specific first-run advocacy and low-friction daily confidence. The release cut coordinates WOW-005, ACTTUI-009/-010/-012, CIB-073/-183/-184/-190, the DSV usefulness cut-line, three-platform candidate rehearsal, and an outcome-based final gate. Post-cut expansion retains WOW-006, richer ACTTUI diagnostics/celebration, always-on visibility, and DASH continuity without silently blocking the release. Design accepted by the operator 2026-07-11. **v0.9.0-beta shipped 2026-07-12** under JOURNEY-006 (record: [v0.9.0-beta](./releases/v0.9.0-beta.md); execution journal: [JOURNEY-006.actions](./execution/JOURNEY-006.actions.md)); JOURNEY-007..010 remain Proposed post-cut expansion.
...
| [first-run-wow](./modules/first-run-wow.aps.md)                         | WOW      | In Progress | 5/6 | WOW-001..004 are Merged: command-effect affordance, deterministic reveal, personalised finding counts, and completion delta. JOURNEY promotes WOW-005 (highest-value real finding, inspect-before-write first win) into the release cut after ACTTUI-009; WOW-006 sandbox autoplay remains coordinated post-cut expansion. |
| [activation-tui](./modules/activation-tui.aps.md)                     | ACTTUI   | In Progress | 6/13 | TUI-first `anvil start`. JOURNEY requires ACTTUI-009 (end-to-end consent), ACTTUI-010 (contract/PTY matrix), and ACTTUI-012 (default-path polish), plus WOW-005. ACTTUI celebration and richer diagnostic depth remain coordinated enhancements unless required to close a correctness defect. |
...
| [policy-value-enforcement-reset](./modules/policy-value-enforcement-reset.aps.md) | POLRESET | Done | **Conductor reset** for real policy value + opt-in save-time/pre-write enforcement — complete 10/10, closed 2026-07-05 with POLRESET-009 (ATC PR #3181, PATT PR #3175). Coordinates POLVAL, OPAE, CPOL, IORISK, EXCEPT, CPACKS, EVALCI, ATC/PATT, ACTAX, OPAG, and enterprise policy modules; first gate landed as ADR-098 (2026-07-04, council plan-18c47503) reconciling ADR 002, ADR 015, ADR 037, and ADR 040. |
...
| [policy-pack-validation](./modules/policy-pack-validation.aps.md)                 | POLVAL  | Done | POLENG/regorus, `crates/anvil-policy-engine` (pack admission home per ADR-098 retarget); POLVAL-001..005 Done — gate-preflight criterion resolved 2026-07-11 (install-time admission via OPAE-004 + gate compile fail-fast via ADR-098 PR-B)                                                                                  |
...
| [contextual-policy-assertions](./modules/contextual-policy-assertions.aps.md)     | CPOL    | Done | POLENG `PolicyInput` v1 + `crates/anvil-policy-engine` (`src/context/`, ADR-098 retarget); CPOL-001..003 delivered via POLRESET-004 / PR #3139; OPAG guidance is a downstream consumer                                                             |
| [io-risk-controls](./modules/io-risk-controls.aps.md)                             | IORISK  | Done | `crates/anvil-kernel-types`, `crates/anvil-policy-engine` (`src/io_risk/`, ADR-098 retarget), POLENG result semantics; IORISK-001..003 delivered via POLRESET-004 / PR #3139; concrete scanners = later intake                                                                         |
...
| [adversarial-testing-catalog](./modules/adversarial-testing-catalog.aps.md)       | ATC     | Done     | ATC-001..004 Merged 2026-07-05 via PR #3181; eval-harness-integration; OPAG guidance is downstream once promoted                                     |
| [prompt-attack-regression-packs](./modules/prompt-attack-regression-packs.aps.md) | PATT    | Done     | PATT-001..003 Merged 2026-07-04 via PR #3175; PATT-004 live-DefenceObserver follow-up filed Proposed 2026-07-11; adversarial-testing-catalog, eval-harness-integration                                                 |
| [eval-regression-ci-gate](./modules/eval-regression-ci-gate.aps.md)               | EVALCI  | In Progress | eval-harness-integration (EVAL, Done — items Merged via #3013); rust-tests.yml Test job (001-004 Merged via #3023; 005/006 Merged 2026-07-04 via #3170 — report-only CI live; 007/008 Proposed — ATC-003 satisfied via #3181, sole remaining decision gate is the CI-blocking-posture ADR)  |
...
| [agent-instruction-contract](./modules/agent-instruction-contract.aps.md)                              | AICON      | 5/5        | Lean root agent contract and authority-routing cleanup for `AGENTS.md`, testing guidance, repository operations, docs governance links, and tool-specific adapter thinness — **Done** 2026-07-07 on `docs/aicon-001-agent-contract`. |
...
| [mcp-client-expansion](./modules/mcp-client-expansion.aps.md) | MCPX | Proposed | 0/6 (new 2026-07-08 owner for expanding supported MCP clients beyond Claude Code and Cursor. Candidate matrix keeps already-supported Claude Code and Cursor as Tier 1 baselines, and evaluates Grok Build, Devin, Google Antigravity, OpenCode, VS Code/Copilot, Codex, Warp, Zed, Copilot CLI, Visual Studio, and generic project MCP; new support requires verified config contracts plus smoke evidence before `mcp-config`, `mcp install`, activation detection, and docs expand.) | RMCPF, RCLI3-016/-016b, ACTMO-012 |
