...
| [ci-cd-validation](./modules/ci-cd-validation.aps.md)                                                 | CICD       | 1/12       | Specialist CI/CD and validation operating model implementation — **In Progress**. Coordinates with OPMODEL rather than replacing it; CICD-001 added CI cost/run-reason reporting via `pnpm ci:cost` and a scheduled/manual summary workflow. Remaining work covers shared path/risk classification, local-first validation commands, trigger targeting, workflow decomposition, candidate-readiness CI touchpoints, and migration-safe validation cutover. |
