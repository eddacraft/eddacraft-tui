...
| [ci-cd-validation](./modules/ci-cd-validation.aps.md)                                                 | CICD       | 5/12       | Specialist CI/CD and validation operating model implementation — **In Progress**. Coordinates with OPMODEL rather than replacing it; CICD-001 added CI cost/run-reason reporting via `pnpm ci:cost`; CICD-002 added the shared path/risk classifier via `pnpm ci:classify`; CICD-003 added local validation commands via `pnpm validate:*`; CICD-004 redesigned fast PR validation around classifier-selected checks without routine PR coverage or broad matrices; CICD-006 moved TypeScript and Rust coverage off `dev`-push integration runs onto the nightly assurance workflow, eliminating routine coverage instrumentation and coverage-artefact uploads. Remaining work covers integration validation, candidate-readiness CI touchpoints, and migration-safe validation cutover. |
