...
| 2 | GCTX-001 assistant graph projection contract — the assistant-facing consumer surface | Merged | [graph-context-delivery](./modules/graph-context-delivery.aps.md) + [graph-v2-foundation](./modules/graph-v2-foundation.aps.md) | The assistant-facing substrate is built (ADR-083 + PV-9 landed 2026-06-15; GV2-023 #2621 + GV2-020 #2622). **GCTX Phase 0 complete: GCTX-001 Merged 2026-06-15 via #2628** (spec folds PV-9 CE-1..CE-12 onto the GV2-023 contract) and **GCTX-002 Merged 2026-06-15 via #2619** (ADR-083). GCTX module now In Progress 2/13. | **Phase 1 (GCTX-010 `anvil_search_symbols`) is the next GCTX unit but is design-gated**, not pick-up-ready: it must build the CE-5 sealed egress DTO + `GctxProjector` + structural no-leak test in Rust (net-new, no scaffolding exists) and resolve how `anvil mcp serve` obtains a graph handle (fresh construction vs daemon-RPC) — wants a plan/ADR before implementation. GV2-031 (rank 1) is the last internal GV2 item. |
