...
| ---- | --- | ---- | ------ | ------- | ----------- |
| 1 | DSV-049..051 — headless save-time driver (Sub-phase C tail) | Ready | [daemon-save-time-validation](./modules/daemon-save-time-validation.aps.md) | **v0.9.0-beta usefulness cut-line:** DSV-048 Merged via PR #3186 (CLI driver entry) and DSV-047 Merged via PR #3191 (daemon supervisor); the driver now spawns per durable worktree but is not yet observable. Remaining wave: DSV-049 (status wire) → DSV-050 (start copy; depends on 049's wire field) → DSV-051 (E2E/runbook + Windows leg). | Start DSV-049 (`save_time_driver` status wire) — consumes the supervisor's `driver_status`/`status_snapshot`; the new wire enum MUST ship `#[serde(other)] Unknown`. |
