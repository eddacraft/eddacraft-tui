...
| [user-journey](./modules/user-journey.aps.md)                           | UJ       | In Progress | 8/11 | Two beta golden paths — `anvil welcome` (discovery wow) and `anvil start` → watch/MCP (daily value) — made strong and self-guiding. Created 2026-06-10 from the v0.8.0-beta user-journey completeness review (operator-directed: beta posture permits explicit "run `anvil start` or `anvil welcome`" guidance; out-of-the-box usefulness ranks above tutorials). UJ-004 (auth-wall placement) + UJ-007 (daemon offer-to-start) design-gated Proposed; UJ-011 tutorials overhaul shapes last. Coordinates with CIB-047/-054/-055, INSIGHTS-005, DISTRIB-002, DSV-021/ADR-075. |
