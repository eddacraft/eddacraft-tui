...
> **Latest promoted release: `v0.9.0-beta`** (shipped 2026-07-12) — "First-Run
> Wins and the Assistant Graph": the assistant-facing graph over MCP (GCTX
> 14/14, ADR-083), warm-start persistence (ADR-069 plus the ADR-105 base+delta
> store, `ANVIL_PERSIST_GRAPH` default-on), the MCP-optional useful daemon
> (ACTMO plus the ADR-101 headless save-time driver), USAGE analytics, and the
> JOURNEY-conducted first-run/daily-confidence cut. Record:
...
| ---- | --- | ---- | ------ | ------- | ----------- |
| 1 | JOURNEY-007 — sandboxed autoplay demonstration | In flight | [release-user-journeys](./modules/release-user-journeys.aps.md) | Highest-value post-cut expansion in the active `v0.10.0-beta` window; its operator-approved design closed the WOW-006 execution gate. | Implementation is open as **PR #3441** (`feat/wow-006-sandbox-autoplay`), which also reconciles WOW-006. Review and land #3441, then flip JOURNEY-007. |
...
| [dev-acceleration-benchmarks](./modules/dev-acceleration-benchmarks.aps.md)     | DEVACC | Ready       | 0/12 (filed 2026-07-15; Ready wave 2026-07-16. Design: [`docs/architecture/dev-acceleration-benchmark-spec.md`](../docs/architecture/dev-acceleration-benchmark-spec.md). Task-level **Developer Acceleration** suite — token efficiency and success **with Anvil on vs off**. Distinct from RLB (process CPU/RSS) and GCTX-031 (payload micro). **Default cadence: on-demand.** DEVACC-001..006 **Ready** (catalogue + Tier A spine). DEVACC-007..010 Draft (Tier B / planning / claims). DEVACC-011/012 **Proposed opt-in** nightly/CI — not required for Complete.)                                                                                                |
...
| ----------------------------------------------------------------------- | -------- | ------ | -------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [release-user-journeys](./modules/release-user-journeys.aps.md)         | JOURNEY | In Progress | 7/10 | **Conductor** for repository-specific first-run advocacy and low-friction daily confidence. The release cut coordinates WOW-005, ACTTUI-009/-010/-012, CIB-073/-183/-184/-190, the DSV usefulness cut-line, three-platform candidate rehearsal, and an outcome-based final gate. Post-cut expansion retains WOW-006, richer ACTTUI diagnostics/celebration, always-on visibility, and DASH continuity without silently blocking the release. Design accepted by the operator 2026-07-11. **v0.9.0-beta shipped 2026-07-12** under JOURNEY-006 (record: [v0.9.0-beta](./releases/v0.9.0-beta.md); execution journal: [JOURNEY-006.actions](./execution/JOURNEY-006.actions.md)); JOURNEY-008 Merged 2026-07-25 via PR #3408; JOURNEY-007 Ready; JOURNEY-009 on hold and JOURNEY-010 blocked on the DASH view surfaces (2026-07-26).
...
| [activation-tui](./modules/activation-tui.aps.md)                     | ACTTUI   | Done | 14/14 | TUI-first `anvil start` — **all 14 items Merged-or-Done; module Done 2026-07-30**. JOURNEY's required gates ACTTUI-009 (end-to-end consent), ACTTUI-010 (contract/PTY matrix), ACTTUI-012 (default-path polish) and WOW-005 all Merged. ADR-103 Accepted 2026-07-25; **ACTTUI-013 Merged 2026-07-26 via PR #3411** delivered the Release 2 TTY-default flip — an interactive `anvil start` now enters the TUI with no flag, `--tui` is a hidden no-op and `ANVIL_ACTIVATION_TUI` is inert, with `--no-tui` / `ANVIL_NO_TUI=1` as the permanent escape hatches. ACTTUI-001..007 statuses reconciled 2026-07-30 (PRs #3234/#3236/#3237/#3238/#3254/#3256/#3257) — they had been left In Progress after merge. Celebration and richer diagnostic depth need a new item or module. Release evidence still owed before Released/Shipped. |
...
| [documentation-sync](./modules/documentation-sync.aps.md)                                             | DOCSYNC    | 16/22      | Public docs-site sync (`docs/public/anvil/`, `docs/public/kindling/`, `docs/public/aps/`) — **In Progress** (DOCSYNC-028 Merged 2026-07-20 via PR #3366. DOCSYNC-029 In Progress — APS first-time-user rebuild against anvil-plan-spec v0.6.0 with public-only language, complete intent-based navigation, and source-pinned command validation; plan: `plans/execution/DOCSYNC-029.actions.md`. Remaining Drafts: DOCSYNC-005 API reference, -011 Dashboard, -013 Multi-language, -016 VSCode/CI warning divergence; DOCSYNC-012 policy governance is In Progress.)                                                                                                                         |
...
| [mcp-2026-07-28-dual-era-support](./modules/mcp-2026-07-28-dual-era-support.aps.md) | MCP26 | In Progress | 0/12 (filed 2026-07-27 on `feat/mcp26-dual-era-support`. Dual-era stdio for modern MCP `2026-07-28` plus sealed legacy initialise-era clients. **MCP26-001 In Progress** (RC build authorised on branch). Dual-era host + discover + modern envelopes against RC via temporary typed adapter. **MCP26-012 Ready:** adopt stable official `rmcp` and remove temporary adapter after §8.2 gate. No merge to main until final schema seal + ADR Accept.) | RMCPF, MCPX, ACTMO; final MCP `2026-07-28`; ADR-113; `rmcp` v3 or typed adapter |
