...
| [daemon-save-time-validation](./modules/daemon-save-time-validation.aps.md)     | DSV    | In Progress | 18/20 (daemon save-time arc Released/Shipped via v0.8.0-beta, 2026-06-11; Sub-phase A 9/9 + A-W 2/2 Merged, deferred follow-ups Done, A′ 2/2 Done — GV2 backing swap reconciled from #2446 and DSV-021 #2473 flipped `ANVIL_WATCH_DAEMON` default-on with live-daemon guard + opt-out/force. **DSV-045 (full-scan executor) In Progress (added 2026-06-16) per [ADR-085](./decisions/085-daemon-full-scan-executor.md)** (planning council `plan-898d9222`) — drives `request_full_scan` `Pending→Running→Clean` and populates the warm cache without a save, closing the ADR-084 C1 cold-start gap; new `AssuranceState::Bounded` wire variant (+`#[serde(other)]` fallback); merges before the GCTX-010 warm-up triggers. Sub-phase B warm-start persistence remains Blocked; sub-phase + PR detail in the module file) |
