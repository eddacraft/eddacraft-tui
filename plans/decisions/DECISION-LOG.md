...
| [059](059-production-tracing-sink.md) | Production tracing sink: OTLP-neutral instrumentation → managed OTLP-native backend (recommended Honeycomb; fallback Grafana Cloud), exporting **operator-hosted surfaces only** (`apps/anvil-api` via the Vercel OTEL drain) while the local-first Rust CLI/daemon stay formatter + local-file (never auto-export); redaction-wrapped (TRACE-003), config-gated off by default. Resolves EXPORT OQ1; vendor selection is the one founder-ratification point | Proposed |
