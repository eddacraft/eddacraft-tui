...
| [052](052-automated-drift-snapshots.md) | Automate drift-snapshot capture via a scheduled CI workflow that opens a weekly auto-merge PR adding `anvil drift snapshot` output to `.anvil/snapshots/` (26-week retention, no trunk bypass); the committed series is the canonical team drift time-series consumed by `anvil drift report` + INSIGHTS-003 — not a per-machine daemon/hook series | Proposed |
