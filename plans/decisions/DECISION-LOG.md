...
| [071](071-ast-aware-antipattern-detection.md) | Enables the AST-dependent half of RSTLAN-003 (Rust T2 catalogue). The `anvil-checks` scanner is **regex + same-line post-filter only**, so cfg(test)-aware `unwrap`/`expect`/`panic`, SAFETY-comment-aware `unsafe`, and serde hygiene are infeasible (regex FPs on inline `#[cfg(test)]` would fail the §16.5 #9 bar). The daemon links `anvil-checks` on the save-time hot path and ADR-064 forbids tree-sitter in the daemon; Cargo workspace **feature unification** means a feature flag on `anvil-checks` would still drag tree-sitter into the daemon. Decision: AST detection lives in a **new crate `eddacraft-anvil-checks-ast`** (deps `anvil-checks` types + `anvil-kernel` grammar + `tree-sitter`) that the daemon does **not** link, and AST rules run **gate-time only** (`anvil check`/`gate`) — the same hot/non-hot split as ADR-061, so save-time stays regex-only + parser-free and `coverage: certified` keeps attesting only the regex antipattern family. The dormant `Detection::Ast { ast_query }` variant (already in the loader + TS schema) becomes live, consumed only by the AST scanner; rule = tree-sitter query + Rust predicate table (cfg(test) ancestor-exclude, preceding-SAFETY, serde-derive-without-deny_unknown_fields), suppression on node start-line. Mechanism is language-general (reusable by the Python anchor). A new `daemon_dep_boundary`-style guard asserts `anvil-checks-ast`/tree-sitter stays out of the daemon closure. Rejected: `ast` feature on `anvil-checks` (feature unification breaks ADR-064); parse-then-feed tree to the daemon for save-time AST (adds parse to hot path, widens attestation); regex-only with allowlists (FP bar + serde impossible); AST inside `anvil-kernel` (layering inversion) | Proposed |
