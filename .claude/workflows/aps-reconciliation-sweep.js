...
6. kind 'count-narrative-mismatch': do NOT trust the numbers quoted in the finding — establish ground truth yourself. The DERIVED progress count comes from work-item headings, which the gate matches as \`#{3,4}\` (BOTH \`###\` and \`####\`) — READ \`scripts/aps/lib/modules.mjs\` to confirm exactly what it counts before asserting anything about gate behaviour. Then read the module's own header line and its \`plans/index.aps.md\` row, and re-derive the count. If the baseline above reports 0 count drifts, the header and index ALREADY agree with the derived count, so any remaining COUNT mismatch lives ONLY in NARRATIVE PROSE (running tallies in the body, slate/summary descriptions) — which the deterministic gates do not validate. Tag it as prose drift and point the fix at the prose. Real only if a quoted ground-truth number is genuinely wrong.
7. kind 'status-body-mismatch': verify by reading the actual statuses, NOT the count baseline — a status contradiction can be real with zero count drift, so do not downgrade it just because counts are in sync. Read the module/header Status line and the work-item or body statuses it supposedly contradicts; real only if they genuinely disagree AND no audit-note/rescope block (step 1) explains it.
