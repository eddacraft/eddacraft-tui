...
1. **Input Validation** (PRIMARY RISK)
   - All external formats parsed from user files
   - Zod schema validation is first defense
   - Risk: Malicious SpecKit/BMAD files

```typescript
// ✅ Good: Validate before processing
const validated = apsSchema.parse(untrustedInput);
...
3. **Dependency Security**
   - Check: `pnpm audit`
   - Risk: Vulnerable deps (especially Zod, Vitest, Playwright)
   - Action: Regular updates via Renovate/Dependabot

4. **Secrets in Evidence**
   - Gate checks may capture logs/outputs
   - Risk: Secrets in evidence bundles
   - Mitigation: Secret scanning gate check ✅

5. **Code Execution**
   - Gate checks execute external tools (ESLint, pytest, etc.)
   - Risk: Command injection
   - Mitigation: Use spawn with args array, not shell strings

```typescript
// ✅ Good: Spawn with args
spawn('eslint', ['--format', 'json', file]);

// ❌ Bad: Shell execution
exec(`eslint --format json ${file}`);
```

**No Authentication/Authorization** (CLI tool, local execution)

**Security Checklist for Anvil**:

- [ ] Zod validation on all external inputs
- [ ] Path validation for file operations
- [ ] No shell string execution (use spawn)
- [ ] Secret scanning in gate checks
- [ ] Regular `pnpm audit` in CI
- [ ] No hardcoded credentials (N/A for current scope)

**Audit Commands**:

```bash
pnpm audit                              # Check dependencies
grep -r "exec\|eval" packages/          # Find risky execution
grep -r "fs\." packages/ | grep -v test # File system usage
```

**Current Risk Level**: 🟢 LOW (CLI tool, local execution, no network)

**Future Risks** (Act II):

- GitHub Action integration → GitHub token handling
- Policy engine → Rego code execution risks
- Multi-repo → Credential management
