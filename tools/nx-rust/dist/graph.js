...
function computeCached(workspaceRoot) {
  let currentMtime = 0;
  try {
    currentMtime = (0, node_fs_1.statSync)(
      (0, node_path_1.join)(workspaceRoot, 'Cargo.lock')
    ).mtimeMs;
  } catch {
...
exports.createNodesV2 = [
  ...
  async (configFilePaths, options, context) => {
    ...
    return (0, devkit_1.createNodesFromFiles)(
      async (configFile) => {
...
function computeGraph(workspaceRoot) {
  ...
  for (const pkg of metadata.packages) {
    if ((0, cargo_1.isExternal)(pkg, workspaceRoot)) continue;
    const root = (0, devkit_1.normalizePath)(
      (0, node_path_1.dirname)((0, node_path_1.relative)(workspaceRoot, pkg.manifest_path))
    );
    ...
    for (const dep of pkg.dependencies) {
      if (dep.kind === 'dev') continue;
      if (!(0, cargo_1.isExternal)(dep, workspaceRoot)) continue;
