//! MLP2-009: shared sliding-window rate primitive.
//!
//! Caps the rate at which an emitter publishes events so a burst
//! (keystroke storm, watcher debounce flurry, fence-cascade
//! retry loop) cannot flood the receiver. The primitive is
//! deliberately small and standalone so multiple consumers share
//! it without coupling to each other:
//!
//! - MLP2-006: gate-evaluated Kindling observations.
//! - MLP2-026: `degraded:fence-cascade` mode at 5 fences in 60 s
//!   (the same primitive, different cap + window).
//! - MLP2-059: per-worktree invalidation rate limit (Council
//!   follow-on; planned).
//!
//! ## Surface
//!
//! - [`RateWindow::new(capacity, window)`] builds a window-sliding
//!   counter that allows up to `capacity` events per `window`
//!   duration.
//! - [`RateWindow::record(now)`] is the hot-path call. Returns
//!   [`RateDecision::Allow`] when the caller should emit the event
//!   normally, or [`RateDecision::Throttle { drops }`] when the
//!   event is dropped — `drops` is the **running total of drops**
//!   since the previous `Allow` (so the next `Allow` carries the
//!   drop count for the throttle marker the spec calls for).
//!
//! ## Semantics
//!
//! Sliding window, not fixed bucket. Each `record(now)` first
//! evicts every recorded timestamp older than `now - window`,
//! then either records the new timestamp and returns `Allow`, or
//! refuses and returns `Throttle { drops: N }`. Eviction happens
//! lazily on access — there is no background tick.
//!
//! The "running total of drops" semantics ensure the consumer can
//! emit exactly **one** `degraded:observation-throttled` row per
//! sustained burst, carrying the cumulative drop count. The drop
//! counter resets to zero on the next `Allow`, so subsequent
//! bursts emit their own markers.
//!
//! ## Out of scope
//!
//! - No per-emitter scoping in this layer — callers wrap one
//!   `RateWindow` per emit point (or per (worktree, emitter)
//!   pair for MLP2-026 / MLP2-059).
//! - No telemetry emission. The consumer wires the
//!   `degraded:observation-throttled` envelope; this module is
//!   purely the rate-tracking primitive.
//! - No async surface. The primitive is sync; consumers running
//!   in tokio call it from a worker thread or `spawn_blocking`
//!   when needed.
