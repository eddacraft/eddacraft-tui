...
// ============================================================================
// GCTX-provider `symbol_at` — position-to-symbol resolution (ADR-084)
// ============================================================================

/// Query for the LSP-provider `symbol_at` verb: a file plus a byte offset into
/// it, resolved to the innermost resident [`SymbolIdentity`] whose defining
/// span contains that offset. Identity-only and graph-free.
///
/// Feeds the `textDocument/references` flow: `anvil lsp` converts an LSP
/// (line, character) position to a byte offset client-side (it already holds
/// the open document's text), calls `symbol_at` to resolve the enclosing
/// symbol, then calls `anvil_find_callers` (GCTX-014) with that identity —
/// this verb only answers "what symbol is here," not "who calls it."
#[derive(Debug, Clone, Default, PartialEq, Eq, Serialize, Deserialize)]
pub struct SymbolAtQuery {
    /// Workspace-relative file the offset is into. Required in practice;
    /// `Option` so a bare `{}` deserialises (rejected daemon-side with a
    /// structured `InvalidQuery`).
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub file: Option<String>,
    /// Byte offset into `file`'s source text (UTF-8 bytes, half-open span
    /// semantics matching the resident symbol spans — an end-exclusive
    /// offset resolves to whatever starts there, never the span it closes).
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub byte_offset: Option<u32>,
}

/// The identity-only projection returned when the symbol graph is readable.
/// At most one symbol — this is a point lookup, not a traversal, so unlike
/// [`FindCallersProjection`] there is no pagination.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct SymbolAtProjection {
    /// The innermost symbol containing the queried offset, or `None` when no
    /// resident symbol's span covers it (e.g. the offset is in whitespace, a
    /// comment, or a symbol with no recorded span).
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub symbol: Option<SymbolIdentity>,
    /// Counts-only elision summary (CE-11) — `matched`/`returned` are `0` or
    /// `1` for this point lookup; `omitted_sensitive_paths` is `1` only when a
    /// resolved symbol's file was withheld by the CE-3 deny-list (in which
    /// case `symbol` is `None`, matching [`FindCallersProjection`]'s posture).
    pub redaction_summary: RedactionSummary,
}

/// The status-tagged outcome of a `symbol_at` lookup. Same named degradation
/// surface as [`FindCallersOutcome`] (ADR-084 CE-7).
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(tag = "status", rename_all = "snake_case")]
pub enum SymbolAtOutcome {
    /// Graph readable: `symbol` is `Some` on a hit, `None` on a clean miss —
    /// both are `Ready`, not a degradation.
    Ready(SymbolAtProjection),
    /// Graph not yet readable (warming, or cold and not yet save-populated).
    NotReady {
        /// Human-readable, enum-stable recovery guidance.
        recovery_hint: String,
...
impl SymbolAtOutcome {
    /// Classify into the shared PII-free [`GctxOutcome`] telemetry enum
    /// (CE-10). A readable result splits into `Hit` (`symbol.is_some()`) and
    /// `Miss` (a clean lookup that resolved nothing).
    #[must_use]
    pub fn telemetry_outcome(&self) -> GctxOutcome {
        match self {
            Self::Ready(projection) if projection.symbol.is_some() => GctxOutcome::Hit,
            Self::Ready(_) => GctxOutcome::Miss,
            Self::NotReady { .. } => GctxOutcome::Warming,
            Self::Unavailable => GctxOutcome::Unavailable,
            Self::Disabled => GctxOutcome::GraphDisabled,
            Self::InvalidQuery { .. } => GctxOutcome::InvalidQuery,
