//! MLP2-019: L4 verification of witness `rules_sha` against
//! recognised versions.
//!
//! The L4 server-side validation lane sees one `rules_sha` per witness
//! line (the digest of the rule set that produced the witness — see
//! `anvil_rules::rules_sha`). The lane must answer "do I recognise
//! this digest?" before it trusts the witness's claims about
//! `anvil_version`, `opa_runtime_version`, or the rule list — because
//! a `rules_sha` the server has never published cannot have come from
//! a release the server understands.
//!
//! ## Scope
//!
//! - [`RuleSetMetadata`] — the bundle of facts the server knows about
//!   one recognised `rules_sha` (anvil version, OPA runtime version,
//!   sorted rule ids, `config_sha`, ISO-8601 recognised-at timestamp).
//! - [`RecognisedRulesRegistry`] — `HashMap<rules_sha, RuleSetMetadata>`
//!   wrapper with `O(1)` lookup. Insert-deterministic — duplicate
//!   inserts of the same `rules_sha` with conflicting metadata return
//!   [`RegistryError::Conflict`] so a misbehaving loader surfaces the
//!   bug at registry-build time rather than silently shadowing.
//! - [`evaluate_rules_sha`] — given a registry, a witness's optional
//!   `rules_sha`, and the branch rule's `OnNoWitness` knob, produce a
//!   [`RulesShaOutcome`] the L4 caller renders into the final verdict.
//!
//! ## Out of scope
//!
//! - Distribution mechanism for the registry (rule-pack feed, GitHub
//!   release artefact, etc.) — that's MLP2-046 / MLP2-055 follow-up.
//! - Wiring this into `validate_at_l4`'s engine — that's the L4 CLI
//!   lane's job; this library just provides the primitive.
//! - Persisting the registry to disk — the v1 API constructs from an
//!   in-memory list of entries; serialisation can ride on serde later.
//!
//! ## Routing semantics
//!
//! `evaluate_rules_sha` mirrors the well-known [`OnNoWitness`] routing
//! pattern so the L4 caller has only one routing vocabulary:
//!
//! | Witness `rules_sha`    | Registry            | `OnNoWitness`    | Outcome                       |
//! | ---------------------- | ------------------- | ---------------- | ----------------------------- |
//! | absent                 | —                   | —                | [`RulesShaOutcome::Absent`]   |
//! | present + recognised   | hit                 | —                | [`RulesShaOutcome::Recognised`] |
//! | present + unrecognised | miss                | `Allow`          | [`RulesShaOutcome::AdmitUnrecognised`] |
//! | present + unrecognised | miss                | `ValidateAtL4`   | [`RulesShaOutcome::NeedsRevalidation`] |
//! | present + unrecognised | miss                | `Reject`         | [`RulesShaOutcome::Block`]    |
//!
//! The `Absent` case is its own variant so the L4 caller can compose
//! it with [`crate::CommitDecision`] (decided from `has_witness`)
//! without losing the "no `rules_sha` to check" signal — admitting it
//! silently would hide degraded-witness data; surfacing it lets the
//! caller emit a `degraded:rules-sha-absent` diagnostic if the branch
