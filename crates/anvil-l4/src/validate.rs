//! MLP2-016: server-side `validate_at_l4` pipeline.
//!
//! When pre-push policy resolution returns
//! [`CommitDecision::NeedsL4Validation`](crate::CommitDecision::NeedsL4Validation),
//! the operator's commit lacks an L3 witness but the branch's
//! [`OnNoWitness`](crate::OnNoWitness) is `validate_at_l4` — so the
//! server-side rule engine must run against the commit and produce a
//! real [`ValidationVerdict::Allow`] / [`ValidationVerdict::Block`].
//!
//! Before MLP2-016 the pre-push hook short-circuited every
//! `NeedsL4Validation` decision with an inline
//! [`Verdict::InternalError { class: TimedOut }`][anvil_hook_internal]
//! emit and admitted the push. This module replaces that inline
//! fall-through with a typed pipeline: a [`ValidationEngine`] trait
//! takes the [`ValidationRequest`] and produces a
//! [`ValidationVerdict`]. The hook dispatches through this trait so:
//!
//! 1. **`Allow`** — real engine ran the rules and the commit passed.
//!    The hook proceeds without any emit; the push is admitted.
//! 2. **`Block { diagnostics }`** — real engine ran the rules and the
//!    commit failed. The hook surfaces the per-rule diagnostics and
//!    refuses the push.
//! 3. **`EngineUnavailable { reason }`** — the engine could not
//!    execute (binary missing, rule pack not loaded, future
//!    integration not landed yet). The hook preserves the
//!    pre-MLP2-016 surface: emit
//!    [`Verdict::InternalError { class: TimedOut }`][anvil_hook_internal]
//!    once via the suppression log and admit the push (ADR-038
//!    "internal failures never block the user").
//!
//! The default [`NoOpValidationEngine`] returns `EngineUnavailable {
//! reason: NotImplemented }` and stays bound at the hook call site
//! until a follow-up wires the real `anvil-checks` rule pipeline
//! through. Test code substitutes a `BlockingEngine` / `AllowingEngine`
//! fixture to drive the Allow / Block paths through the production
//! call site.
//!
//! ## Why a trait rather than a free function
//!
//! - The rule engine is the long-running follow-up; trait dispatch
//!   means the hook code is finished today and gets the real engine
//!   for free once a downstream PR lands a real
//!   [`ValidationEngine`] impl.
//! - Trait dispatch makes the production call site driveable from
//!   unit tests with a `BlockingEngine` mock — no environment-variable
//!   feature flags, no `#[cfg(test)]` branches in `commands/hook.rs`.
//! - The free-function alternative either bakes the engine into the
//!   library (forces a heavy dep at compile time) or carries a
//!   `Box<dyn FnOnce>` everywhere (less ergonomic, same outcome).
//!
//! [anvil_hook_internal]: https://docs.rs/anvil-hook
