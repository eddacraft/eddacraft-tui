...
/// KDS-004: under `ANVIL_KINDLING_SINK=daemon` with no daemon reachable, the
...
fn daemon_sink_degrades_to_sidecar_when_daemon_unreachable() {
    ...
    let mut cmd = Command::new(ANVIL_BIN);
    cmd.args(["kindling", "usage", "top", "--json"])
        ...
        .env("ANVIL_DEV", "1")
        .env("ANVIL_SKIP_WELCOME", "1")
        .env("ANVIL_KINDLING_SINK", "daemon");
    cmd.env_remove("ANVIL_TOUCH_PROJECT_STATE");
    cmd.env_remove("TRACEPARENT");
    ...
    // stdout stays clean JSON and still carries the sidecar rows.
    let stdout = String::from_utf8_lossy(&out.stdout);
    let parsed: serde_json::Value =
        serde_json::from_str(&stdout).expect("valid JSON (clean stdout)");
    assert!(
        parsed
            .as_array()
            .expect("top is an array")
            .iter()
            .any(|e| e["command"] == "check"),
        "sidecar rows survive under the daemon sink: {stdout}",
    );

    // The daemon-unreadable degrade note goes to stderr (never corrupting stdout).
    let stderr = String::from_utf8_lossy(&out.stderr);
    assert!(
        stderr.contains("could not be read"),
        "expected a daemon-unreadable degrade note on stderr; got:\n{stderr}",
    );
}
