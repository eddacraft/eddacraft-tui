...
fn discover_policy_files(dir: &Path) -> Vec<PathBuf> {
    let mut files: Vec<PathBuf> = walkdir::WalkDir::new(dir)
        ...
        .filter(|e| !e.path().components().any(|c| c.as_os_str() == ".generated"))
...
/// A source file is generated when it carries the Anvil auto-generation header.
/// The `.generated/` path segment is handled at discovery; this covers
/// generated files written outside that directory (legacy loader parity).
fn is_generated_source(content: &str) -> bool {
    content.contains("# Auto-generated by Anvil")
}

/// Resolve a policy-supplied severity override to the gate's vocabulary.
///
/// Recognised values (`error`/`err`, `warning`/`warn`, `info`) are honoured.
/// Any unrecognised string fails closed to the rule's own default severity
/// (`violation`/`deny` → error, `warn` → warning) rather than being accepted
/// verbatim — since severity now controls pass/fail, a typo must not silently
/// land a violation in the non-blocking bucket.
...
fn admit_policy_files(
    ...
    for path in policy_files {
        ...
        // Resource caps (mirroring commands/policy/eval.rs): reject an oversized
        // file before it lands in memory, and cap the whole bundle.
        ...
        // run_check_architecture already evaluates — skip, don't double-count.
...
fn run_check_policy(
    ...
    // order. An empty bundle has nothing to evaluate — a clean skip, as today.
    let policy_files = discover_policy_files(&canonical_dir);
