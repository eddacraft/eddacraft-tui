...
/// First-run orientation shown ahead of the root command's help body.
///
/// ADR-114: bare `anvil` (no subcommand) is the daily ensure surface. Root
/// `--help` still lists every command; this banner names the three entry
/// verbs: bare ensure, `start` (activate/reconfigure), and `welcome` (tour).
/// Kept free of internal identifiers so the CLIC-010 help lints stay green.
pub const FIRST_RUN_POINTER: &str = "Run `anvil` to turn protection on (daemon + existing MCP). \
New here? `anvil welcome` for a tour, or `anvil start` to activate or reconfigure this repository.";
