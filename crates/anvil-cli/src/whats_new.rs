...
//! (the same neighbourhood as the detected-agents cache). When project
...
//! repeat on every run and break the exactly-once contract.
...
/// the first marked run; honours the opt-out env var and gated project
/// writes.
...
fn post_upgrade_hint_inner(
    ...
    // Persist on seed and on change. Best-effort: an unwritable marker means
    // the hint may repeat, which is preferable to suppressing it forever.
