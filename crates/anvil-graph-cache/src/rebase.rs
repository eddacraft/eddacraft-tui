//! GBASE-005 (ADR-105 §3): **disjoint id allocation + compose-time
//! cross-boundary re-resolution** — the id/parity contract that lets a shared
//! base graph and a worktree overlay be composed into a graph *identical to a
//! cold scan of the combined on-disk state* (the GBASE-007 correctness anchor).
//!
//! This module owns the **plan**, not the composition. It rebases an
//! [`OverlayFragment`](crate::overlay::OverlayFragment)'s raw parser ids into a
//! range disjoint from the base, and it derives the cross-boundary edge work
//! composition must do so that no edge is ever trusted by a **stale base id**.
//! Materialising the resident per-worktree graph (loading the base, applying the
//! plan, staleness, health envelopes, daemon wiring) is **GBASE-006**; this
//! module stops at a typed [`ComposePlan`] that GBASE-006 consumes.
//!
//! # The id-space contract (ADR-105 §3)
//!
//! ```text
//!        base ids                     overlay ids
//!   ┌──────────────────┐        ┌────────────────────────┐
...
//! ```
//!
//! - **The base owns `[0, base_next_id)`.** `base_next_id` is *recoverable
//!   directly from the persisted payload*: [`SnapshotPayload`](crate::SnapshotPayload)
//!   serialises `next_id` (one past the largest id ever inserted) and
//!   [`SnapshotPayload::into_graphs`](crate::SnapshotPayload) restores it through
//!   [`SymbolGraph::set_next_id_floor`](crate::SymbolGraph::set_next_id_floor),
//!   so a freshly-loaded base's [`SymbolGraph::next_id`](crate::SymbolGraph::next_id)
//!   **is** the watermark. No derivation and no payload change is required — the
//!   golden wire bytes are untouched (see [`OverlayIdAllocator::new`]).
//! - **The overlay allocates `[watermark, ..)`.** [`OverlayIdAllocator`] rebases
//!   every overlay symbol id above the watermark, deterministically, so the two
//!   id spaces are **disjoint by construction**. This is the invariant the
//!   schema-epoch clause (ADR-105 §9) protects: a mixed-epoch base + overlay
//!   would break it, so it is forbidden by the shared version policy, not patched
//!   here.
//!
//! Because an overlay [`FileSymbols`] references its own symbols only through
//! `SymbolNode.id` (its `imports` / `reexports` / `calls` name their targets by
//! **specifier / export-name**, never by id — see [`anvil_kernel_types::ImportEdge`]
//! and [`anvil_kernel_types::CalleeRef`]), rebasing is *complete* once each
//! `SymbolNode.id` is reassigned: there are no intra-fragment id cross-references
//! to rewrite. Cross-boundary edges are then re-resolved from **specifiers**, not
//! ids, at compose time — which is exactly why a stale base id can never leak
//! into the composed graph.
//!
//! # Cross-boundary re-resolution (ADR-105 §3 — "never trusted by stale id")
//!
//! Two directions cross the base↔overlay boundary; each is resolved from raw
//! specifiers, never from a persisted resolved id:
//!
//! - **overlay → base** (an overlay file imports a base-only file). Handled by
//!   composition applying each rebased upsert through the ordinary
//!   [`update_file`](crate::update_file) lift: the overlay file's raw import
//!   specifier resolves against the *composed* file set (base ∪ overlay), binding
//!   to the base symbol's id. No plan entry is needed — it falls out of the
//!   upsert. (The mini-parity test in this module exercises it end-to-end.)
//! - **base → overlay** (a *surviving* base file imports a file the overlay
//!   **tombstoned + re-added**, i.e. a modified file). This is the load-bearing
//!   case: the base's persisted edge pointed at the modified file's **old** base
//!   id, which the tombstone removes. `update_file` cannot restore it — it only
//!   lifts the *updated* file's own out-edges, and the surviving base file's raw
//!   import specifier is not re-parsed. So [`plan_compose`] emits a
//!   [`BaseReresolve`] directive for each such edge, derived from the **persisted
//!   file-dependency forward map** ([`DependencyGraph`], the Imports-only
//!   `(source → target)` relation ADR-069 §1 already stores), following the
//!   `dependents_of` locality pattern (ADR-086). Composition re-binds the edge to
//!   the file's **new** overlay symbol, looked up live — never the stale base id.
//!
//! # Scope of the re-resolution contract (imports only — a documented boundary)
//!
//! ADR-105 §3 scopes cross-boundary re-resolution to **imports**, and the
//! persisted forward map ([`DependencyGraph`]) is **Imports-only, cross-file** by
//! construction (see `anvil_cli::graph_base_producer::derive_dependency_graph`
//! and `anvil_intercept::kernel_cache::refresh_file_dependencies`, kept in
//! lockstep). Consequently:
//!
//! - **Overlay → base** imports, re-exports, and calls all re-resolve correctly,
//!   because the overlay file *is* re-parsed and carries its raw specifiers /
//!   call-sites, which `update_file` lifts against the composed file set.
//! - **Base → overlay re-exports and calls are NOT reconstructable** from the
//!   persisted state: the base persists *resolved* edges, not the surviving base
//!   file's raw `ReexportEdge` / `CallSite`, and the dependency forward map does
//!   not carry them. A surviving base file that re-exports from, or calls into, a
//!   modified overlay file would therefore lose that specific edge after
//!   composition. This is a **known limitation of the persisted format**, flagged
//!   for the **GBASE-007** polyglot parity fixture: closing it requires either
//!   persisting the base's raw specifiers/call-sites (a schema-additive payload
//!   change under the epoch/golden discipline) or widening the forward map — both
//!   deferred. The import contract this module implements is exactly what ADR-105
