...
describe('option coercion utilities', () => {
  describe('coercePositiveInt', () => {
    ...
    it('rejects zero', () => {
      expect(() => coercePositiveInt('0', '--limit')).toThrow(CliError);
      expect(() => coercePositiveInt('0', '--limit')).toThrow('--limit must be a positive integer');
    });

    it('rejects negative values', () => {
      expect(() => coercePositiveInt('-5', '--limit')).toThrow(CliError);
    });

    it('rejects non-numeric values', () => {
      expect(() => coercePositiveInt('abc', '--limit')).toThrow(CliError);
    });
  });
  ...
  describe('coerceNonNegativeInt', () => {
    ...
    it('rejects negative values', () => {
      expect(() => coerceNonNegativeInt('-1', '--refresh')).toThrow(CliError);
    ...
    it('rejects non-numeric values', () => {
      expect(() => coerceNonNegativeInt('not-a-number', '--refresh')).toThrow(CliError);
    });
  });
  ...
  describe('coercePort', () => {
    ...
    it('rejects zero and negative values', () => {
      expect(() => coercePort('0', '--port')).toThrow(CliError);
      expect(() => coercePort('-1', '--port')).toThrow(CliError);
    });

    it('rejects values above the upper boundary', () => {
      expect(() => coercePort('65536', '--port')).toThrow(CliError);
    });

    it('rejects non-numeric values with a CliError', () => {
      try {
        coercePort('NaN', '--port');
