...
| `cli/`       | **CLI**         | Real `anvil` binary via child_process                             |
...
- **Reuses existing utilities** — patterns from `test-workspace.ts`,
  `tuistory-utils.ts`
- **Process-based CLI testing** — spawns real binary via
  `child_process.execFile`
- **No live database** — API tests use Hono's `app.request()` with DB mocks
- **Isolated workspaces** — each test gets a temp directory, cleaned up
  automatically
