...
Each session persists under `.claude/council/sessions/<session-id>/`:

| File              | Contents                                                      |
| ----------------- | ------------------------------------------------------------- |
| `manifest.json`   | Target, base ref, branch, phase, timestamps                   |
| `findings.ndjson` | Structured findings with ID, severity, status, file, evidence |
| `events.ndjson`   | Review, triage, fix, and publish events                       |
| `summary.md`      | Rendered summary (generated by publish)                       |
...
Before starting analysis, check for pending messages:

```bash
./receive-messages.sh <your-agent-name> --format summary
```

After completing analysis, send significant findings to relevant agents:

```bash
./send-message.sh --from <your-agent> --to <recipient> --type finding --priority high \
  --payload '{"file":"path/to/file","issue":"description","severity":"high","suggestion":"fix"}'
```
