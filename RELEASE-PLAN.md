...
**Scope extended 2026-06-10 (operator).** The A′ slice and default-on routing
have landed; the **user-journey module
([UJ](./plans/modules/user-journey.aps.md)) is required before the cut** so the
release that turns daemon routing default-on also guides every installer and
upgrader to that daemon-backed protection (both beta golden paths —
`anvil welcome` and `anvil start` — strong and self-guiding).
...
| **GV2 A′-critical-path foundation** (ADR-075) | The GV2-027 dependency closure: GV2-010 (semantic schema), 011 (incremental hot indexes), 012 (trust/policy contract), 022 (hot-read API + guardrails), 024 (hot-read type split), 025 (ADR-031 latency CI gate), 028 (production parser feed), 029 (privilege containment).                                                     | **Merged** — the full closure landed on the #2442/#2446 swap wave (GV2-022/027/028/029) plus GV2-024 (hot-read type split + seal + ADR-077 depth cap, #2470) and GV2-025 (ADR-031 p95 latency CI gate — a `harness = false` exit-code bench, #2459). GV2 13/20; the open balance is non-critical-path (v0.9 deferrals + Sub-phase B). |
| **A→A′ backing swap** (ADR-075)               | GV2-027 retires the interim re-derive; `validate_paths`/`save_time` read the resident GV2 hot-index under the **unchanged wire**; `backing_schema_version` → `gv2-hotindex-v1`.                                                                                                                                                  | **Merged** — GV2-027 landed on the #2446 wave; `validate_paths` certifies through the resident GV2 hot-read index with the verdict-parity proof green, and the GV2-025 ADR-031 latency gate guards the save-time budget. GV2-028 (production parser feed) is Done.                                                                    |
| **Default-on daemon routing** (ADR-075)       | Flip `ANVIL_WATCH_DAEMON` to default-on for `check` watches so the save-time fix reaches every user — **with rollout controls** (opt-out env, daemon-presence/auto-start guard, revert signal, staged rollout).                                                                                                                  | **Merged** — DSV-021 (#2473): unset routes only when a live daemon answers, `ANVIL_WATCH_DAEMON=0` opts out, and `=1` preserves forced diagnostics; release runbook must exercise the opt-out/revert signal before tagging (gate recorded in the daemon runbooks, 2026-06-10).                                                        |
| **User-journey golden paths** (UJ)            | Make the two beta golden paths strong and self-guiding before the release that flips daemon routing default-on reaches users: `anvil welcome` discovery and `anvil start` daily value — next-step threading, save-time posture visibility, entry-doc rewrite, upgrader reach. CIB-054 (changelog wording) rides pre-tag with it. | **In progress** — module filed 2026-06-10 ([user-journey](./plans/modules/user-journey.aps.md), Ready, 0/11). **Operator decision 2026-06-10: required before the `v0.8.0-beta` cut.** Design-gated UJ-004/-007/-011 satisfy the gate by a recorded decision (including "current posture affirmed").                                  |
...
- **A→A′ swap proven** — GV2-027 verdict-parity property test green vs the
  interim backing; the **GV2-025 latency gate** (ADR-031 save-time budget, named
  CI job on a quiet-box runner) green on the canonical corpus; **and GV2-028
  (production parser feed) Done** (else `ContentModify` stays `partial`). All
  three are hard gates, not prose.
...
User-journey bar (operator decision, 2026-06-10):

- **The [user-journey module](./plans/modules/user-journey.aps.md) (UJ) ships
  before the cut** — both beta golden paths (`anvil welcome` discovery,
  `anvil start` daily value) strong and self-guiding, plus CIB-054's
  changelog-wording corrections. The design-gated items (UJ-004 auth-wall
  placement, UJ-007 offer-to-start, UJ-011 tutorials shaping) satisfy the bar by
  a recorded decision; "current posture affirmed" is an acceptable outcome.
