...
release that turns daemon routing default-on also carries the user to it (both
beta golden paths — `anvil welcome` and `anvil start` — strong and
self-guiding).
...
| **GV2 A′-critical-path foundation** (ADR-075) | The GV2-027 dependency closure: GV2-010 (semantic schema), 011 (incremental hot indexes), 012 (trust/policy contract), 022 (hot-read API + guardrails), 024 (hot-read type split), 025 (Criterion CI gate), 028 (production parser feed), 029 (privilege containment).                                                           | **Merged** — the full closure landed on the #2442/#2446 swap wave (GV2-022/027/028/029) plus GV2-024 (hot-read type split + seal + ADR-077 depth cap, #2470) and GV2-025 (Criterion p95 / ADR-031 CI gate, #2459). GV2 13/20; the open balance is non-critical-path (v0.9 deferrals + Sub-phase B).  |
| **A→A′ backing swap** (ADR-075)               | GV2-027 retires the interim re-derive; `validate_paths`/`save_time` read the resident GV2 hot-index under the **unchanged wire**; `backing_schema_version` → `gv2-hotindex-v1`.                                                                                                                                                  | **Merged** — GV2-027 landed on the #2446 wave; `validate_paths` certifies through the resident GV2 hot-read index with the verdict-parity proof green, and the GV2-025 Criterion gate guards the latency budget. GV2-028 (production parser feed) is Done.                                           |
