...
## Active window — `v0.9.0-beta` "The Assistant-Facing Graph" (JOURNEY cut in progress)
...
**Operator usefulness review (2026-06-29):** do not cut `v0.9.0-beta` as a pure
assistant-graph release if the default-on daemon still feels like passive
...
[`ACTMO-013..020`](./plans/modules/activation-mcp-optional.aps.md) and
[`DSV-046..051`](./plans/modules/daemon-save-time-validation.aps.md); their
implementation is Merged. JOURNEY now owns the candidate-level proof that this
substrate produces a compelling first run and trustworthy daily use.
...
| **Default-on daemon usefulness addendum** | Registration UX (ACTMO-014..020 **Merged**) + headless save-time driver (DSV-046 design **Done**, DSV-047..051 **Merged**; [ADR-101](./plans/decisions/101-headless-save-time-driver.md)). ACTMO-021 tray app remains Proposed/non-blocking.                                                                                                                                                                                        | **implementation complete; release evidence pending** — `anvil start` from a worktree registers it and runs unattended save-time validation; later worktrees have explicit and guided registration; status reports `save_time_driver`, membership, and assurance truthfully. JOURNEY-005 reuses this substrate in the candidate rehearsal. |
| **Release user journeys** (JOURNEY)       | [`release-user-journeys`](./plans/modules/release-user-journeys.aps.md): WOW-005; ACTTUI-009/-010/-012; CIB-073/-183/-184/-190; DSV usefulness cut-line; Linux/macOS/Windows candidate rehearsal; outcome-based final gate. WOW-006, richer ACTTUI diagnostics/celebration, always-on visibility, and DASH continuity remain post-cut expansion.                                                                                    | **accepted 2026-07-11; implementation pending** — prove repository-specific value within 60 seconds, no-documentation activation, terse healthy repeat use, privacy-safe share evidence, one recovery owner per failure, and the full journey matrix on the tagged source SHA.                                                             |
...
- If the usefulness addendum is promoted into the `v0.9.0-beta` cut-line:
  ...
  registration must be a heartbeat/update; there must be a guided opt-in path
  for automatic `anvil start --no-mcp` when a Worktrunk/Git worktree is created;
  any global auto-discovery mode must be opt-in and limited by explicit config
  that identifies in-scope apps/workspaces; and status output must distinguish
  `watching` from MCP-backed `protecting`.
- JOURNEY-006 passes on the candidate SHA: a supported repo reaches truthful
  repository-specific value within 60 seconds; WOW-005 provides the real-finding
  first win; ACTTUI-009/-010/-012 and CIB-184 prove working explicit consent;
  CIB-183/-190 prove terse healthy repeat use; CIB-073's share format passes
  redaction; and Linux, macOS, and Windows fresh/restart/repeat/recovery
  journeys are recorded with one recovery owner per unsuccessful ending.
