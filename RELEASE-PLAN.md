...
- **Active window:** `v0.9.0-beta` (below, **scoping**). Direction confirmed by
  ...
  bug-fixing, with USAGE added as additive, start-deferred scope. The ADR-075
  entry decisions **landed 2026-06-15** (ADR-083 Accepted + the context-egress
  privacy review (PV-9) filed), clearing the gate — the window is ready to move
  from `scoping` to active on operator confirmation. Records for
  `v0.6.x`–`v0.8.x` are in [`plans/releases/`](./plans/releases/).
...
execution-ready now; the window stayed `scoping` only until the GCTX entry
decisions landed — **both landed 2026-06-15**
...
| **Assistant graph product** (GCTX)       | `graph-context-delivery` module (0/13): context delivery to assistant callers over the existing MCP surface.                                                                                                                                                                                                                                                                                                                        | **entry gates cleared 2026-06-15** — module promoted **Ready** (0/13); GCTX-002 Ready; GCTX-001 contract (folds CE-1..CE-12; CE-1 + CE-5 hard gates) is the next pickup, then the Phase 1 tools.                        |
| **Graph consumer surface** (GV2 balance) | GV2-013 (control/session), GV2-014 (plan/provenance), GV2-026 (reverse-impact lever) — internal substrate, deps Merged; GV2-020 (multi-graph registry) + GV2-023 (consumer query contract) layer on top.                                                                                                                                                                                                                            | GV2-013 / -014 / -026 **Ready** (promoted 2026-06-13); **GV2-020 + GV2-023 promoted Ready 2026-06-15** (GV2-020 deps GV2-010..014 Merged; GV2-023 the MCP/weave consumer contract, governed by the PV-9 egress review). |
