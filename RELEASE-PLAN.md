...
it derives from Accepted ADR deferrals, and the gated entry decisions below
must land before any phase starts coding.
...
- Entry decisions recorded: GCTX-002 ADR Accepted, egress-privacy review
  passed, operator scope confirmation noted here.
- The GCTX slice ships behind the existing daemon trust boundary — no new
  policy engine on the hot path (ADR-061 §6 / ADR-064 invariants hold).
- ADR-031 latency gate (GV2-025 CI job) stays green — context delivery must
...
| ---------------------- | ------------------------------------- | --------------------------------------------------------------------- |
| `v0.8.x` patch         | Weekly while user signal is non-empty | Bug fixes, false-positive reductions, doc corrections.                |
| `v0.8.x` patch         | Within 48h of any P0 bug              | Crash, data loss, false-claim regression, daemon corruption.          |
