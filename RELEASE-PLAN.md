...
| `MLP-007` baseline command                    | After 001 + 002   | Done (2026-05-13) | v1 library primitive shipped via `crates/anvil-baseline/`: `Baseline` schema + move-resistant `compute_fingerprint` + TOCTOU-hardened I/O + diff partition; 44 tests green. CLI command, scanner integration, cutoff_commit policy pinning, witness genesis emission, hook install deferred to consumers (MLP-003 / MLP-006). |
