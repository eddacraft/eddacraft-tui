...
eight of nine items (DASHCORE-001 via PR #3363, DASHCORE-003..009 via PR #3379),
leaving **DASHCORE-002** (retained history and trends) as the only open item —
Ready with an approved spec. Until it lands the dashboard reports current state
only, because `.anvil/gates.json` is latest-run by design. No dashboard
_product_ slice is named in the phase plan below; scoping one is an operator
decision, not a fact this document can record on its own. JOURNEY's own post-cut
...
| **Dashboard core views** (DASHCORE) | Product routes on latest-gate evidence: DASHCORE-001, and DASHCORE-003 through DASHCORE-009. DASHCORE-002 (retained history and trends) is Ready with an approved spec but **not scoped into this window**. | 8/9 Merged (PR #3363, PR #3379) — DASHCORE-002 open, awaiting a scope decision                                                      |
...
| **v0.9 follow-through** (lane)      | Beta-signal fixes on the shipped first-run/daemon/graph surfaces (48h-P0 patch lane on `v0.9.x`); CIB-193/-194/-195/-196 and the release-recovery hardening (#3309).                                        | CIB-193/-194/-195/-196 merged (2026-07-12, 2026-07-26 via #3422); #3309 still open — the lane stays the vehicle for anything urgent |
