...
| ------------ | --------- | ----------- | ------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Release plan | Derived   | APS modules | Live   | Reconciled 2026-07-04: assistant-graph scope complete; usefulness addendum **accepted** — ACTMO-014..020 Merged (registration UX), DSV-046 design Done ([ADR-101](./plans/decisions/101-headless-save-time-driver.md) Accepted), DSV-047..051 Ready (headless driver implementation cut-line). `v0.9.0-beta` cut blocked on DSV-047..051 landing, not on further design. (Prior: 2026-06-29 candidate addendum under review; 2026-06-26 scoped modules complete.) |
...
- **Active window:** `v0.9.0-beta` (below, **assistant-graph scope complete;
  default-on daemon usefulness addendum under APS review**). Direction confirmed
  by the operator 2026-06-13 — assistant-facing graph (ADR-075 deferrals) +
  bug-fixing, with USAGE added as additive scope. The ADR-075 entry decisions
  landed 2026-06-15 (ADR-083 Accepted + the context-egress privacy review (PV-9)
  filed); the scoped feature work then completed — as of 2026-06-26 every scoped
  module is Merged/Done (see the phase plan). What remains is the cut itself:
  the standing base bar + latency gate (cut criteria below) and operator go.
  Records for `v0.6.x`–`v0.8.x` are in [`plans/releases/`](./plans/releases/).
...
[`DSV-046`](./plans/modules/daemon-save-time-validation.aps.md). Treat these as
design gates first: if accepted, split the implementation work into Ready APS
items before treating the addendum as committed release scope.
...
| **Default-on daemon usefulness addendum** | Registration UX (ACTMO-014..020 **Merged**) + headless save-time driver (DSV-046 design **Done** 2026-07-04, [ADR-101](./plans/decisions/101-headless-save-time-driver.md); implementation **DSV-047..051 Ready**). ACTMO-021 tray app remains Proposed/non-blocking. | **implementation in flight 2026-07-04** — design accepted; cut-line requires DSV-047..051. Target shape unchanged: `anvil start` from a worktree registers it and runs unattended save-time validation (daemon-supervised `--save-time-driver` sidecars); `anvil workspace register [path]` attaches later worktrees; status reports `save_time_driver`, membership, and assurance truthfully. |
