...
Each **Notes** cell opens with a wiring marker recording whether the namespace
is actually emitted by anvil code today: **Wired** (emitted in-tree), **Not yet
wired** (registry stub / provisional), or **Out-of-tree** (owned by a partner
system, enforced by review rather than anvil code). Membership is still enforced
by founder-reviewed PR — there is no build-time validation hook (see V060F-014).

| Namespace           | Owner / origin                                                                       | Pipe(s)                                                                 | Notes                                                                                                                                                                                                                                                                                                                                   |
| ------------------- | ------------------------------------------------------------------------------------ | ----------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `anvil.flags.*`     | FLAGS module / [ADR-019](../../plans/decisions/019-flags-observability-alignment.md) | Tracing (per-eval), Kindling (gate-affecting outcomes)                  | **Wired** (FLAGS module Complete; ADR-019). Routine evaluations on tracing; only gate-affecting outcomes earn a Kindling row.                                                                                                                                                                                                           |
| `anvil.cli.*`       | CLI / TRACE-004                                                                      | Tracing                                                                 | **Wired** (TRACE-004 — `cli.command` span). Command-entry debugging spans; local diagnostics only, not a governance pipe.                                                                                                                                                                                                               |
| `anvil.intercept.*` | Intercept daemon / TRACE-004                                                         | Tracing                                                                 | **Wired** (TRACE-004 — `jsonrpc.*` spans). JSON-RPC dispatch and scan-buffer debugging spans.                                                                                                                                                                                                                                           |
| `kindling.*`        | Kindling system (Edda Stack)                                                         | Kindling                                                                | **Out-of-tree** (Edda Stack; membership enforced by founder-reviewed PR, not anvil code). Governance facts only; write-once, query-shaped. Source-of-truth for governance.                                                                                                                                                              |
| `anvil.rtai.*`      | RTAI module (provisional)                                                            | Tracing                                                                 | **Not yet wired** — registry stub only (TRACE-001). Provisional namespace pending RTAI promotion.                                                                                                                                                                                                                                       |
| `anvil.usage.*`     | USAGE module / [ADR-035](../../plans/decisions/035-three-pipe-observability-rule.md) | Kindling (source-of-truth); Tracing (existing `cli.command` breadcrumb) | **Wired** (USAGE-001, live 2026-06-14). Command-invocation usage rows (`command.invoked`) — redacted argument shapes + anonymised principal. The Kindling row is the source-of-truth; USAGE-001 adds no new tracing attributes, reusing TRACE-004's `cli.command` span as the breadcrumb. See [usage-analytics.md](usage-analytics.md). |
