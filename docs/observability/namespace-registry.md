...
| `anvil.usage.*`     | USAGE module / [ADR-035](../../plans/decisions/035-three-pipe-observability-rule.md) | Kindling (source-of-truth); Tracing (existing `cli.command` breadcrumb) | Command-invocation usage rows (`command.invoked`) — redacted argument shapes + anonymised principal. The Kindling row is the source-of-truth; USAGE-001 adds no new tracing attributes, reusing TRACE-004's `cli.command` span as the breadcrumb. See [usage-analytics.md](usage-analytics.md). |
