...
| **Skill behaviour**     | `anvil-developer-functions` skill                                       | Teach the agent *when* to use the above                   |
...
| **HTML vs Markdown APS token experiment** (archived)   | Plan *format* changes token cost of loading APS modules                              | Not Anvil-on vs Anvil-off; format experiment only                                   |
...
exist to attribute *why* a win happened (context vs gate vs skill).
...
- **Tier A (scripted):** use `estimate_gctx_tokens` (`gctx-simple-v1`) for
  GCTX payloads **and** the same estimator for file-read baselines so ratios
  stay internally consistent. Disclose estimator bias (lexical vs `bytes/4`)
  exactly as GCTX-031 does.
- **Tier B (live agent):** prefer the provider's official token counts for the
  ...
  cross-model comparability of *payload* cost only — never mix the two into one
...
- Attribution split: fraction of token delta explained by
  `tokens_file_reads` ↓ vs other
...
**What:** No LLM. A fixed *tool script* (ordered list of tool calls + expected
...
Usage analytics and human diary studies can *corroborate* Tier B but never
...
(optional later under a separate fairness protocol), default PR-blocking
DEVACC gates.
...
2. **Whether Tier A edit scenarios count "ideal tool scripts" as treatment.**
   Ideal scripts measure *ceiling* savings if the agent uses tools perfectly;
   Tier B measures *achieved* savings. Both are useful; label them
   `ceiling` vs `achieved`.
...
- [ ] `full-accel` runs record skill install digest when skill is part of the arm
