...
PocketFlow is a **100-line minimalist LLM framework** for building Agents, Task
Decomposition, RAG, and more. It provides:

- **Lightweight**: Core graph abstraction in ~100 lines, zero dependencies
- **Expressive**: Agents, Workflows, RAG, MapReduce patterns
- **Agentic-Coding**: Simple enough for AI agents to help build complex LLM
  applications
...
The `@eddacraft/kindling-adapter-pocketflow` package provides `KindlingNode` and
`KindlingFlow` that automatically capture observations:
