...
>   `packages/adapters/src/aps-markdown/`; the Kindling capture / observation
...
in a single chain. `packages/adapters/src/aps-markdown/` imports
...
`plans/archive/modules/launch-flow-readiness.aps.md` ships SpecKit as the
flagship format in `v0.6.0-beta`). Largest adapter in the package by file count
and test surface.
...
`packages/adapters/src/aps-markdown/`, and by tooling that generates / validates
`.aps.md` documents.
...
`packages/adapters/src/aps-markdown/` both depend on it.
...
`packages/aps/scripts/generate-templates.js`). Three variants × three document
types = nine files (`packages/aps/templates/`, `packages/aps/AGENTS.md:140`):
...
`packages/aps/scripts/generate-templates.js:19`). Templates are consumed by the
(future) `anvil aps init` family — the Rust CLI's APS init command reads from
...
Single script: `packages/aps/scripts/generate-templates.js` (33 lines). Imports
...
`./templates`, `packages/aps/scripts/generate-templates.js:12`). Wired through
...
Single script: `packages/kindling-integration/scripts/generate-openapi.ts` (576
lines). Generates an OpenAPI 3.1 spec from the four query endpoints by
**manual** schema extraction (no `zod-to-json-schema` dep,
`packages/kindling-integration/scripts/generate-openapi.ts:6-7`):
...
by the build pipeline + `packages/adapters/src/aps-markdown/`.
`packages/adapters/` is **TS only**: SpecKit / BMAD / Generic / APS- markdown
all run in the JS toolchain (linked into Rust via the agent-facing CLI shell,
not in-process). `packages/kindling-integration/` is **TS only** — the Rust CLI
calls into Kindling through the SQLite file directly when it needs to (a Rust
port is not on the slate per
...
`packages/aps/scripts/generate-templates.js`, and it depends on `dist/` so it
requires a build first. The `anvil validate` Rust CLI is the operator-facing
surface; there's no `anvil-aps validate` or similar direct wrapper, which means
users wanting to validate `.aps.md` files **without the Rust CLI** have to
import the library programmatically. **Risk:** Low — by design, but worth
flagging for operators expecting a standalone tool.

### G-07: `packages/adapters/src/common/` legacy shape lingers
