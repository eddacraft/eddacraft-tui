...
| `detail`          | enum: `full` / `minimal`                        | optional                                    | Response envelope detail (RMCPF-040/042). Request overrides env `ANVIL_MCP_VALIDATE_DETAIL`. Default **full** until RMCPF-043 flips it. On clean `allow` with `minimal`, the body is only `{ schema, decision }`; warn/veto/error keep full actionable payloads. Same field on `anvil_apply_patch`.                                                                                                                                                                                                     |
