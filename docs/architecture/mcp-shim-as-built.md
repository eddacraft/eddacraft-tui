...
| `detail`          | enum: `full` / `minimal`                        | optional                                    | Response envelope detail (RMCPF-040/042/043). Request overrides env `ANVIL_MCP_VALIDATE_DETAIL`. Default **minimal** (RMCPF-043): clean `allow` body is only `{ schema, decision }`. `full` restores summary/diagnostics/correlation/claim/tier. Warn/veto/error keep full actionable payloads. Same field on `anvil_apply_patch`.                                                                                                                                                                      |
