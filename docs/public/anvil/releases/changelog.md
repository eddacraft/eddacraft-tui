...
A patch on the `v0.8.x` daemon lane. It made the default-on daemon path easier
to recover and exercise, especially around interactive watch startup and Windows
daemon ensure handling.

### Added

- **`anvil start` can auto-start the save-time daemon.** Interactive Unix runs
  start or reuse the per-user daemon instead of requiring an operator to launch
  `anvil intercept start --foreground` first.
- **`anvil watch` can offer to start the daemon.** When no daemon answers, an
  interactive watch run prompts before starting one; non-interactive, JSON, CI,
  hook, and piped runs keep falling back deterministically.
...
- **Portable review capsules (`anvil capsule`).** New
  ...
  diagnostics, and an `anvil.capsule.v1` SHA-256 digest manifest (ADR-074), so a
  reviewer, auditor, or supplier can verify the range locally without trusting
  Anvil Cloud. `inspect`, JSON CI output, tamper-test hardening, retention/prune
...
- **Resource budgets now cover the save-time path.** New CPU/RSS benches measure
  ...
  SLO envelope for future regressions.
...
- **`ANVIL_HOME` / `--anvil-home` install-root override.** Anvil now resolves a
  ...
  [side-by-side install runbook](https://github.com/eddacraft/anvil-001/blob/main/docs/runbooks/anvil-home-side-by-side.md).
...
delta is additive; the [upgrade notes](./upgrade-notes.md#upgrading-to-070-beta)
call out the few places where operator action or expectation needs to shift.
...
- [0.7.0-beta upgrade notes](./upgrade-notes.md#upgrading-to-070-beta) — surface
  delta and operator action.
- [v0.7.0-beta release runbook](https://github.com/eddacraft/anvil-001/blob/main/docs/runbooks/v0.7.0-beta-release-runbook.md)
...
- **Operator artefacts** — the release shipped a `v0.6.0-beta` operator runbook
  (five operator items) and security note (four HIGH security trade-offs
  documented for review). Public docs no longer link directly into archived
  runbooks.
