...
anvil 0.9.0-beta. Secrets, architecture, policy, command-safety, and other gate
checks have separate engines and are not listed here. The registry contains **44
enabled rules across 10 families**.
...
| `WC-002` | Broken cipher or ECB mode in a construction call               | weak-cryptography         | warning          | .ts, .tsx, .js, .jsx, .mjs, .cjs, .py, .java    |
| `WC-003` | JWT configured with the `none` algorithm                       | weak-cryptography         | warning          | .ts, .tsx, .js, .jsx, .mjs, .cjs, .py, .java    |
