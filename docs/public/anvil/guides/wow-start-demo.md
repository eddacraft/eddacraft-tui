...
description: install → cd repo → anvil start. The first-minute story for v0.6.0-beta.
sidebar_position: 1
---
...
- **`ready_restart_required`** — MCP config was written safely and the server
  starts, but the editor or agent has to restart so the MCP entry attaches.
  This is **not** protection yet — restart is still required.
- **`watching`** — pre-write MCP attachment is not in evidence; the kernel
  ...
  surface says so.
- **`needs_action`** — anvil cannot make a literal protection claim and you
  have a concrete next step (run `anvil init`, install a supported editor,
  etc.).
...
sentence somewhere else that walks it back. Surfaces never invent ad-hoc
phrases like "config written" or "almost protected"; the six literals above are
...
- A real TypeScript or JavaScript project. SQL and Markdown get partial
  coverage; Python and Rust are unsupported in this release and will land you
  on `unsupported` honestly.
- A clean working tree. `anvil start` is idempotent, but a clean tree makes the
  diff between "what was there before" and "what activation just wrote"
  obvious.
...
when anvil has actually observed an `anvil_validate_write` call land. Until
that evidence arrives, the diagnostic stops at `ready_restart_required`. This
is by design: a `protecting` claim that hasn't been earned would be the
worst-case lie for a governance tool.
...
This is the most common first-run outcome. anvil wrote `~/.cursor/mcp.json`
and `~/.claude.json`, the server starts cleanly, but the editor is holding the
old MCP server list. Restart it once. The next `anvil start --verify` should
move you to `ready_restart_required` → `protecting` once a write actually flows
...
Watch fallback is offered; it is **not** running yet. To run it: `anvil start
--watch` (covered below).

> **Note on the sample output above.** The exact line spacing, label widths,
> and per-client tier strings come from `crates/anvil-cli/src/activation/render.rs`
...
> activation probes evolve. The `state:` literal and the six allowed values
> are pinned by tests and will not drift.
...
2. **First-scan via the language profile.** Walk the working tree (excluding
   ...
   each detected language as `supported` / `partial` / `unsupported` against
   the registered language pack, and record a small baseline of existing
   findings so future runs can diff new edges only.
...
4. **Run the activation diagnostic.** This is the read-only probe that
   produces the `ActivationDiagnostic` rendered above — config status, MCP
   tier per client, watch tier, baseline summary, language profile,
   `last_error` (if any).
5. **Emit one literal `state:`** from the six-word vocabulary, based on the
   diagnostic. The headline copy comes from `ProtectionState::headline()` so
   the wording cannot drift between `start`, `status`, and `doctor`.
...
- It does not inject `.cursorrules` / `.clauderules` / any global AI rule
  file.
...
`--verify` skips init, the first-scan, and the MCP install step. It forwards
to the same backend as `anvil status --verify` (LAUNCH-012) — the diagnostic is
...
- Re-checking the state after restarting your editor (the most common path
  from `ready_restart_required` → `protecting`).
...
single-JSON-document contract. anvil rejects both combinations explicitly with
a hint.

## The watch fallback — `anvil start --watch`

When MCP pre-write validation is not live (no Cursor / Claude Code installed,
or the editor refused to load the server), the kernel watcher is the
**save-time fallback**:
...
Each case prints a state-specific reason instead of running a watcher that
would generate noise without findings:
...
If you want save-time signal anyway in a state where `--watch` has skipped,
the unconditional path is `anvil watch --source` (covered in
...
This is the same vocabulary used by `anvil status --verify`, `anvil doctor`,
and the protection-loop tutorial path. There is one renderer; surfaces cannot
drift.
...
The wow-start gets you to `protecting` (or the honest reason you are not
there). The guardrail demo is what protection actually looks like when an AI
agent collides with it.

## What is honest in v0.6.0-beta

The wow-start is a real product surface, not a recording-only setup, but
several things you might expect from the brainstorm phase are explicitly **not
...
- **No rule-file injection.** `.cursorrules`, `.clauderules`, and global AI
  rule files are user-owned. anvil does not edit them.
...
[v0.6.0-beta release runbook](https://github.com/eddacraft/anvil-001/blob/main/docs/runbooks/v0.6.0-beta-release-runbook.md) for the full list):
...
If you are recording the demo and want a fresh-state run, the reset path
...
For finer-grained per-scenario resets (between AI Guardrail Demo scenarios A /
B / C without nuking everything), see
...
- [Beta testing guide](/anvil/beta-testing-guide) — what we're asking testers
  to try and how to send feedback.
