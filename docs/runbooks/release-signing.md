...
- The workflow can normalise legacy forms whose outer base64 decodes to only the
  key-material line or to the raw binary key blob. New rotations must store the
  complete two-line key file shown here.

```sh
base64 < anvil-release.key | tr -d '\r\n' | pbcopy  # macOS
base64 < anvil-release.key | tr -d '\r\n' | xclip   # Linux
