...
| ------- | ------------- | ------ | ------ | ------------------------------------------------------------- |
| Manpage | Authoritative | @aneki | Live   | First filed 2026-05-18 as N4 doc-lane closure for v0.7.0-beta |
...
- **Fish shell** is unsupported. Source `shell/anvil-run.sh` only from zsh or
  bash.
- **macOS interrupt path** is fence-first this release: the launcher does not
  run the full SIGINT → SIGTERM → SIGKILL sequence on macOS. See
