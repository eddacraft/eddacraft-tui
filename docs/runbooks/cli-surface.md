...
`esc` backs out, `q` quits. Mouse: click selects, drag pans the canvas (or moves
a node), the wheel zooms at the cursor; mouse capture is released on every exit
