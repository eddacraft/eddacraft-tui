...
| `resource-budget.yml`                  | Integration + Assurance + Release candidate | `push` to `main` (crate and resource-budget policy paths), nightly `schedule` (17:15 UTC), `workflow_dispatch`, and `workflow_call` from `release-readiness.yml` — **not** on routine PRs                                                                                                                                                                  | ADOPT        |
...
| `ci-nightly.yml` `Unit Tests`     | `schedule` (daily 17:00 UTC) or `workflow_dispatch`                                                                                                                                                                                                                                              | Routine PR and integration push events — nightly assurance owns scheduled evidence |
...
`push` to `main`, nightly schedule (17:15 UTC), manual dispatch, and as a
