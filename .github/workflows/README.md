...
full cross-compile matrix. Prefer nightly for continuous signal and
release-readiness for cut-blocking gates.
...
| `resource-budget.yml`                  | Integration + Assurance + Release candidate | `push` to `main` (crate and resource-budget policy paths), nightly `schedule` (02:15 UTC), `workflow_dispatch`, and `workflow_call` from `release-readiness.yml` — **not** on routine PRs                                                                                                                                                                  | ADOPT        |
...
| `ci-nightly.yml`                       | Assurance                                   | `schedule` (daily 02:00 UTC) plus `workflow_dispatch` — coverage (TS + Rust), expanded matrices, multi-version Node, cross-compile assurance; resource budgets run as a sibling nightly via `resource-budget.yml` @ 02:15                                                                                                                                  | CICD         |
...
- `rust.yml` `Doc` vs the eddacraft-tui rustdoc step in `rust-tests.yml` `Test`
  ...
  `Doc` was removed; `Test` is now the sole PR-side owner. Coverage is
  unchanged, and does not rely on `rust.yml`'s path filter: `rust-tests.yml`
  triggers on every PR and gates its heavy steps on an internal
  `dorny/paths-filter` whose `rust` filter lists the same Rust path set
...
ADOPT-002 resource-budget gate for `anvil watch` and related RLB/GCALL/GV2
latency budgets. Builds the release CLI, runs the per-process benches, uploads
JSON verdicts, and fails when steady-state CPU, peak RSS, or latency SLOs exceed
the pins in `docs/policies/resource-budget.md`.

**Triggers (de-bloated 2026-08):** not on `pull_request`. Runs on path-filtered
`push` to `main`, nightly schedule (02:15 UTC), manual dispatch, and as a
reusable workflow from `release-readiness.yml` so a release cut still cannot
ship a red budget.
...
`resource-budget.yml` (02:15 UTC) rather than duplicated here.
...
SHA validation): format/lint/script contracts, publication-token preflight,
