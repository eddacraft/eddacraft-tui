...
export async function collectGitContext(workspaceRoot: string): Promise<GitContext | undefined> {
  ...
  try {
    const [branch, commit, commitMessage, author, status, stagedFiles] = await Promise.all([
      gitCurrentBranch(workspaceRoot).catch(() => undefined),
      gitRevParse(workspaceRoot).catch(() => undefined),
      gitLastCommitMessage(workspaceRoot).catch(() => undefined),
      gitLastCommitAuthor(workspaceRoot).catch(() => undefined),
      gitStatusPorcelain(workspaceRoot).catch(() => ''),
      gitStagedFiles(workspaceRoot).catch(() => []),
    ]);
    ...
    try {
      repository = await gitRemoteUrl(workspaceRoot);
    } catch (error) {
...
export async function detectAITool(workspaceRoot: string): Promise<AITool | undefined> {
  ...
  try {
    const { stdout } = await gitExec(['log', '-5', '--format=%s'], { cwd: workspaceRoot });
...
export async function createProvenanceRecord(params: {
  ...
  try {
    const { stdout } = await gitExec(['config', 'user.name'], { cwd: workspaceRoot });
    user = stdout || undefined;
  } catch (error) {
