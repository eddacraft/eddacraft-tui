...
`;
    ...
    it('should detect generated policies by legacy capitalised header', async () => {
      const policyContent = `# Auto-generated by Anvil - do not edit manually
...
violation contains result if {
  true
  result := {"message": "legacy generated"}
}
`;
      writeFileSync(join(policyDir, 'legacy.rego'), policyContent);

      const result = await loader.loadPolicies(tempDir);

      expect(result.policies).toHaveLength(1);
      expect(result.policies[0].name).toBe('legacy');
      expect(result.policies[0].generated).toBe(true);
      expect(result.policies[0].sourceHash).toBe('abcdef0123456789');
    });
