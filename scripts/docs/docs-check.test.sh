...
# Case 1: orchestrator surfaces the twelve expected labels in summary order.
echo "case 1: orchestrator emits all twelve surface labels"
out="$(cd "${repo_root}" && node "${orchestrator}" 2>&1 || true)"
for surface in metadata tags links public-docs public-diagrams aps adr index-freshness asbuilt-paths docs-owed release-plan retired-claims; do
...
if grep -qE "^  (pass|FAIL|ERROR \(tooling\))[[:space:]]+retired-claims$" <<<"${out}"; then
  pass "all twelve surfaces present in summary"
fi
...
out="$(cd "${repo_root}" && node "${orchestrator}" 2>&1 || true)"
if echo "${out}" | grep -qE "^\[docs-check\] 12/12 surfaces passed"; then
  pass "live repo passes all twelve surfaces under baseline"
else
  fail "live repo expected 12/12 passed; got tail: $(echo "${out}" | tail -3)"
fi
