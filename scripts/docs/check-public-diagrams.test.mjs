...
const OFFICIAL_DRAWIO_SVG_PROLOG =
...
function officialRawSvg(source) {
  return `${OFFICIAL_DRAWIO_SVG_PROLOG}${rawSvg(source)}`;
}
...
async function fixture({ rawSvgForSource = rawSvg } = {}) {
...
async function findingsFor(mutate = async () => {}, fixtureOptions) {
  const state = await fixture(fixtureOptions);
  try {
    await mutate(state);
...
test('Draw.io prolog normalisation remains exact and fail-closed', async (t) => {
  const declaration = '<?xml version="1.0" encoding="UTF-8"?>\n';
  const comment = '<!-- Do not edit this file with editors other than draw.io -->\n';
  const cases = [
    [
      'altered public doctype',
      `${declaration}${comment}<!DOCTYPE svg PUBLIC "-//W3C//DTD SVG 1.1//EN" "https://example.invalid/svg11.dtd">\n`,
    ],
    [
      'entity-bearing doctype',
      `${declaration}${comment}<!DOCTYPE svg [<!ENTITY xxe SYSTEM "file:///etc/passwd">]>\n`,
    ],
    [
      'system doctype',
      `${declaration}${comment}<!DOCTYPE svg SYSTEM "https://example.invalid/svg.dtd">\n`,
    ],
    ['other processing instruction', '<?xml-stylesheet href="https://example.invalid/a.css"?>\n'],
  ];
  for (const [name, prolog] of cases) {
    await t.test(name, async () => {
      const findings = await findingsFor(undefined, {
        rawSvgForSource: (source) => `${prolog}${rawSvg(source)}`,
      });
      assert.ok(findings.some(({ code }) => code === 'unsafe-svg'));
    });
  }
...
test('normalised official prolog remains covered by the deterministic export hash', async () => {
  const findings = await findingsFor(
    async ({ familyRoot }) => {
      const path = join(familyRoot, 'sample-flow.svg');
      await writeFile(path, (await readFile(path, 'utf8')).replace('<rect', '<circle'), 'utf8');
    },
    { rawSvgForSource: officialRawSvg }
  );
  assert.ok(findings.some(({ code }) => code === 'stale-export'));
});
...
test('export CLI enforces the pinned Desktop flags and writes the sibling SVG', async () => {
  const { root, familyRoot, contract } = await fixture();
...
if (args[0] === '--version') {
  ...
  const source = readFileSync(args.at(-1), 'utf8');
  const embedded = source.replaceAll('&', '&amp;').replaceAll('"', '&quot;').replaceAll('<', '&lt;').replaceAll('>', '&gt;');
  writeFileSync(output, ${JSON.stringify(OFFICIAL_DRAWIO_SVG_PROLOG)} + '<svg xmlns="http://www.w3.org/2000/svg" content="' + embedded + '"><rect/></svg>');
}
`,
    'utf8'
  );
  ...
  try {
    ...
    const exported = await readFile(join(familyRoot, 'sample-flow.svg'), 'utf8');
    assert.deepEqual(calls[0].slice(0, CONTRACT.exportArgs.length), CONTRACT.exportArgs);
    assert.doesNotMatch(exported, /<\?xml|<!DOCTYPE/);
    assert.deepEqual(await validatePublicDiagrams(root, contract), []);
    assert.match(exported, /data-drawio-version="31\.1\.8"/);
    assert.match(
