...
const OFFICIAL_DRAWIO_SVG_PROLOG_LINES = [
...
function normaliseOfficialDrawioSvgProlog(svg) {
  for (const newline of ['\n', '\r\n']) {
    const prolog = `${OFFICIAL_DRAWIO_SVG_PROLOG_LINES.join(newline)}${newline}`;
    if (svg.startsWith(prolog)) return svg.slice(prolog.length);
  }
  return svg;
}
