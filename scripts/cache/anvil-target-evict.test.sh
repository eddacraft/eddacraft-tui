...
# --- T4: a freshly-touched dir is skipped; a stale one is evicted ---
mkslug fresh1 0
mkslug stale1 99999
"$script" --apply --high-water 1 --low-water 0 --freshness-mins 30 >/dev/null 2>&1
check "fresh dir skipped" '[[ -d "$base/fresh1" ]]'
check "stale dir evicted" '[[ ! -d "$base/stale1" ]]'

# --- T5: below high-water => no-op ---
mkslug keep1 99999
"$script" --apply --high-water 100 --low-water 0 >/dev/null 2>&1
check "below high-water is a no-op" '[[ -d "$base/keep1" ]]'

# --- T6: unsafe base ($HOME) fails closed ---
check "refuses unsafe base (\$HOME)" '! ANVIL_TARGET_BASE="$HOME" "$script" --apply --high-water 1 --low-water 0 >/dev/null 2>&1'

# --- T7: low >= high is rejected ---
check "rejects low-water >= high-water" '! "$script" --high-water 50 --low-water 50 >/dev/null 2>&1'
